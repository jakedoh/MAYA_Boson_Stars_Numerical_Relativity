...
{
  int m = n - 1;
  //Create a covariant vector with m elements  
  (*v).d0 = dvector (0, m);
  (*v).d1 = dvector (0, m);
  (*v).d2 = dvector (0, m);
  (*v).d3 = dvector (0, m);
...
#pragma omp for
  for (i = 0; i < n1; i++)
  {
	...
	//BK: Compute the hamiltonian constraint violation and store in values[0]
	NonLinEquations (i, j, k, n1, n2, n3, A, B, X, R, x, r, phi, y, z, U, values,time);


  //Compute derivatives of v w.r.t A, B and phi	
	for (ivar = 0; ivar < nvar; ivar++)
	{
	  indx = Index (ivar, i, j, k, nvar, n1, n2, n3);
	  F[indx] = values[ivar] * FAC;  //BK: Why multiplication by FAC?	 
...
#pragma omp for schedule(dynamic)
  for (int i = 0; i < n1; i++)
  {
	...
	LinEquations (i, j, k, n1, n2, n3, A, B, X, R, x, r, phi, y, z, dU, U, values, time);
	for (int ivar = 0; ivar < nvar; ivar++)
	{
	  int indx = Index (ivar, i, j, k, nvar, n1, n2, n3);
...
JFD_times_dv (int i, int j, int k, int nvar, int n1, int n2,
	      int n3, derivs dv, derivs u, double *values, double time)
...
SetMatrix_JFD (int nvar, int n1, int n2, int n3, derivs u,
	       int *ncols, int **cols, double **Matrix, double time)
{
  DECLARE_CCTK_PARAMETERS;
  int N1, N2, N3, ntotal = nvar * n1 * n2 * n3;
  double *values;
  derivs dv;
			  ...
			  int nvar, int n1, int n2, int n3)
{
  ...
  N = maximum3 (n1, n2, n3);
  p = dvector (0, N);
  values1 = dvector (0, N);    		//allocate mmeory for double vector with N elements
  values2 = dmatrix (0, N, 0, N);	//allocate memory for metrix with (N,N) elements of type double
...
/* Calculates the value of v at an arbitrary position (A,B,phi)*/
double
PunctEvalCoeffsAtArbitPosition (double *v, int ivar, double A, double B, double phi,
        int nvar, int n1, int n2, int n3)
{
  int i, j, k;
  double *q, *r, result;
  ...
  for (k = 0; k < n3; k++)
  {
    for (j = 0; j < n2; j++)
    {
      ...
      double dip2, dip1, di; /* d_{i+2}, d_{i+1} and d_i */
			    ...
			    double z)
{
  ...
  //Find U(A,B,phi) = result using roots of Newton Solver (v.d0) i.e. v.d0 = vec{U} where \vec{F}(\vec{U})=0 
  result = PunctEvalAtArbitPosition (v.d0, ivar, A, B, phi, nvar, n1, n2, n3);

  Ui = (A - 1) * result; //Ui = ui (of paper)

  return Ui;
}

double
PunctIntPolForArbitVector (int ivar, int nvar, int n1,
			    int n2, int n3, double *v, double x, double y,
			    double z)
{
  DECLARE_CCTK_PARAMETERS;
  double xs, ys, zs, rs2, phi, X, R, A, B, aux1, aux2, result, Ui;
  ...
  result = PunctEvalAtArbitPosition (v, ivar, A, B, phi, nvar, n1, n2, n3);
			    ...
			    double z)
{
  ...
  result = PunctEvalCoeffsAtArbitPosition (v.d0, ivar, A, B, phi, nvar, n1, n2, n3);
...
double ValueAtArbitPosition(int ivar, int nvar, int n1, int n2, int n3, derivs v, derivs v_coeffs, double xx, double yy, double zz)
{
	...
	switch (gsm)
	{
		case GSM_Taylor_expansion:
			return PunctTaylorExpandAtArbitPosition(ivar, nvar, n1, n2, n3, v, xx, yy, zz);
			break;
		case GSM_evaluation:
			if (use_full_transform) {
				return PunctIntPolCoeffsAtArbitPosition
					(ivar, nvar, n1, n2, n3, v_coeffs, xx, yy, zz);
			} else {
			return PunctIntPolAtArbitPosition
					(ivar, nvar, n1, n2, n3, v, xx, yy, zz);
			}
			break;
		default:
...
gsm_type getGSM()
{
	...
	{
		return GSM_Taylor_expansion;
	}
	else if (CCTK_EQUALS(grid_setup_method, "evaluation"))
	{
		return GSM_evaluation;
	}
	else
	{
