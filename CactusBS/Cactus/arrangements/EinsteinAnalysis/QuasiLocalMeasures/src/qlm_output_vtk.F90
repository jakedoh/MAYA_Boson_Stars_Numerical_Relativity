...
subroutine qlm_output_vtk (CCTK_ARGUMENTS, hn, file_name)
  ...
  write (unit, '(A,X,I5,X,A)') 'POINTS', nptheta*npphi, 'float'
  ...
  write (unit, '()')
  write (unit, '(A,X,I5,X,I10)') 'POLYGONS', npphi*(nptheta-1), 5*npphi*(nptheta-1)

  do j=0, nptheta-2
     do i=0, npphi-2
        write (unit,'(I1,4(I6))') &
             4, j*npphi+i, (j+1)*npphi+i, (j+1)*npphi+i+1, j*npphi+i+1
     end do
     write (unit,'(I1,4(I6))') &
          4, j*npphi+npphi-1, (j+1)*npphi+npphi-1, (j+1)*npphi, j*npphi
  end do

  write (unit, '()')
  write (unit, '(A,X,I5)') 'POINT_DATA', nptheta*npphi
