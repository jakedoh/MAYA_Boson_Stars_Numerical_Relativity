...
extern "C" void bssn_FLRW_lapse3_SelectBCs(CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  
  if (cctk_iteration % bssn_FLRW_lapse3_calc_every != bssn_FLRW_lapse3_calc_offset)
    return;
...
static void bssn_FLRW_lapse3_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  CCTK_ENDLOOP3(bssn_FLRW_lapse3);
}
extern "C" void bssn_FLRW_lapse3(CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Entering bssn_FLRW_lapse3_Body");
  }
  if (cctk_iteration % bssn_FLRW_lapse3_calc_every != bssn_FLRW_lapse3_calc_offset)
  {
    return;
  ...
  const char* const groups[] = {
    ...
    "grid::coordinates",
    "Kranc2BSSN::K_group"};
  AssertGroupStorage(cctkGH, "bssn_FLRW_lapse3", 5, groups);
  
  switch (constraintsfdorder)
  {
    case 2:
    {
      EnsureStencilFits(cctkGH, "bssn_FLRW_lapse3", 3, 3, 3);
      break;
    }
  ...
  LoopOverInterior(cctkGH, bssn_FLRW_lapse3_Body);
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Leaving bssn_FLRW_lapse3_Body");
  }
}
