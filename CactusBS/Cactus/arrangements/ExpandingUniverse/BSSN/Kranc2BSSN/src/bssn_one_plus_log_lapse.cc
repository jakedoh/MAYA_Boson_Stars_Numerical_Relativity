...
extern "C" void bssn_one_plus_log_lapse_SelectBCs(CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  
  if (cctk_iteration % bssn_one_plus_log_lapse_calc_every != bssn_one_plus_log_lapse_calc_offset)
    return;
...
static void bssn_one_plus_log_lapse_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  #pragma omp parallel
  CCTK_LOOP3(bssn_one_plus_log_lapse,
  ...
  CCTK_ENDLOOP3(bssn_one_plus_log_lapse);
}
extern "C" void bssn_one_plus_log_lapse(CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Entering bssn_one_plus_log_lapse_Body");
  }
  if (cctk_iteration % bssn_one_plus_log_lapse_calc_every != bssn_one_plus_log_lapse_calc_offset)
  {
    return;
  ...
  const char* const groups[] = {
    ...
    "Kranc2BSSN::K_group"};
  AssertGroupStorage(cctkGH, "bssn_one_plus_log_lapse", 4, groups);
  
  switch (constraintsfdorder)
  {
    case 2:
    {
      EnsureStencilFits(cctkGH, "bssn_one_plus_log_lapse", 2, 2, 2);
      break;
    }
  ...
  LoopOverInterior(cctkGH, bssn_one_plus_log_lapse_Body);
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Leaving bssn_one_plus_log_lapse_Body");
  }
}
