...
extern "C" void bssn_project_constraints_psu_boundary_SelectBCs(CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  
  if (cctk_iteration % bssn_project_constraints_psu_boundary_calc_every != bssn_project_constraints_psu_boundary_calc_offset)
    return;
...
static void bssn_project_constraints_psu_boundary_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  #pragma omp parallel
  CCTK_LOOP3(bssn_project_constraints_psu_boundary,
  ...
  CCTK_ENDLOOP3(bssn_project_constraints_psu_boundary);
}
extern "C" void bssn_project_constraints_psu_boundary(CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Entering bssn_project_constraints_psu_boundary_Body");
  }
  if (cctk_iteration % bssn_project_constraints_psu_boundary_calc_every != bssn_project_constraints_psu_boundary_calc_offset)
  {
    return;
  ...
  const char* const groups[] = {
    "Kranc2BSSN::A_group",
    "Kranc2BSSN::h_group"};
  AssertGroupStorage(cctkGH, "bssn_project_constraints_psu_boundary", 2, groups);
  ...
  LoopOverBoundary(cctkGH, bssn_project_constraints_psu_boundary_Body);
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Leaving bssn_project_constraints_psu_boundary_Body");
  }
}
