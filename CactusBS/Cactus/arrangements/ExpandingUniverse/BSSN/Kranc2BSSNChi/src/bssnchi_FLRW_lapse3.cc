...
extern "C" void bssnchi_FLRW_lapse3_SelectBCs(CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  
  if (cctk_iteration % bssnchi_FLRW_lapse3_calc_every != bssnchi_FLRW_lapse3_calc_offset)
    return;
...
static void bssnchi_FLRW_lapse3_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  #pragma omp parallel
  CCTK_LOOP3(bssnchi_FLRW_lapse3,
    ...
    cctk_ash[0],cctk_ash[1],cctk_ash[2])
  {
    ...
    CCTK_REAL alpharhsL CCTK_ATTRIBUTE_UNUSED = 
      ...
      beta3L*PDlopsided4th3(&alpha[index]))*lapseAdvection - 2*alphaL*(KL + 
      3*Hubble)*(0.5 - 0.5*tanh((rL - Ro)*pow(sigma,-1))) - 
      0.16666666666666666*KL*pow(alphaL,2)*(1. + 1.*tanh((rL - 
      Ro)*pow(sigma,-1)));
    /* Copy local copies back to grid functions */
    alpharhs[index] = alpharhsL;
  }
  CCTK_ENDLOOP3(bssnchi_FLRW_lapse3);
}
extern "C" void bssnchi_FLRW_lapse3(CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Entering bssnchi_FLRW_lapse3_Body");
  }
  if (cctk_iteration % bssnchi_FLRW_lapse3_calc_every != bssnchi_FLRW_lapse3_calc_offset)
  {
    return;
  ...
  AssertGroupStorage(cctkGH, "bssnchi_FLRW_lapse3", 5, groups);
  
  switch (constraintsfdorder)
  {
    case 2:
    {
      EnsureStencilFits(cctkGH, "bssnchi_FLRW_lapse3", 3, 3, 3);
      break;
    }
  ...
  LoopOverInterior(cctkGH, bssnchi_FLRW_lapse3_Body);
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Leaving bssnchi_FLRW_lapse3_Body");
  }
}
