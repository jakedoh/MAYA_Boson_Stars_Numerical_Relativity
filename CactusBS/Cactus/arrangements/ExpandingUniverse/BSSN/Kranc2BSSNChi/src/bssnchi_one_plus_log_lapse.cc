...
extern "C" void bssnchi_one_plus_log_lapse_SelectBCs(CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  
  if (cctk_iteration % bssnchi_one_plus_log_lapse_calc_every != bssnchi_one_plus_log_lapse_calc_offset)
    return;
...
static void bssnchi_one_plus_log_lapse_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  #pragma omp parallel
  CCTK_LOOP3(bssnchi_one_plus_log_lapse,
  ...
  CCTK_ENDLOOP3(bssnchi_one_plus_log_lapse);
}
extern "C" void bssnchi_one_plus_log_lapse(CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Entering bssnchi_one_plus_log_lapse_Body");
  }
  if (cctk_iteration % bssnchi_one_plus_log_lapse_calc_every != bssnchi_one_plus_log_lapse_calc_offset)
  {
    return;
  ...
  AssertGroupStorage(cctkGH, "bssnchi_one_plus_log_lapse", 4, groups);
  
  switch (constraintsfdorder)
  {
    case 2:
    {
      EnsureStencilFits(cctkGH, "bssnchi_one_plus_log_lapse", 2, 2, 2);
      break;
    }
  ...
  LoopOverInterior(cctkGH, bssnchi_one_plus_log_lapse_Body);
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Leaving bssnchi_one_plus_log_lapse_Body");
  }
}
