...
extern "C" void bssnmatter_project_constraints_psu_boundary_SelectBCs(CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  
  if (cctk_iteration % bssnmatter_project_constraints_psu_boundary_calc_every != bssnmatter_project_constraints_psu_boundary_calc_offset)
    return;
...
static void bssnmatter_project_constraints_psu_boundary_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  #pragma omp parallel
  CCTK_LOOP3(bssnmatter_project_constraints_psu_boundary,
  ...
  CCTK_ENDLOOP3(bssnmatter_project_constraints_psu_boundary);
}
extern "C" void bssnmatter_project_constraints_psu_boundary(CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Entering bssnmatter_project_constraints_psu_boundary_Body");
  }
  if (cctk_iteration % bssnmatter_project_constraints_psu_boundary_calc_every != bssnmatter_project_constraints_psu_boundary_calc_offset)
  {
    return;
  ...
  AssertGroupStorage(cctkGH, "bssnmatter_project_constraints_psu_boundary", 2, groups);
  ...
  LoopOverBoundary(cctkGH, bssnmatter_project_constraints_psu_boundary_Body);
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Leaving bssnmatter_project_constraints_psu_boundary_Body");
  }
}
