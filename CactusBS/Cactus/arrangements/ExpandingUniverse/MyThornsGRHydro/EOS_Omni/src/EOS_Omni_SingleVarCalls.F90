  ...
  select case (eoskey)
     ...
     case (6)
      ...
      case (7)
         ...
         if(keytemp.eq.1) then
            do i=1,npoints
               eps(i) = press_gf * rp_k_cgs * &
                    (rho(i)*inv_rho_gf)**(rp_gamma) / &
                    (rp_gamma - 1.0d0) / rho(i)
            enddo
         endif
         do i=1,npoints
            press(i) = (rp_gamma - 1.0d0) * rho(i) * (eps(i) + 1.0d0)
  ...
  select case (eoskey)
     ...
     case (5)
      ...
      case (7)
         ...
         do i=1,npoints
            xpress = (gl_gamma-1.0d0)*rho(i)*(eps(i)+1)
            xdpdrhoe = (gl_gamma-1.0d0)*(eps(i)+1)
            xdpderho = (gl_gamma-1.0d0)*rho(i)
            cs2(i) = (xdpdrhoe + xpress * xdpderho / (rho(i)**2)) / &
                 (1.0d0 + eps(i) + xpress/rho(i))
         enddo
  ...
  select case (eoskey)
     ...
     case (4)
      ...
      case (7)
         ! radiation pressure EOS
         do i=1,npoints
            xeps(i) = press(i) / (rp_gamma - 1.0d0) / rho(i) - 1.0d0
         enddo
     case DEFAULT
        write(warnstring,*) "eoskey ",eoskey," not implemented!"
        call CCTK_ERROR(warnstring)
...
!                 !$OMP END CRITICAL
      ...
      case (7)
         ! radiation pressure EOS
         do i=1,npoints
            rho(i) = press(i) / ((rp_gamma - 1.0d0)*(eps(i) + 1))
         enddo
