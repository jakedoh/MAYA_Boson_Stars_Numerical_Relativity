...
//
   ...
   "   setting mask grid function to \"buffer\"/\"inside\" for horizon %d",
	...
	// horizon bounding box, expanded to allow buffer, and rounded "out" to the next grid point
	const fp Cactus_dx = mgi.base_grid_mean_coord_delta;
	const double bufferedmin_x = BH_diagnostics.min_x - ( BH_diagnostics.centroid_x - BH_diagnostics.min_x )*mask_info.buffer_radius_multiplier - mask_info.buffer_thickness*Cactus_dx;
	const double bufferedmin_y = BH_diagnostics.min_y - ( BH_diagnostics.centroid_y - BH_diagnostics.min_y )*mask_info.buffer_radius_multiplier - mask_info.buffer_thickness*Cactus_dx;
	const double bufferedmin_z = BH_diagnostics.min_z - ( BH_diagnostics.centroid_z - BH_diagnostics.min_z )*mask_info.buffer_radius_multiplier - mask_info.buffer_thickness*Cactus_dx;
	const double bufferedmax_x = BH_diagnostics.max_x - ( BH_diagnostics.centroid_x - BH_diagnostics.max_x )*mask_info.buffer_radius_multiplier + mask_info.buffer_thickness*Cactus_dx;
	const double bufferedmax_y = BH_diagnostics.max_y - ( BH_diagnostics.centroid_y - BH_diagnostics.max_y )*mask_info.buffer_radius_multiplier + mask_info.buffer_thickness*Cactus_dx;
	const double bufferedmax_z = BH_diagnostics.max_z - ( BH_diagnostics.centroid_z - BH_diagnostics.max_z )*mask_info.buffer_radius_multiplier + mask_info.buffer_thickness*Cactus_dx;
	const int AH_min_i = mgi.ijk_floor_of_global_xyz(X_AXIS, bufferedmin_x);
	const int AH_max_i = mgi.ijk_ceil_of_global_xyz (X_AXIS, bufferedmax_x);
	const int AH_min_j = mgi.ijk_floor_of_global_xyz(Y_AXIS, bufferedmin_y);
	const int AH_max_j = mgi.ijk_ceil_of_global_xyz (Y_AXIS, bufferedmax_y);
	const int AH_min_k = mgi.ijk_floor_of_global_xyz(Z_AXIS, bufferedmin_z);
	const int AH_max_k = mgi.ijk_ceil_of_global_xyz (Z_AXIS, bufferedmax_z);
	if (verbose_info.print_algorithm_debug)
	   then {
		CCTK_VInfo(CCTK_THORNSTRING,
			   "      horizon bounding box is x=[%g,%g], buffered to x=[%g,%g]",
			   double(BH_diagnostics.min_x),
			   double(BH_diagnostics.max_x),
			   bufferedmin_x, bufferedmax_x);
		CCTK_VInfo(CCTK_THORNSTRING,
			   "                              y=[%g,%g], buffered to y=[%g,%g]",
			   double(BH_diagnostics.min_y),
			   double(BH_diagnostics.max_y),
			   bufferedmin_y, bufferedmax_y);
		CCTK_VInfo(CCTK_THORNSTRING,
			   "                              z=[%g,%g], buffered to z=[%g,%g]",
			   double(BH_diagnostics.min_z),
			   double(BH_diagnostics.max_z),
			   bufferedmin_z, bufferedmax_z);
		}
...
{
const fp Cactus_dx = mgi.base_grid_mean_coord_delta;
return r_inner*mask_info.buffer_radius_multiplier
       + mask_info.buffer_thickness*Cactus_dx;
}
	  }
