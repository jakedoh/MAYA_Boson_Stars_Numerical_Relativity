...
bool is_find_individual_set = false;
for (int n=1; n<N_horizons; ++n) {
	if ( find_every_individual[n] > 0 )
		is_find_individual_set = true;
}
if ( move_origins && is_find_individual_set ) {
...
if (mask_info.set_mask_for_any_horizon)
   then {
	...
	mask_info.buffer_radius_multiplier = mask_buffer_radius_multiplier;
