/*@@
  @file       NaNCheck.c
   ...
   @enddesc
   @version   $Id: NaNCheck.c,v 1.44 2008/11/24 10:45:49 tradke Exp $
 @@*/
...
static const char *rcsid = "$Header: /cactusdevcvs/CactusUtils/NaNChecker/src/NaNCheck.c,v 1.44 2008/11/24 10:45:49 tradke Exp $";
...
void NaNChecker_levelinfo(const cGH *GH, int *reflevel, int *maxreflevel);
...
int NaNChecker_CheckVarsForNaN (const cGH *cctkGH,
                                int _report_max,
                                const char *_vars,
                                const char *_check_for,
                                const char *_action_if_found)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  if (_action_if_found && (! CCTK_Equals (_action_if_found, "just warn") &&
                          ! CCTK_Equals (_action_if_found, "terminate") &&
                          ! CCTK_Equals (_action_if_found, "abort")))
  {
    CCTK_VWarn (1, __LINE__, __FILE__, CCTK_THORNSTRING,
  ...
  info.bitmask = 0;
  info.verbose = CCTK_Equals (verbose, "all");
  info.report_max = _report_max;
  info.action_if_found = _action_if_found;
  info.NaNmask = NULL;
  info.cctk_iteration = cctk_iteration;
  info.ignore_restricted_points = ignore_restricted_points;

#ifdef HAVE_FINITE
  if (CCTK_Equals (_check_for, "NaN"))
  {
    info.check_for = CHECK_FOR_NAN;
  }
  else if (CCTK_Equals (_check_for, "Inf"))
  {
    info.check_for = CHECK_FOR_INF;
  }
  else
  {
    if (! CCTK_Equals (_check_for, "both"))
    {
      CCTK_VWarn (1, __LINE__, __FILE__, CCTK_THORNSTRING,
                  "Invalid value '%s' passed for 'check_for' parameter. "
                  "Defaulting to 'both' instead.", _check_for);
    }
    info.check_for = CHECK_FOR_BOTH;
  }
#endif

  if (CCTK_TraverseString (_vars, CheckForNaN, &info, CCTK_GROUP_OR_VAR) < 0)
  {
    CCTK_VWarn (1, __LINE__, __FILE__, CCTK_THORNSTRING,
                "Couldn't traverse 'vars' string '%s'", _vars);
    return (-4);
  }
...
{                                                                             \
  ...
  {                                                                           \
    if ((!CarpetWeights || CarpetWeights[_i] == 1.0) && (                     \
...
static void CheckForNaN (int vindex, const char *optstring, void *_info)
{
  ...
  NaNChecker_levelinfo(info->GH, &reflevel, &maxreflevel);
  ...
  if (ignore_restricted_points)
    CarpetWeights = (CCTK_REAL *)(CCTK_VarDataPtr(info->GH, timelevel, "CarpetReduce::weight"));
  else 
    CarpetWeights = NULL;
