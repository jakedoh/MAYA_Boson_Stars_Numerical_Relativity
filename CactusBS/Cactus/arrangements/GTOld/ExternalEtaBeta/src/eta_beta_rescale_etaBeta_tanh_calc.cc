...
static void eta_beta_rescale_etaBeta_tanh_calc_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
  ...
  #pragma omp parallel
  CCTK_LOOP3(eta_beta_rescale_etaBeta_tanh_calc,
    ...
    cctk_ash[0],cctk_ash[1],cctk_ash[2])
  {
    ...
    exetaBetaL = exetaBetaL*(0.5 - 0.5*tanh((rL - 
      Rcutoff)*pow(sigma,-1)));
    /* Copy local copies back to grid functions */
    exetaBeta[index] = exetaBetaL;
  }
  CCTK_ENDLOOP3(eta_beta_rescale_etaBeta_tanh_calc);
}
extern "C" void eta_beta_rescale_etaBeta_tanh_calc(CCTK_ARGUMENTS)
{
  #ifdef DECLARE_CCTK_ARGUMENTS_eta_beta_rescale_etaBeta_tanh_calc
  DECLARE_CCTK_ARGUMENTS_CHECKED(eta_beta_rescale_etaBeta_tanh_calc);
  #else
  DECLARE_CCTK_ARGUMENTS;
  #endif
  DECLARE_CCTK_PARAMETERS;
  ...
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Entering eta_beta_rescale_etaBeta_tanh_calc_Body");
  }
  if (cctk_iteration % eta_beta_rescale_etaBeta_tanh_calc_calc_every != eta_beta_rescale_etaBeta_tanh_calc_calc_offset)
  {
    return;
  }
  ...
  const char* const groups[] = {
    ...
    "grid::coordinates"};
  AssertGroupStorage(cctkGH, "eta_beta_rescale_etaBeta_tanh_calc", 2, groups);
  
  
  LoopOverEverything(cctkGH, eta_beta_rescale_etaBeta_tanh_calc_Body);
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Leaving eta_beta_rescale_etaBeta_tanh_calc_Body");
  }
}
