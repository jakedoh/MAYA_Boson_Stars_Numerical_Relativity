...
{
  return rho*(1+EOS_SpecificIntEnergy(eos_handle, rho, matterlibs_poison))+EOS_Pressure(eos_handle, rho, matterlibs_poison);
}
    ...
    double dJz, double psi, double *drho, double *dlorentz)
{
  ...
  else
  {
    ...
    *dlorentz = sqrt( 0.5 * ( 1 + 
    ...
    // the error tolerance
    if( *dlorentz < 1. && 
	 GSL_SUCCESS == matterlibs_gsl_root_test_delta(1., *dlorentz, 
	     abs_err, rel_err))
    {
      *dlorentz = 1.;
    }
...
static double e_to_rho_RHS_f(double rho, void *params)
{
  ...
  const double P = EOS_Pressure(p->eos_handle, rho, matterlibs_poison);
...
static double e_to_rho_RHS_df(double rho, void *params)
{
  ...
  const double P = EOS_Pressure(p->eos_handle, rho, matterlibs_poison);
  const double rho_h = rho_times_h(p->eos_handle, rho);
  const double DPbyDRho = EOS_DPressByDRho(p->eos_handle, rho, matterlibs_poison);
