...
int
matterlibs_gsl_root_test_delta (double x1, double x0, double epsabs, double epsrel)
{
...
int
matterlibs_gsl_root_test_interval (double x_lower, double x_upper, double epsabs, double epsrel)
{
  ...
  do
   {
     ...
     if((status = steffenson_iterate (&state, fdf, &x)) != GSL_SUCCESS)
       break;
     status = matterlibs_gsl_root_test_delta (x, xold, epsabs, epsrel);
