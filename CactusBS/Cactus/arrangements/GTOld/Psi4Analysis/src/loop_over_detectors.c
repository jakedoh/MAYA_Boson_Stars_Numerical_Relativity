...
int Psi4Analysis_setup_out_every_det(CCTK_ARGUMENTS)
{
  ...
  for (int i=0;i<number_of_detectors;i++) {
    if (out_every_det[i]==-1) {
      ...
      ierr=CCTK_ParameterSet(paramstr,"Psi4Analysis",default_out_every);
      if (ierr<0) {
...
void Psi4Analysis_loop_over_detectors (CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  CCTK_INT ierr=Psi4Analysis_setup_out_every_det(CCTK_PASS_CTOC);
  if (ierr<0) {
  ...
  for (int i=0;i<number_of_detectors;i++) {
    ...
    *nphi=maxnphi;
    ...
    Psi4Analysis_ProjectSphere(CCTK_PASS_CTOC);

    if (verbose>2) {
      CCTK_INFO("calling Integrate_Psi4");
    }

    Psi4Analysis_Integrate_Psi4(CCTK_PASS_CTOC);

    if (verbose>2) {
      CCTK_INFO("calling Compute_EPRad");
    }

    Psi4Analysis_Compute_EPRad(CCTK_PASS_CTOC);
