...
static CCTK_INT perturb_star( const CCTK_INT pert_idx, const CCTK_REAL r_rs, const CCTK_REAL mu, CCTK_REAL *rho, CCTK_REAL *eps, CCTK_REAL *press );
				...
				epsL = ( rns_energyL - rhoL )/rhoL;
                       ...
                       if ( add_pert ) {
                          ...
                          if ( isnan(rL/r_surface) || r_surface < 0.001 ) {
                             #pragma omp critical
	                     CCTK_VWarn(0,__LINE__,__FILE__,CCTK_THORNSTRING,
                                  "At r=%g, mu=%g, r_surface=%g. ns_radius_iso=%g.", rL, muL, r_surface, *ns_radius_iso);
                          }
                          CCTK_REAL clean_rho=rhoL;
                          perturb_star(pert_idx, rL/r_surface, muL, &rhoL, &epsL, &pressL );
                          #pragma omp critical
                          {
                          fprintf( out_pert, "%g %g %g %.19g %.19g %.19g %.19g %.19g %.19g\n", xL, yL, zL, muL, rL, r_surface, rL/r_surface, 
                                    ...
                                    CCTK_REAL *ns_radius, CCTK_REAL *ns_radius_iso, CCTK_REAL *ns_mass, CCTK_REAL *ns_oblateness, CCTK_REAL *ns_omega )
{
  ...
  while ( fgets( line, sizeof(line), file ) != NULL ) {
	...
	int iL,jL, idxL;
	CCTK_REAL sL, muL, rhoL, gamaL, alpL, omegaL, energyL, pressL, enthL, velsqL;

	/* Data line */
	sscanf( line, format_str, &iL, &jL, &sL, &muL, &rhoL, &gamaL, &alpL, &omegaL, &energyL, &pressL, &enthL, &velsqL);
   ...
   CCTK_REAL ns_radius, CCTK_REAL ns_radius_iso, CCTK_REAL ns_mass, CCTK_REAL ns_oblateness, CCTK_REAL ns_omega )
{
  ...
  for ( int i=0; i<npts_intrad; i++ ) {
	...
	fprintf(file,format_str_real,i,j,sL,muL,
...
static CCTK_INT perturb_star( const CCTK_INT pert_idx, const CCTK_REAL r_rs, const CCTK_REAL mu, CCTK_REAL *rho, CCTK_REAL *eps, CCTK_REAL *press )
{
  ...
  if ( pert_idx == 2 ) {
     drho = pert_amp * central_rho * sin( PI*r_rs ) * ( 3.*SQR(mu)-1. );
     //dpress = eos_Gamma * (*press)* ( drho/ (*rho) ); 
  }
