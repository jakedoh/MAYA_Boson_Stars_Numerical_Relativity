  ...
  if ( pert_idx == 1 ) {
   //  drho = pert_amp * central_rho * cos( 0.5*PI*r_rs );
     //dpress = eos_Gamma * (*press)* ( drho/ (*rho) );
      drho = pert_amp * central_rho*(SQR(xx)-SQR(yy))*SQR(r_inv);       

  }
  if ( pert_idx == 2 ) {
  //     drho = pert_amp * central_rho * sin( PI*r_rs ) * ( 3.*SQR(mu)-1. );
  //     drho = pert_amp * central_rho * sin( PI*r_rs ) * (1.-SQR(mu))*cos(2.*acos(mu/sqrt(1. - SQR(mu))));
  //     drho = pert_amp * central_rho * sin( PI*r_rs ) * (1.-SQR(mu))*(SQR(r_inv)*(SQR(xx)-SQR(yy)));
     //dpress = eos_Gamma * (*press)* ( drho/ (*rho) ); 
