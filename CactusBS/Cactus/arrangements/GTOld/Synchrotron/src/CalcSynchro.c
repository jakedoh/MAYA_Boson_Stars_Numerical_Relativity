...
//****** External Routines ******//
//*******************************//
void Synchro_Scalars(CCTK_ARGUMENTS);
void Synchro_Init(CCTK_ARGUMENTS);
void SynchroCalc(CCTK_ARGUMENTS);
void ZeroSynchro(CCTK_ARGUMENTS);
void SumSynchro(CCTK_ARGUMENTS);
void Synchro_Output(CCTK_ARGUMENTS);

//*******************************//
//****** Internal Routines ******//
//*******************************//
static int sumLocalSynchro(CCTK_ARGUMENTS, CCTK_REAL * locVolSyn, CCTK_REAL * locVolDSyn);
static void synchro_get_centre(CCTK_ARGUMENTS, int c, CCTK_REAL *xc, CCTK_REAL *yc, CCTK_REAL *zc);
...
void Synchro_Scalars(CCTK_ARGUMENTS)
{
  ...
  *SymmetryFactor = symfactL;
  ...
  CCTK_REAL dVFact;
  if (do_cgs_units) {
      /* EmissFact includes density conversion */
      //*EmissFact= 5.9078e-6 * Z_gas / SQR(Msys_SolarMasses*gas_massunit); (OLD & WRONG)
      *EmissFact= 2.015e16 * Z_gas / SQR( SQR(Msys_SolarMasses)*gas_massunit); // Now compares to Farris' paper
      dVFact=3.24e15 * Msys_SolarMasses*SQR(Msys_SolarMasses);
  } else {
      *EmissFact=unitsfudge;
      dVFact=1;
  }
  CCTK_VInfo(CCTK_THORNSTRING,"Synchrotron emissivities are multiplied by %g, dV factor is %g",*EmissFact,dVFact);
...
void SynchroCalc(CCTK_ARGUMENTS)
{
  ...
  CCTK_REAL rhoL, epsL, pressL, Mbeta, SynchroL;
  const double ptoe_mass_ratio = 1836.11;
  CCTK_REAL Ttheta, Tfac;

  Tfac = ptoe_mass_ratio * temperature_fac;
  ...
  for(int k=0 ; k<cctk_lsh[2] ; k++)
  {
	...
	if (Bbeta_from_field) {
		...
		/* Mbeta = 8*pi*pressL / B^2 */
                ...
                CCTK_REAL Bx,By,Bz,B2;
                CCTK_INT indx, indy, indz;
                indx = CCTK_VECTGFINDEX3D (cctkGH, i, j, k, 0);
                indy = CCTK_VECTGFINDEX3D (cctkGH, i, j, k, 1);
                indz = CCTK_VECTGFINDEX3D (cctkGH, i, j, k, 2);
                Bx = Bvec[indx];
                By = Bvec[indy];
                Bz = Bvec[indz];
                B2 = gxxL*SQR(Bx)+gyyL*SQR(By)+gzzL*SQR(Bz)
                   + 2.*gxyL*Bx*By + 2.*gxzL*Bx*Bz + 2.*gyzL*By*Bz;

		Mbeta = 8.*PI*pressL / B2;

	} else {
		Mbeta = Bbeta;
	} 

	/* Ttheta = kB T_e(Kelvin) / m_e c^2 */
	if (assume_ideal) {
		Ttheta = Tfac * ( epsL*(eos_gamma - 1.) ); 
	} else {
		Ttheta = Tfac * ( pressL / rhoL ); 
	}

	SynchroL = *EmissFact * SQR(rhoL) * SQR(Ttheta)*Ttheta / Mbeta;
	Synchro[idx] = SynchroL;
...
void SumSynchro(CCTK_ARGUMENTS)
{
  ...
  int ierr, vol, nobs;
  int do_calc_syn, my_calc_syn_every;
  ...
  for (vol=0 ; vol<intvolumes; vol++)
  {
    ...
    if ( (my_calc_syn_every > 0) && (cctk_iteration % my_calc_syn_every == 0) )
    {
        do_calc_syn = 1;
        break;
    }
  ...
  if (do_calc_syn) 
  {
     ...
     double locVolSyn[intvolumes], locVolDSyn[intvolumes*num_obs_angles];
     for ( vol=0; vol<intvolumes; vol++)
     {
       locVolSyn[vol]  = -1e44;
       for ( nobs=0; nobs<num_obs_angles; nobs++ )
       {
           int ind2d=vol+intvolumes*nobs;
           locVolDSyn[ind2d] = -1e44;
       }
     }

     if (verbose)
	CCTK_VInfo(CCTK_THORNSTRING,"Calculating total Synchrotron at iteration %d.  Ready for local loops.",cctk_iteration);

     /* WARNING: This would break if one patch/level doesn't exist on all cores */
     const int ierri = sumLocalSynchro(CCTK_PASS_CTOC, locVolSyn, locVolDSyn);
     assert(!ierri);
     if (ierri) CCTK_WARN (CCTK_WARN_ABORT, "Could not sum Synchrotron");
     ...
     enum out_buffer_indices { sumS=0, NUM_BSTYPES };
     CCTK_REAL in_buffer_S[intvolumes][num_obs_angles+1];
     CCTK_REAL out_buffer[NUM_BSTYPES][intvolumes][num_obs_angles+1];
     for ( vol=0; vol<intvolumes; vol++ )
     {
         in_buffer_S[vol][0]=locVolSyn[vol];
         out_buffer[sumS][vol][0]=-1e45;
         for ( nobs=0; nobs<num_obs_angles; nobs++ )
         {
             int ind2d=vol+intvolumes*nobs;
             in_buffer_S[vol][nobs+1]=locVolDSyn[ind2d];
             out_buffer[sumS][vol][nobs+1]=-1e45;
         }
     }
     ierr = CCTK_ReduceArray (cctkGH,
                              0,                    /* destination processor */
                              sum_op,               /* operation_handle */
                              intvolumes*(num_obs_angles+1), /* num output per input array */
                              ...
                              in_buffer_S) ;           /* in_arrays, (void *[num_in_fiels], varargs) */
     assert(!ierr);
     if (ierr) CCTK_WARN (CCTK_WARN_ABORT, "Error in ReduceArrays");
     for (vol=0; vol<intvolumes; vol++)
     {
     	TotSynchro[vol] += *SymmetryFactor * out_buffer[sumS][vol][0];
        for ( nobs=0; nobs<num_obs_angles; nobs++ )
        {
            int ind2d = vol+intvolumes*nobs;
     	    TotDSynchro[ind2d] += out_buffer[sumS][vol][nobs+1]; /* All symmetries included in integrand here */
        }
        if (verbose)
...
static int sumLocalSynchro(CCTK_ARGUMENTS, CCTK_REAL * locVolSyn, CCTK_REAL * locVolDSyn) 
{
  ...
  CCTK_REAL SynchroL,weightL,rhoL;
  ...
  for (vol=0; vol<intvolumes; vol++)
  {
    ...
    synchro_get_centre(CCTK_PASS_CTOC, vol, &x0L[vol], &y0L[vol], &z0L[vol]);
  }
  ...
  for(int k = imin[2] ; k < imax[2] ; k++)
  {
	...
	w2L = INITVALUE;
        ...
        intSynchroL = INITVALUE;

	/* Information from the grid */
        int idx = CCTK_GFINDEX3D(cctkGH, i,j,k);

	rhoL = rho[idx];
	SynchroL = Synchro[idx];
	...
	/* Doppler factor, observer at -\hat{y} and rotating above plane within yz-plane */
        CCTK_REAL tanth, obsmag;
        for ( nobs=0; nobs<num_obs_angles; nobs++ ) {
            Df4[nobs] = 0;
	...
	for (vol=0; vol<intvolumes; vol++)
	{
	   ...
	   if ( r2L < SQR(int_wi_rad[vol]) )
	   {
              ...
              locVolSyn[vol]   += weightL*intSynchroL;
              for ( nobs=0; nobs<num_obs_angles; nobs++ )
              {
                  int ind2d=vol+intvolumes*nobs;
                  locVolDSyn[ind2d]  += weightL*Df4[nobs]*intSynchroL;
              }
	   }
	}
  ...
  if (verbose){
     ...
     for (vol=0; vol<intvolumes; vol++)
     {
       CCTK_VInfo(CCTK_THORNSTRING,"\t Within r=%g: Synchro = %.17g", 
	          int_wi_rad[vol], locVolSyn[vol]);
     }
  }
...
void Synchro_Output(CCTK_ARGUMENTS)
{
  ...
  char addformat[50];
  int vol, nobs, do_calc_syn, my_calc_syn_every;
  FILE *file;
  ...
  if ( (myproc == 0) && do_calc_syn ) 
  {
    ...
    if (!file) {
      CCTK_VWarn (1, __LINE__, __FILE__, CCTK_THORNSTRING,
                  "Synchro_Output: Could not open integrated scalar output file '%s'",
                  filename);
    }

    // write header on startup
    if (file_created<=0) {
      fprintf(file,"# Integrated Synchrotron luminosities.\n");
      fprintf(file,"# gnuplot column index:\n");
      fprintf(file,"# 1:it 2:t \n");
      int LperV=num_obs_angles+1;
      for (vol=0;vol<intvolumes;vol++)
      {
	fprintf(file,"#\t( r=%g ) %d:L_Syn",int_wi_rad[vol],LperV*vol+3);
        for ( nobs=0; nobs<num_obs_angles; nobs++ )
        {
            fprintf(file," %d:L_Syn(th=%3.4g)",LperV*vol+nobs+4,obs_angles[nobs]);
        }
        fprintf(file,"\n");
      }
    }
    ...
    for (vol=0;vol<intvolumes;vol++)
    {
	fprintf(file,addformat,TotSynchro[vol]);
        for (nobs=0; nobs<num_obs_angles; nobs++ )
        {
            int ind2d=vol+intvolumes*nobs;
            fprintf(file,addformat,TotDSynchro[ind2d]);
        }
    }
    fprintf(file,"\n");
