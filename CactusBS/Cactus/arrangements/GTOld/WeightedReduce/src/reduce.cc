...
namespace WeightedReduce {
  ...
  struct my_numeric_limits {
    ...
    static T min ()
    {
      ...
      return WeightedReduce::mymin (numeric_limits<T>::min(), -numeric_limits<T>::max());
    }
  ...
  {
    return complex<CCTK_REAL4> (WeightedReduce::mymin(x.real(), y.real()),
                                WeightedReduce::mymin(x.imag(), y.imag()));
  }
  
  template<> inline complex<CCTK_REAL4>
  mymax (const complex<CCTK_REAL4> x, const complex<CCTK_REAL4> y)
  {
    return complex<CCTK_REAL4> (WeightedReduce::mymax(x.real(), y.real()),
                                WeightedReduce::mymax(x.imag(), y.imag()));
  }
  ...
  {
    return complex<CCTK_REAL8> (WeightedReduce::mymin(x.real(), y.real()),
                                WeightedReduce::mymin(x.imag(), y.imag()));
  }
  ...
  {
    return complex<CCTK_REAL8> (WeightedReduce::mymax(x.real(), y.real()),
                                WeightedReduce::mymax(x.imag(), y.imag()));
  }
  ...
  {
    return complex<CCTK_REAL16> (WeightedReduce::mymin(x.real(), y.real()),
                                 WeightedReduce::mymin(x.imag(), y.imag()));
  }
  ...
  {
    return complex<CCTK_REAL16> (WeightedReduce::mymax(x.real(), y.real()),
                                 WeightedReduce::mymax(x.imag(), y.imag()));
  }
  ...
  struct minimum : reduction {
    ...
    struct op {
      ...
      static inline void reduce (T& accum, T& cnt, const T& val, const CCTK_REAL weight) { if (weight!=0) accum = WeightedReduce::mymin(accum, val); cnt += T(weight); }
      static inline void combine (T& accum, T& cnt, const T& accum2, const T& cnt2) { accum = WeightedReduce::mymin(accum,accum2); cnt += cnt2; }
      static inline void finalise (T& accum, const T& cnt) { }
    };
  ...
  struct maximum : reduction {
    ...
    struct op {
      ...
      static inline void reduce (T& accum, T& cnt, const T& val, const CCTK_REAL weight) { if (weight!=0) accum = WeightedReduce::mymax(accum, val); cnt += T(weight); }
      static inline void combine (T& accum, T& cnt, const T& accum2, const T& cnt2) { accum = WeightedReduce::mymax(accum,accum2); cnt += cnt2; }
      static inline void finalise (T& accum, const T& cnt) { }
    };
  ...
  struct sum_squared : reduction {
    ...
    struct op {
      static inline void initialise (T& accum, T& cnt) { accum = T(0); cnt = T(0); }
      static inline void reduce (T& accum, T& cnt, const T& val, const CCTK_REAL weight) { if (weight!=0) accum += weight*WeightedReduce::mysqr(val); cnt += T(weight); }
      static inline void combine (T& accum, T& cnt, const T& accum2, const T& cnt2) { accum += accum2; cnt += cnt2; }
      static inline void finalise (T& accum, const T& cnt) { }
    };
    MPI_Op mpi_op () const { return dist::mpi_sum; }
  ...
  struct sum_abs_squared : reduction {
    ...
    struct op {
      static inline void initialise (T& accum, T& cnt) { accum = T(0); cnt = T(0); }
      static inline void reduce (T& accum, T& cnt, const T& val, const CCTK_REAL weight) { if (weight!=0) accum += weight*WeightedReduce::mysqrabs(val); cnt += T(weight); }
      static inline void combine (T& accum, T& cnt, const T& accum2, const T& cnt2) { accum += accum2; cnt += cnt2; }
      static inline void finalise (T& accum, const T& cnt) { }
    };
    MPI_Op mpi_op () const { return dist::mpi_sum; }
  ...
  struct norm2 : reduction {
    ...
    struct op {
      ...
      static inline void reduce (T& accum, T& cnt, const T& val, const CCTK_REAL weight) { if (weight!=0) accum += weight*WeightedReduce::mysqrabs(val); cnt += T(weight); }
      static inline void combine (T& accum, T& cnt, const T& accum2, const T& cnt2) { accum += accum2; cnt += cnt2; }
      static inline void finalise (T& accum, const T& cnt) { accum = WeightedReduce::mysqrt(accum / cnt); }
    };
    MPI_Op mpi_op () const { return dist::mpi_sum; }
  };
  ...
  struct norm_inf : reduction {
    ...
    struct op {
      static inline void initialise (T& accum, T& cnt) { accum = T(0); cnt = T(0); }
      static inline void reduce (T& accum, T& cnt, const T& val, const CCTK_REAL weight) { if (weight!=0) accum = WeightedReduce::mymax(accum, T(abs(val))); cnt += T(weight); }
      static inline void combine (T& accum, T& cnt, const T& accum2, const T& cnt2) { accum = WeightedReduce::mymax(accum,accum2); cnt += cnt2; }
      static inline void finalise (T& accum, const T& cnt) { }
    };
    MPI_Op mpi_op () const { return dist::mpi_max; }
        ...
        ENTER_LEVEL_MODE(cgh, rl) {
          ...
          if (need_time_interp) {
            ...
            if (num_tl == 1) {
              ...
              if (active_tl < num_tl) {
                int const max_tl = CCTK_DeclaredTimeLevelsVI(vi);
                ...
                BEGIN_LOCAL_COMPONENT_LOOP(cgh, grouptype) {
                  ...
                  if (want_global_mode or want_level_mode) {
                    ...
                    if (iweight == -1) {
                      ...
                      int type;
                      weightvar = (char **)
                          CCTK_ParameterGet("WeightVar", CCTK_THORNSTRING, &type);
                      assert(type == PARAMETER_STRING);
                      assert(weightvar && *weightvar && **weightvar);
                      iweight = CCTK_VarIndex (*weightvar);
                      if (iweight < 0) {
                        CCTK_VWarn (0, __LINE__, __FILE__, CCTK_THORNSTRING,
                                    "Cannot find desired mask variable \"%s\".",
                                    *weightvar);
                      }
...
#undef REDUCTION
  ...
  int WeightedReduceStartup ()
  {
    CCTK_RegisterReductionOperator (count_GVs,           "wcount");
    CCTK_RegisterReductionOperator (minimum_GVs,         "wminimum");
    CCTK_RegisterReductionOperator (maximum_GVs,         "wmaximum");
    CCTK_RegisterReductionOperator (product_GVs,         "wproduct");
    CCTK_RegisterReductionOperator (sum_GVs,             "wsum");
    CCTK_RegisterReductionOperator (sum_abs_GVs,         "wsum_abs");
    CCTK_RegisterReductionOperator (sum_squared_GVs,     "wsum_squared");
    CCTK_RegisterReductionOperator (sum_abs_squared_GVs, "wsum_abs_squared");
    CCTK_RegisterReductionOperator (average_GVs,         "waverage");
    CCTK_RegisterReductionOperator (norm1_GVs,           "wnorm1");
    CCTK_RegisterReductionOperator (norm2_GVs,           "wnorm2");
    CCTK_RegisterReductionOperator (norm_inf_GVs,        "wnorm_inf");
    
    CCTK_RegisterReductionOperator (icount_GVs,           "iwcount");
    CCTK_RegisterReductionOperator (iminimum_GVs,         "iwminimum");
    CCTK_RegisterReductionOperator (imaximum_GVs,         "iwmaximum");
    CCTK_RegisterReductionOperator (iproduct_GVs,         "iwproduct");
    CCTK_RegisterReductionOperator (isum_GVs,             "iwsum");
    CCTK_RegisterReductionOperator (isum_abs_GVs,         "iwsum_abs");
    CCTK_RegisterReductionOperator (isum_squared_GVs,     "iwsum_squared");
    CCTK_RegisterReductionOperator (isum_abs_squared_GVs, "iwsum_abs_squared");
    CCTK_RegisterReductionOperator (iaverage_GVs,         "iwaverage");
    CCTK_RegisterReductionOperator (inorm1_GVs,           "iwnorm1");
    CCTK_RegisterReductionOperator (inorm2_GVs,           "iwnorm2");
    CCTK_RegisterReductionOperator (inorm_inf_GVs,        "iwnorm_inf");
    
    CCTK_RegisterReductionArrayOperator (count_arrays,           "wcount");
    CCTK_RegisterReductionArrayOperator (minimum_arrays,         "wminimum");
    CCTK_RegisterReductionArrayOperator (maximum_arrays,         "wmaximum");
    CCTK_RegisterReductionArrayOperator (product_arrays,         "wproduct");
    CCTK_RegisterReductionArrayOperator (sum_arrays,             "wsum");
    CCTK_RegisterReductionArrayOperator (sum_abs_arrays,         "wsum_abs");
    CCTK_RegisterReductionArrayOperator (sum_squared_arrays,     "wsum_squared");
    CCTK_RegisterReductionArrayOperator (sum_abs_squared_arrays, "wsum_abs_squared");
    CCTK_RegisterReductionArrayOperator (average_arrays,         "waverage");
    CCTK_RegisterReductionArrayOperator (norm1_arrays,           "wnorm1");
    CCTK_RegisterReductionArrayOperator (norm2_arrays,           "wnorm2");
    CCTK_RegisterReductionArrayOperator (norm_inf_arrays,        "wnorm_inf");
    
    CCTK_RegisterReductionArrayOperator (icount_arrays,           "iwcount");
    CCTK_RegisterReductionArrayOperator (iminimum_arrays,         "iwminimum");
    CCTK_RegisterReductionArrayOperator (imaximum_arrays,         "iwmaximum");
    CCTK_RegisterReductionArrayOperator (iproduct_arrays,         "iwproduct");
    CCTK_RegisterReductionArrayOperator (isum_arrays,             "iwsum");
    CCTK_RegisterReductionArrayOperator (isum_abs_arrays,         "iwsum_abs");
    CCTK_RegisterReductionArrayOperator (isum_squared_arrays,     "iwsum_squared");
    CCTK_RegisterReductionArrayOperator (isum_abs_squared_arrays, "iwsum_abs_squared");
    CCTK_RegisterReductionArrayOperator (iaverage_arrays,         "iwaverage");
    CCTK_RegisterReductionArrayOperator (inorm1_arrays,           "iwnorm1");
    CCTK_RegisterReductionArrayOperator (inorm2_arrays,           "iwnorm2");
    CCTK_RegisterReductionArrayOperator (inorm_inf_arrays,        "iwnorm_inf");
