...
#define len(x) (sizeof(x)/size((x)[0]))
...
void TOV_C_Integrate_RHS(CCTK_ARGUMENTS)
{
  ...
  CCTK_INT star, star_i, i, TOV_Surface_Index, last_pt;
  ...
  CCTK_REAL inv_press[TOV_Num_Radial], inv_m[TOV_Num_Radial], inv_mbar[TOV_Num_Radial], 
	    inv_phi[TOV_Num_Radial], inv_r[TOV_Num_Radial], inv_rbar[TOV_Num_Radial],
	    inv_rprop[TOV_Num_Radial];
  ...
  for (star=0; star < TOV_Num_TOVs; star++)
  {
    ...
    const CCTK_REAL Rho_atm_init       = TOV_Rho_Atmosphere[star];
    const CCTK_REAL Mass_init          = TOV_Mass[star];
    const CCTK_REAL Mass_baryon_init   = TOV_BaryonMass[star];
    const CCTK_REAL Radius_init        = TOV_Radius[star];
    const CCTK_REAL Phi_init           = 0.5*log(1.0 - 2.0*Mass_init/Radius_init);
    //double psi_rstar_fac               = (1.0+Mass_init/(pow(2.0*Radius_init))**2.;
    const CCTK_REAL Riso_init         = 0.5*(Radius_init - Mass_init + sqrt(Radius_init*Radius_init-2*Mass_init*Radius_init)); //Baumgarte - NR(1.61) - do not use 1.62, there is typo
    const CCTK_REAL Rprop_init = 1.0/(1.0 - 2.0*Mass_init/Radius_init); 
    ...
    /* set start values */
    TOV_press_1d[star_i] = TOV_K * pow(Rho_atm_init, TOV_Gamma);  //change this for change in EOS
    TOV_m_1d	[star_i] = Mass_init;
    TOV_phi_1d  [star_i] = Phi_init;
    TOV_r_1d    [star_i] = Radius_init;
    TOV_rbar_1d [star_i] = Riso_init;
    TOV_mbary_1d[star_i] = Mass_baryon_init;
    TOV_rprop_1d[star_i] = Rprop_init;		//check this
  
    CCTK_VInfo(CCTK_THORNSTRING,"Initial Data: Pressure = %g, mass = %g, baryon mass = %g, radius = %g, potential (phi) = %g, isotropic coordinate = %g, K = %g, gamma = %g \n", TOV_press_1d[star_i], TOV_m_1d[star_i], TOV_mbary_1d[star_i], TOV_r_1d[star_i], TOV_phi_1d[star_i], TOV_rbar_1d[star_i], TOV_K, TOV_Gamma);

    last_pt = TOV_Num_Radial;
    /* build TOV_r_1d[] */
	...
	if (TOV_r_1d[i] <= 0)
	{
	    TOV_r_1d[i]=0;
	    last_pt = i;
	    //CCTK_VInfo(CCTK_THORNSTRING, "i = %d, r = %g", i, TOV_r_1d[i]);
	    break;
	}
    }
    ...
    //CCTK_VInfo(CCTK_THORNSTRING, "Iteration \t 1:r \t 2:riso \t 3:press \t 4:density \t 5:mass \t 6:Baryon Mass \t 7:Phi \n");
#define RKLOOP for (int rk=0; rk<NUMVARS; rk++)
    /* loop over all radii */
    for (i=star_i; (i < star_i+last_pt); i++)
    {
      /* set up RK arrays */
      ...
      //{
        TOV_Surface[star]   = TOV_r_1d[star_i];
        TOV_R_Surface[star] = TOV_rbar_1d[star_i];
        TOV_RProp_Surface[star] = TOV_rprop_1d[star_i];
        TOV_Surface_Index = star_i;
      //}
      if (TOV_r_1d[i+1] < 0.0)
      {    
	   CCTK_VInfo(CCTK_THORNSTRING,"Breaking the loop at i = %d as r = %g", i, TOV_r_1d[i]);	   
	   break;
	}

       /*if(i%200==0)
       {    //double rhoc_i = pow(TOV_press_1d[i]/TOV_K, 1.0/TOV_Gamma);
   	    CCTK_VInfo(CCTK_THORNSTRING, "%d 	%.5f 	 %.5f 	%.5f 	 %.5f 	%.5f 	%.5f \n" ,i, TOV_r_1d[i], TOV_rbar_1d[i], TOV_press_1d[i],  TOV_m_1d[i], TOV_mbary_1d[i], TOV_phi_1d[i] );
       }*/
    }
  ...
  CCTK_VInfo("", "Region \t TOV    radius  isotropic   mass  bary_mass mass(g) cent.rho rho(cgi)        K   K(cgi)    Gamma");
 
  for (i=0; i<TOV_Num_TOVs; i++)    
  {  
    star_i = i * TOV_Num_Radial;
    double rho_central = pow(TOV_press_1d[(i+1)*TOV_Num_Radial-1]/TOV_K, 1.0/TOV_Gamma);
    if (fabs(TOV_Gamma - 2.0) < LOCAL_TINY)
    {	CCTK_VInfo("","Outside: %d %8g  %8g %8g %8g %8.3g %8g %8.3g %8g %8.3g %8g",
                 (int)i+1,TOV_Surface[i], TOV_R_Surface[i],
                 TOV_m_1d[star_i],
                 TOV_mbary_1d[star_i],
                 TOV_m_1d[star_i]*CONSTANT_Msolar_cgi,
                 TOV_Rho_Atmosphere[i],
                 TOV_Rho_Atmosphere[i]/pow(CONSTANT_G_cgi,3.0)/
                 ...
                 TOV_Gamma);
    	CCTK_VInfo("","Inside: %d  %8g %8g %8g %8g %8.3g %8g %8.3g %8g %8.3g %8g",
                 (int)i+1, TOV_r_1d[(i+1)*TOV_Num_Radial-1], TOV_rbar_1d[(i+1)*TOV_Num_Radial-1],
                 TOV_m_1d[(i+1)*TOV_Num_Radial-1],
                 TOV_mbary_1d[(i+1)*TOV_Num_Radial-1],
                 TOV_m_1d[(i+1)*TOV_Num_Radial-1]*CONSTANT_Msolar_cgi,
                 rho_central,
                 rho_central/pow(CONSTANT_G_cgi,3.0)/
                                    pow(CONSTANT_Msolar_cgi,2.0)*
                                    pow(CONSTANT_c_cgi,6.0),
                 TOV_K,
                 TOV_K*pow(CONSTANT_G_cgi,3.0)*
    ...
    {
      CCTK_VInfo("","Outside:  %d %8g %8g %8g %8.3g %8g %8.3g %8g %8g",
                 (int)i+1, TOV_Surface[i], TOV_R_Surface[i],
                 TOV_m_1d[i],
                 TOV_mbary_1d[i],
                 TOV_m_1d[i]*CONSTANT_Msolar_cgi,
                 TOV_Rho_Atmosphere[i],
                 TOV_Rho_Atmosphere[i]/pow(CONSTANT_G_cgi,3.0)/
                                    pow(CONSTANT_Msolar_cgi,2.0)*
                                    pow(CONSTANT_c_cgi,6.0),
                 TOV_K, TOV_Gamma);
      CCTK_VInfo("","Inside:  %d %8g %8g %8g %8.3g %8g %8.3g %8g %8g",
                 (int)i+1, TOV_r_1d[(i+1)*TOV_Num_Radial-1],TOV_rbar_1d[(i+1)*TOV_Num_Radial-1],
                 TOV_m_1d[(i+1)*TOV_Num_Radial-1],
                 TOV_mbary_1d[(i+1)*TOV_Num_Radial-1],
                 TOV_m_1d[(i+1)*TOV_Num_Radial-1]*CONSTANT_Msolar_cgi,
    ...
    for(int j=star_i; j<star_i+TOV_Num_Radial;j++)
    {
        if(j<=last_pt)
        {
            inv_r[j]     = TOV_r_1d[last_pt-j];
            inv_rbar[j]  = TOV_rbar_1d[last_pt-j];
            inv_rprop[j] = TOV_rprop_1d[last_pt-j];
            inv_press[j] = TOV_press_1d[last_pt - j];
            inv_m[j]     = TOV_m_1d[last_pt - j];
            inv_mbar[j]  = TOV_mbary_1d[last_pt - j];
            inv_phi[j]   = TOV_phi_1d[last_pt - j];
          }
        else
          {  inv_r[j] = inv_r[j-1] + fabs(TOV_dr[i]);
            inv_press[j] = 0;
            inv_m[j]     = TOV_m_1d[0];
            inv_mbar[j]  = TOV_mbary_1d[0];
            inv_rbar[j]  = 0.5*(inv_r[j] - inv_m[j] + sqrt(inv_r[j]*inv_r[j]-2*inv_m[j]*inv_r[j]));
            inv_phi[j]   = 0.5*log(1.0 - 2.0*inv_m[j]/inv_r[j]);
          }
     }
	     
 // FILE *ofile = fopen("%s/tov_internaldata_%d.asc", (out_dir, i), "w");
    FILE *ofile = fopen("tov_internaldata_0.asc", "w");
    if (ofile==NULL)
    {  
          CCTK_VWarn(CCTK_WARN_ABORT, __LINE__, __FILE__, CCTK_THORNSTRING,"File cannot be opened");
    }
          
    fprintf(ofile, "#1:r \t 2:riso \t 3:press  \t 5:mass \t 6:Baryon Mass \t 7:Phi \n");
      for (int j=star_i; j < star_i+TOV_Num_Radial; j++)
      {
          //double rhoc_j = pow(TOV_press_1d[j]/TOV_K, 1.0/TOV_Gamma);
          
          fprintf(ofile, "%d \t %g \t %g \t %g \t %g \t %g \t %g \n" ,j, TOV_r_1d[j], TOV_rbar_1d[j], TOV_press_1d[j], TOV_m_1d[j], TOV_mbary_1d[j], TOV_phi_1d[j] );
      }
      	fclose(ofile); 
   ...
   //Check if arrays inverted correctly
      FILE *ofile2 = fopen("tov_invertedldata_1.asc", "w");
      if (ofile2==NULL)
      {  
            CCTK_VWarn(CCTK_WARN_ABORT, __LINE__, __FILE__, CCTK_THORNSTRING,"File cannot be opened");
      }
    	
      fprintf(ofile2, "#1:r \t 2:riso \t 3:press  \t 5:mass \t 6:Baryon Mass \t 7:Phi \n");
      for (int j=star_i; j < star_i+TOV_Num_Radial; j++)
      {
          //double rhoc_j = pow(TOV_press_1d[j]/TOV_K, 1.0/TOV_Gamma);
          	
            TOV_r_1d[j]     = inv_r    [j];
            TOV_rbar_1d[j]  = inv_rbar [j];
            TOV_rprop_1d[j] = inv_rprop[j];
            TOV_press_1d[j] = inv_press[j];
            TOV_m_1d[j]     = inv_m    [j];
            TOV_mbary_1d[j] = inv_mbar[ j];
            TOV_phi_1d[j]   = inv_phi[j];
          fprintf(ofile2, "%d \t %g \t %g \t %g \t %g \t %g \t %g \n" ,j, TOV_r_1d[j], TOV_rbar_1d[j], TOV_press_1d[j], TOV_m_1d[j], TOV_mbary_1d[j], TOV_phi_1d[j] );
      }
      	fclose(ofile2); 
