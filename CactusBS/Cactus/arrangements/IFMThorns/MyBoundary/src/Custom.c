 /*@@
   ...
   @endhistory
   @version   $Id: FlatBoundary.c 327 2014-04-29 00:57:01Z rhaas $
 @@*/
...
CCTK_FILEVERSION(CactusBase_Boundary_customBoundary_c);

static int ApplyBndcustom (const cGH *GH,
                         CCTK_INT stencil_dir,
                         const CCTK_INT *stencil_alldirs,
                         int dir,
                         int first_var,
                         int num_vars);
static int OldApplyBndcustom (const cGH *GH,
                            int stencil_dir,
...
/*@@
   ...
   @endreturndesc
@@*/
CCTK_INT Bndcustom(const cGH *GH, CCTK_INT num_vars, CCTK_INT *vars,
  ...
  for (i=0; i<num_vars; i+=j) {
    ...
    if (faces[i] != CCTK_ALL_FACES)
    {
      CCTK_VWarn(1, __LINE__, __FILE__, CCTK_THORNSTRING,
                 "Faces specification %d for custom boundary conditions on "
                 "%s is not implemented yet.  "
                 "Applying  bcs to all (external) faces.", (int)faces[i],
                 CCTK_VarName(vars[i]));
    }
    ...
    if ((retval = ApplyBndcustom(GH, 0, width_alldirs, dir, vars[i], j)) < 0)
    {
      CCTK_VWarn(1, __LINE__, __FILE__, CCTK_THORNSTRING,
                 "ApplyBndcustom() returned %d", retval);
    }
  }
  free(width_alldirs);
...
void CCTK_FCALL CCTK_FNAME (BndcustomDirGI)
                           (int *ierr,
                            const cGH **GH,
                            const int *stencil_size,
                            const int *dir,
                            const int *gi);
void CCTK_FCALL CCTK_FNAME (BndcustomGI)
                           (int *ierr,
                            const cGH **GH,
                            const int *stencil,
                            const int *gi);
void CCTK_FCALL CCTK_FNAME (BndcustomDirGN)
                           (int *ierr,
                            const cGH **GH,
                            const int *stencil_size,
                            const int *dir,
                            ONE_FORTSTRING_ARG);
void CCTK_FCALL CCTK_FNAME (BndcustomGN)
                           (int *ierr,
                            const cGH **GH,
                            const int *stencil,
                            ONE_FORTSTRING_ARG);
void CCTK_FCALL CCTK_FNAME (BndcustomDirVI)
                           (int *ierr,
                            const cGH **GH,
                            const int *stencil_size,
                            const int *dir,
                            const int *vi);
void CCTK_FCALL CCTK_FNAME (BndcustomVI)
                           (int *ierr,
                            const cGH **GH,
                            const int *stencil,
                            const int *vi);
void CCTK_FCALL CCTK_FNAME (BndcustomDirVN)
                           (int *ierr,
                            const cGH **GH,
                            const int *stencil_size,
                            const int *dir,
                            ONE_FORTSTRING_ARG);
void CCTK_FCALL CCTK_FNAME (BndcustomVN)
                           (int *ierr,
                            const cGH **GH,
                            const int *stencil,
                            ONE_FORTSTRING_ARG);
...
/*@@
   ...
   @endreturndesc
@@*/
int BndcustomDirGI (const cGH *GH,
                  int stencil_size,
                  int dir,
                  int gi)
{
  ...
  if (first_vi >= 0)
  {
    retval = ApplyBndcustom (GH, stencil_size, NULL, dir, first_vi,
                           CCTK_NumVarsInGroupI (gi));
  }
  else
  {
    CCTK_VWarn (2, __LINE__, __FILE__, CCTK_THORNSTRING,
                "Invalid group index %d in BndcustomDirGI", gi);
    retval = -1;
  }
...
void CCTK_FCALL CCTK_FNAME (BndcustomDirGI)
                           (int *ierr,
                            ...
                            const int *gi)
{
  *ierr = BndcustomDirGI (*GH, *stencil_size, *dir, *gi);
}
               ...
               int gi)
{
  ...
  if (first_vi >= 0)
  {
    retval = OldApplyBndcustom (GH, -1, stencil, 0, first_vi,
                           CCTK_NumVarsInGroupI (gi));
  }
  else
  {
...
void CCTK_FCALL CCTK_FNAME (BndcustomGI)
                           (int *ierr,
                            ...
                            const int *gi)
{
  *ierr = BndcustomGI (*GH, stencil, *gi);
}
...
/*@@
   ...
   @endreturndesc
@@*/
int BndcustomDirGN (const cGH *GH,
                  int stencil_size,
                  int dir,
                  const char *gn)
{
  ...
  if (gi >= 0)
  {
    retval = BndcustomDirGI (GH, stencil_size, dir, gi);
  }
  else
  {
    CCTK_VWarn (2, __LINE__, __FILE__, CCTK_THORNSTRING,
                "Invalid group name '%s' in BndcustomDirGN", gn);
    retval = -1;
  }
                            ...
                            ONE_FORTSTRING_ARG)
{
  ...
  *ierr = BndcustomDirGN (*GH, *stencil_size, *dir, gn);
  free (gn);
}
               ...
               const char *gn)
{
  ...
  if (gi >= 0)
  {
    retval = BndcustomGI (GH, stencil, gi);
  }
  else
  {
    CCTK_VWarn (2, __LINE__, __FILE__, CCTK_THORNSTRING,
                "Invalid group name '%s' in BndcustomGN", gn);
    retval = -1;
  }
...
{
  ONE_FORTSTRING_CREATE (gn)
  *ierr = BndcustomGN (*GH, stencil, gn);
  free (gn);
}
...
/*@@
   ...
   @endreturndesc
@@*/
int BndcustomDirVI (const cGH *GH,
                  int stencil_size,
                  int dir,
                  int vi)
{
  ...
  {
    retval = ApplyBndcustom (GH, stencil_size, NULL, dir, vi, 1);
  }
  else
  {
    CCTK_VWarn (2, __LINE__, __FILE__, CCTK_THORNSTRING,
                "Invalid variable index %d in BndcustomDirVI", vi);
    retval = -1;
  }
...
void CCTK_FCALL CCTK_FNAME (BndcustomDirVI)
                           (int *ierr,
                            ...
                            const int *vi)
{
  *ierr = BndcustomDirVI (*GH, *stencil_size, *dir, *vi);
}
               ...
               int vi)
{
  ...
  if (vi >= 0 && vi < CCTK_NumVars ())
  {
    retval = OldApplyBndcustom (GH, -1, stencil, 0, vi, 1);
  }
  else
  {
    CCTK_VWarn (2, __LINE__, __FILE__, CCTK_THORNSTRING,
                "Invalid variable index %d in BndcustomVI", vi);
    retval = -1;
  }
...
void CCTK_FCALL CCTK_FNAME (BndcustomVI)
                           (int *ierr,
                            ...
                            const int *vi)
{
  *ierr = BndcustomVI (*GH, stencil, *vi);
}
...
/*@@
   ...
   @endreturndesc
@@*/
int BndcustomDirVN (const cGH *GH,
                  int stencil_size,
                  int dir,
                  const char *vn)
{
  ...
  if (vi >= 0)
  {
    retval = BndcustomDirVI (GH, stencil_size, dir, vi);
  }
  else
  {
    CCTK_VWarn (2, __LINE__, __FILE__, CCTK_THORNSTRING,
                "Invalid variable name '%s' in BndcustomDirVN", vn);
    retval = -1;
  }
                            ...
                            ONE_FORTSTRING_ARG)
{
  ...
  *ierr = BndcustomDirVN (*GH, *stencil_size, *dir, vn);
  free (vn);
}
               ...
               const char *vn)
{
  ...
  if (vi >= 0)
  {
    retval = BndcustomVI (GH, stencil, vi);
  }
  else
  {
    CCTK_VWarn (2, __LINE__, __FILE__, CCTK_THORNSTRING,
                "Invalid variable name '%s' in BndcustomVN", vn);
    retval = -1;
  }
...
{
  ONE_FORTSTRING_CREATE (vn)
  *ierr = BndcustomVN (*GH, stencil, vn);
  free (vn);
}
...
/*#define customtwo_BOUNDARY(doBC,                                              \
                      iend, jend, kend,                                       \
                      ii_to, jj_to, kk_to,                                    \
                      ii_from, jj_from, kk_from)                              \
{                                                                             \
...
//  int face_xmin,face_xmax,face_ymin,face_ymax,face_zmin,face_zmax;
...
//  DECLARE_CCTK_ARGUMENTS
  ...
  {
    CCTK_VWarn (1, __LINE__, __FILE__, CCTK_THORNSTRING,
                "ApplyBndcustom: Variable dimension of %d not supported", gdim);
    return (-1);
  }
  ...
  {
    CCTK_VWarn (1, __LINE__, __FILE__, CCTK_THORNSTRING,
                "ApplyBndcustom: direction %d greater than dimension %d",
                dir, gdim);
    return (-2);
  }
  ...
  {
    CCTK_WARN (1, "ApplyBndcustom: NULL pointer passed for boundary width "
               "array");
    return (-3);
  }
  ...
  BndSanityCheckWidths(GH, first_var, gdim, widths, "custom");
...
//QUITAMOS ESTO QUE PARECER SER EL LOOP QUE YO YA DEFINI EN MI SUBRUTINA
    if (gdim > 0)
    {
#ifdef DEBUG_BOUNDARY
      if (doBC[0])
      {
        printf("Boundary: Applying lower x custom boundary condition\n");
      }
      if (doBC[1])
      {
        printf("Boundary: Applying upper x custom boundary condition\n");
      }
#endif /* DEBUG_BOUNDARY */

 /* lower x */

      if(face_xmin==1)  //Applying none boundary conitions "inflow"
      {
         customtwo_BOUNDARY (doBC[0], widths[0], lsh[1], lsh[2], i, j, k);
      }
      else if(face_xmin==2) //Aplying flat boundary conditions "outflow"
      {
         custom_BOUNDARY (doBC[0], widths[0], lsh[1], lsh[2], i, j, k, widths[0], j, k);
      }
 /* upper x */
    
      if(face_xmax==1)
      {
         customtwo_BOUNDARY (doBC[1], widths[1], lsh[1], lsh[2], lsh[0]-i-1, j, k);
      }
      else if(face_xmax==2)
      {   
         custom_BOUNDARY (doBC[1], widths[1], lsh[1], lsh[2], lsh[0]-i-1, j, k, lsh[0]-widths[1]-1, j, k);
       }
    }
...
/* lower y */
      ...
      if(face_ymin==1)  //Applying none boundary conitions "inflow"
      {
        customtwo_BOUNDARY (doBC[2], lsh[0], widths[2], lsh[2], i, j, k);
      } 
      else if(face_ymin==2)//Applying flat boundary condition "ouflow"
      {
        custom_BOUNDARY (doBC[2], lsh[0], widths[2], lsh[2], i, j, k, i, widths[2], k);
      }

/* upper y */

      if(face_ymax==1)  //Applying none boundary conitions "inflow" 
      {
        customtwo_BOUNDARY (doBC[3], lsh[0], widths[3], lsh[2], i, lsh[1]-j-1,k);
      }
      else if(face_ymax==2)//Applying flat boundary condition "ouflow"
      {
        custom_BOUNDARY (doBC[3], lsh[0], widths[3], lsh[2], i, lsh[1]-j-1, k, i, lsh[1]-widths[3]-1, k);
      }
    ...
    if (gdim > 2)
    {
      ...
      if(face_zmin==1)
      {
        customtwo_BOUNDARY (doBC[4], lsh[0], lsh[1], widths[4], i, j, k);
      }
      else if(face_zmin==2)
      {
        custom_BOUNDARY (doBC[4], lsh[0], lsh[1], widths[4], i, j, k, i, j, widths[4]);
      }
    ...
    /* upper z */
      ...
      if(face_zmax==1)
      {
         customtwo_BOUNDARY (doBC[5], lsh[0], lsh[1], widths[5], i, j,lsh[2]-k-1);
      }
      else if(face_zmax==2)
      {
         custom_BOUNDARY (doBC[5], lsh[0], lsh[1], widths[5], i, j, lsh[2]-k-1, i, j, lsh[2]-widths[5]-1);
      }
    }
...
int OldApplyBndcustom(const cGH *GH, int stencil_dir, const int *stencil_alldirs,
                    int dir, int first_var, int num_vars)
{
  ...
  retval = ApplyBndcustom(GH, stencil_dir, boundary_widths, dir, first_var,
                        num_vars);
