...
extern "C" void bssn_boundary_SelectBCs(CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  
  if (cctk_iteration % bssn_boundary_calc_every != bssn_boundary_calc_offset)
    return;
  ...
  if (ierr < 0)
    CCTK_WARN(1, "Failed to register flat BC for Kranc2BSSN::alpha_grouprhs.");
...
static void bssn_boundary_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
    ...
    cctk_ash[0],cctk_ash[1],cctk_ash[2])
  {
    ...
    CCTK_REAL phirhsL CCTK_ATTRIBUTE_UNUSED = 
      ...
      PDonesided2nd3(&phi[index])*n3 - phiL*pow(rL,-1);
  ...
  CCTK_ENDLOOP3(bssn_boundary);
}
extern "C" void bssn_boundary(CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Entering bssn_boundary_Body");
  }
  if (cctk_iteration % bssn_boundary_calc_every != bssn_boundary_calc_offset)
  {
    return;
  ...
  AssertGroupStorage(cctkGH, "bssn_boundary", 18, groups);
  
  switch (constraintsfdorder)
  {
    case 2:
    {
      EnsureStencilFits(cctkGH, "bssn_boundary", 2, 2, 2);
      break;
    }
  ...
  LoopOverBoundary(cctkGH, bssn_boundary_Body);
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Leaving bssn_boundary_Body");
  }
}
