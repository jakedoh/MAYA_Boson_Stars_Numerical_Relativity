...
static void bssn_project_constraints_psu_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
  ...
  #pragma omp parallel
  CCTK_LOOP3(bssn_project_constraints_psu,
  ...
  CCTK_ENDLOOP3(bssn_project_constraints_psu);
}
extern "C" void bssn_project_constraints_psu(CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Entering bssn_project_constraints_psu_Body");
  }
  if (cctk_iteration % bssn_project_constraints_psu_calc_every != bssn_project_constraints_psu_calc_offset)
  {
    return;
  }
  ...
  const char* const groups[] = {
    "Kranc2BSSN::A_group",
    "Kranc2BSSN::h_group"};
  AssertGroupStorage(cctkGH, "bssn_project_constraints_psu", 2, groups);
  ...
  LoopOverEverything(cctkGH, bssn_project_constraints_psu_Body);
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Leaving bssn_project_constraints_psu_Body");
  }
}
