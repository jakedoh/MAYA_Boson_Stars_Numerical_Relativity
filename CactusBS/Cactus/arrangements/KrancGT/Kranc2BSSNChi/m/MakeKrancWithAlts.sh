...
# Create the thorns
if [ -z "`which kranc 2>/dev/null`" ] ; then # walk the directory tree looking for a Kranc installation ...
    oldcwd="`pwd`"
    cwd="`pwd`"
    while [ "$cwd" != "/" ] ; do
        KRANCPATHS=(. repos) # look for Kranc directly and in repos (from GetComponents)
        for i in ${KRANCPATHS[@]} ; do
          if [ -x "$cwd/$i/Kranc/Bin/kranc" ] ; then
              KRANCPATH="$cwd/$i/Kranc"
              break
          elif [ -x "$cwd/$i/kranc/Bin/kranc" ] ; then
              KRANCPATH="$cwd/$i/kranc"
              break
          fi
        done
        if [ -n "$KRANCPATH" ] ; then
           PATH="$PATH:$KRANCPATH/Bin"
           break
        fi
        cd ..
        cwd="`pwd`"
...
echo "Using kranc from `which kranc`"
# Create file with info concerning Kranc version used
cwd="`pwd`"
KFile=$cwd/KrancRepositoryInformation
   KRANC=`which kranc`
   cd ${KRANC%/kranc}/..
   echo    "Kranc version used to generate thorn" &> $KFile
   echo -e "Repository location: ${KRANC%/Bin/kranc}\n" >> $KFile
   echo -e "Branches:" >> $KFile
   git branch >> $KFile
   echo -e "\nTop Commits:\n" >> $KFile
   git log -3 >> $KFile
   cd $cwd
...
## Add mol_register_tmunu parameter
cat << 'EOF' >> Kranc2BSSN${Chi}Matter/param.ccl.new
restricted:
BOOLEAN mol_register_tmunu "Register Tmunu with MoL? Known to cause problems with IllinoisGRMHD."
{
} "yes"
EOF
...
found++
print \"\"
print \"  if (mol_register_tmunu && (CCTK_EQUALS(couple_matter, \\\"yes\\\") || CCTK_EQUALS(recalculate_constraints, \\\"matter\\\"))) {\"
