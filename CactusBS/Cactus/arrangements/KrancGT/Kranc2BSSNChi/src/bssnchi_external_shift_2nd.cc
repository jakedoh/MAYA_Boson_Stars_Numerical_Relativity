...
extern "C" void bssnchi_external_shift_2nd_SelectBCs(CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  
  if (cctk_iteration % bssnchi_external_shift_2nd_calc_every != bssnchi_external_shift_2nd_calc_offset)
    return;
...
static void bssnchi_external_shift_2nd_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  #pragma omp parallel
  CCTK_LOOP3(bssnchi_external_shift_2nd,
    ...
    cctk_ash[0],cctk_ash[1],cctk_ash[2])
  {
    ...
    CCTK_REAL betat1rhsL CCTK_ATTRIBUTE_UNUSED = 
      -(betat1L*externalEtaBetaL) + (beta1L*PDonesided2nd1(&betat1[index]) + 
      beta2L*PDonesided2nd2(&betat1[index]) + 
      beta3L*PDonesided2nd3(&betat1[index]))*betatAdvection - 
    ...
    CCTK_REAL betat2rhsL CCTK_ATTRIBUTE_UNUSED = 
      -(betat2L*externalEtaBetaL) + (beta1L*PDonesided2nd1(&betat2[index]) + 
      beta2L*PDonesided2nd2(&betat2[index]) + 
      beta3L*PDonesided2nd3(&betat2[index]))*betatAdvection - 
    ...
    CCTK_REAL betat3rhsL CCTK_ATTRIBUTE_UNUSED = 
      -(betat3L*externalEtaBetaL) + (beta1L*PDonesided2nd1(&betat3[index]) + 
      beta2L*PDonesided2nd2(&betat3[index]) + 
      beta3L*PDonesided2nd3(&betat3[index]))*betatAdvection - 
  ...
  CCTK_ENDLOOP3(bssnchi_external_shift_2nd);
}
extern "C" void bssnchi_external_shift_2nd(CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Entering bssnchi_external_shift_2nd_Body");
  }
  if (cctk_iteration % bssnchi_external_shift_2nd_calc_every != bssnchi_external_shift_2nd_calc_offset)
  {
    return;
  ...
  AssertGroupStorage(cctkGH, "bssnchi_external_shift_2nd", 8, groups);
  
  switch (constraintsfdorder)
  {
    case 2:
    {
      EnsureStencilFits(cctkGH, "bssnchi_external_shift_2nd", 2, 2, 2);
      break;
    }
  ...
  LoopOverInterior(cctkGH, bssnchi_external_shift_2nd_Body);
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Leaving bssnchi_external_shift_2nd_Body");
  }
}
