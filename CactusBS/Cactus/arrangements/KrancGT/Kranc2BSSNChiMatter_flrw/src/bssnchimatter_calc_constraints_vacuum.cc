...
extern "C" void bssnchimatter_calc_constraints_vacuum_SelectBCs(CCTK_ARGUMENTS)
{
  #ifdef DECLARE_CCTK_ARGUMENTS_bssnchimatter_calc_constraints_vacuum_SelectBCs
  DECLARE_CCTK_ARGUMENTS_CHECKED(bssnchimatter_calc_constraints_vacuum_SelectBCs);
  #else
  DECLARE_CCTK_ARGUMENTS;
  #endif
  DECLARE_CCTK_PARAMETERS;
  
  if (cctk_iteration % bssnchimatter_calc_constraints_vacuum_calc_every != bssnchimatter_calc_constraints_vacuum_calc_offset)
    return;
  CCTK_INT ierr CCTK_ATTRIBUTE_UNUSED = 0;
  ierr = KrancBdy_SelectGroupForBC(cctkGH, CCTK_ALL_FACES, GetBoundaryWidth(cctkGH), -1 /* no table */, "Kranc2BSSNChiMatter::bssnCcons_group","flat");
  if (ierr < 0)
    CCTK_WARN(CCTK_WARN_ALERT, "Failed to register flat BC for Kranc2BSSNChiMatter::bssnCcons_group.");
  ierr = KrancBdy_SelectGroupForBC(cctkGH, CCTK_ALL_FACES, GetBoundaryWidth(cctkGH), -1 /* no table */, "Kranc2BSSNChiMatter::bssnmom_group","flat");
  if (ierr < 0)
...
static void bssnchimatter_calc_constraints_vacuum_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  #pragma omp parallel
  CCTK_LOOP3(bssnchimatter_calc_constraints_vacuum,
    ...
    cctk_ash[0],cctk_ash[1],cctk_ash[2])
  {
    ...
    CCTK_REAL R31 CCTK_ATTRIBUTE_UNUSED = 0.5*(Gam1L*(h31L*gamma111 + 
      ...
      h32L*gamma111*gamma231 + 3*h21L*gamma131*gamma231 + 
      h11L*gamma132*gamma231 + h32L*gamma211*gamma232 + 
      h21L*gamma231*gamma232 + h31L*gamma133*gamma311 + 
      h32L*gamma233*gamma311 + h33L*gamma111*gamma331 + 
      h11L*gamma133*gamma331 + 2*h32L*gamma231*gamma331 + 
      h21L*gamma233*gamma331 + h31L*gamma131*(gamma111 + 3*gamma331) + 
    ...
    CCTK_REAL R32 CCTK_ATTRIBUTE_UNUSED = 0.5*(Gam1L*(h31L*gamma121 + 
      ...
      hInv13*(h32L*gamma121*gamma231 + 2*h21L*gamma132*gamma231 + 
      h32L*gamma221*gamma232 + 2*h22L*gamma231*gamma232 + 
      h31L*gamma133*gamma321 + h32L*gamma233*gamma321 + 
      h33L*gamma121*gamma331 + h21L*gamma133*gamma331 + 
      h32L*gamma232*gamma331 + h22L*gamma233*gamma331 + 
      h31L*gamma132*(gamma221 + gamma331) + h33L*gamma221*gamma332 + 
      2*h32L*gamma231*gamma332 + h33L*gamma331*gamma332 + 
      gamma131*(h31L*gamma121 + h11L*gamma132 + h22L*gamma231 + h21L*gamma232 
      + h32L*gamma331 + h31L*gamma332) + h33L*gamma321*gamma333 + 
      h32L*gamma331*gamma333 + h21L*pow(gamma131,2)) + 
  ...
  CCTK_ENDLOOP3(bssnchimatter_calc_constraints_vacuum);
}
extern "C" void bssnchimatter_calc_constraints_vacuum(CCTK_ARGUMENTS)
{
  #ifdef DECLARE_CCTK_ARGUMENTS_bssnchimatter_calc_constraints_vacuum
  DECLARE_CCTK_ARGUMENTS_CHECKED(bssnchimatter_calc_constraints_vacuum);
  #else
  DECLARE_CCTK_ARGUMENTS;
  #endif
  DECLARE_CCTK_PARAMETERS;
  ...
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Entering bssnchimatter_calc_constraints_vacuum_Body");
  }
  if (cctk_iteration % bssnchimatter_calc_constraints_vacuum_calc_every != bssnchimatter_calc_constraints_vacuum_calc_offset)
  {
    return;
  ...
  AssertGroupStorage(cctkGH, "bssnchimatter_calc_constraints_vacuum", 9, groups);
  
  switch (constraintsfdorder)
  {
    case 2:
    {
      EnsureStencilFits(cctkGH, "bssnchimatter_calc_constraints_vacuum", 1, 1, 1);
      break;
    }
    
    case 4:
    {
      EnsureStencilFits(cctkGH, "bssnchimatter_calc_constraints_vacuum", 2, 2, 2);
      break;
    }
    
    case 6:
    {
      EnsureStencilFits(cctkGH, "bssnchimatter_calc_constraints_vacuum", 3, 3, 3);
      break;
    }
    default:
      CCTK_BUILTIN_UNREACHABLE();
  ...
  LoopOverInterior(cctkGH, bssnchimatter_calc_constraints_vacuum_Body);
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Leaving bssnchimatter_calc_constraints_vacuum_Body");
  }
}
