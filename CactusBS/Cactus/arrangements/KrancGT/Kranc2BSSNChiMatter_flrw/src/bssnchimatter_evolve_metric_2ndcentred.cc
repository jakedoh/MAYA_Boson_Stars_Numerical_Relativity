...
extern "C" void bssnchimatter_evolve_metric_2ndcentred_SelectBCs(CCTK_ARGUMENTS)
{
  #ifdef DECLARE_CCTK_ARGUMENTS_bssnchimatter_evolve_metric_2ndcentred_SelectBCs
  DECLARE_CCTK_ARGUMENTS_CHECKED(bssnchimatter_evolve_metric_2ndcentred_SelectBCs);
  #else
  DECLARE_CCTK_ARGUMENTS;
  #endif
  DECLARE_CCTK_PARAMETERS;
  
  if (cctk_iteration % bssnchimatter_evolve_metric_2ndcentred_calc_every != bssnchimatter_evolve_metric_2ndcentred_calc_offset)
    return;
...
static void bssnchimatter_evolve_metric_2ndcentred_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  #pragma omp parallel
  CCTK_LOOP3(bssnchimatter_evolve_metric_2ndcentred,
    ...
    cctk_ash[0],cctk_ash[1],cctk_ash[2])
  {
    ...
    CCTK_REAL chirhsL CCTK_ATTRIBUTE_UNUSED = beta1L*PDstandard2nd1chi + 
      ...
      PDstandard2nd2beta2 + PDstandard2nd3beta3)) + beta3L*PDstandard2nd3chi;
  ...
  CCTK_ENDLOOP3(bssnchimatter_evolve_metric_2ndcentred);
}
extern "C" void bssnchimatter_evolve_metric_2ndcentred(CCTK_ARGUMENTS)
{
  #ifdef DECLARE_CCTK_ARGUMENTS_bssnchimatter_evolve_metric_2ndcentred
  DECLARE_CCTK_ARGUMENTS_CHECKED(bssnchimatter_evolve_metric_2ndcentred);
  #else
  DECLARE_CCTK_ARGUMENTS;
  #endif
  DECLARE_CCTK_PARAMETERS;
  ...
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Entering bssnchimatter_evolve_metric_2ndcentred_Body");
  }
  if (cctk_iteration % bssnchimatter_evolve_metric_2ndcentred_calc_every != bssnchimatter_evolve_metric_2ndcentred_calc_offset)
  {
    return;
  ...
  AssertGroupStorage(cctkGH, "bssnchimatter_evolve_metric_2ndcentred", 8, groups);
  
  switch (constraintsfdorder)
  {
    case 2:
    {
      EnsureStencilFits(cctkGH, "bssnchimatter_evolve_metric_2ndcentred", 1, 1, 1);
      break;
    }
  ...
  LoopOverInterior(cctkGH, bssnchimatter_evolve_metric_2ndcentred_Body);
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Leaving bssnchimatter_evolve_metric_2ndcentred_Body");
  }
}
