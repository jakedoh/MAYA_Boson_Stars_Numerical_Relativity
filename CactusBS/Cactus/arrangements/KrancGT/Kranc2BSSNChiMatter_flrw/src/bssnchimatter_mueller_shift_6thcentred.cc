...
extern "C" void bssnchimatter_mueller_shift_6thcentred_SelectBCs(CCTK_ARGUMENTS)
{
  #ifdef DECLARE_CCTK_ARGUMENTS_bssnchimatter_mueller_shift_6thcentred_SelectBCs
  DECLARE_CCTK_ARGUMENTS_CHECKED(bssnchimatter_mueller_shift_6thcentred_SelectBCs);
  #else
  DECLARE_CCTK_ARGUMENTS;
  #endif
  DECLARE_CCTK_PARAMETERS;
  
  if (cctk_iteration % bssnchimatter_mueller_shift_6thcentred_calc_every != bssnchimatter_mueller_shift_6thcentred_calc_offset)
    return;
...
static void bssnchimatter_mueller_shift_6thcentred_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  #pragma omp parallel
  CCTK_LOOP3(bssnchimatter_mueller_shift_6thcentred,
    ...
    cctk_ash[0],cctk_ash[1],cctk_ash[2])
  {
    ...
    CCTK_REAL mueller CCTK_ATTRIBUTE_UNUSED = 
      exetaBetaL*pow(muellerBottom,-1)*pow(fabs(muellerTop),0.5);
  ...
  CCTK_ENDLOOP3(bssnchimatter_mueller_shift_6thcentred);
}
extern "C" void bssnchimatter_mueller_shift_6thcentred(CCTK_ARGUMENTS)
{
  #ifdef DECLARE_CCTK_ARGUMENTS_bssnchimatter_mueller_shift_6thcentred
  DECLARE_CCTK_ARGUMENTS_CHECKED(bssnchimatter_mueller_shift_6thcentred);
  #else
  DECLARE_CCTK_ARGUMENTS;
  #endif
  DECLARE_CCTK_PARAMETERS;
  ...
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Entering bssnchimatter_mueller_shift_6thcentred_Body");
  }
  if (cctk_iteration % bssnchimatter_mueller_shift_6thcentred_calc_every != bssnchimatter_mueller_shift_6thcentred_calc_offset)
  {
    return;
  ...
  AssertGroupStorage(cctkGH, "bssnchimatter_mueller_shift_6thcentred", 10, groups);
  
  switch (constraintsfdorder)
  {
    case 2:
    {
      EnsureStencilFits(cctkGH, "bssnchimatter_mueller_shift_6thcentred", 3, 3, 3);
      break;
    }
  ...
  LoopOverInterior(cctkGH, bssnchimatter_mueller_shift_6thcentred_Body);
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Leaving bssnchimatter_mueller_shift_6thcentred_Body");
  }
}
