...
static void bssnchimatter_project_constraints_psu_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
  ...
  #pragma omp parallel
  CCTK_LOOP3(bssnchimatter_project_constraints_psu,
    ...
    cctk_ash[0],cctk_ash[1],cctk_ash[2])
  {
    ...
    chiL = fmax(chiL,chiUniversalCutoff);
  ...
  CCTK_ENDLOOP3(bssnchimatter_project_constraints_psu);
}
extern "C" void bssnchimatter_project_constraints_psu(CCTK_ARGUMENTS)
{
  #ifdef DECLARE_CCTK_ARGUMENTS_bssnchimatter_project_constraints_psu
  DECLARE_CCTK_ARGUMENTS_CHECKED(bssnchimatter_project_constraints_psu);
  #else
  DECLARE_CCTK_ARGUMENTS;
  #endif
  DECLARE_CCTK_PARAMETERS;
  ...
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Entering bssnchimatter_project_constraints_psu_Body");
  }
  if (cctk_iteration % bssnchimatter_project_constraints_psu_calc_every != bssnchimatter_project_constraints_psu_calc_offset)
  {
    return;
  }
  ...
  AssertGroupStorage(cctkGH, "bssnchimatter_project_constraints_psu", 3, groups);
  ...
  LoopOverEverything(cctkGH, bssnchimatter_project_constraints_psu_Body);
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Leaving bssnchimatter_project_constraints_psu_Body");
  }
}
