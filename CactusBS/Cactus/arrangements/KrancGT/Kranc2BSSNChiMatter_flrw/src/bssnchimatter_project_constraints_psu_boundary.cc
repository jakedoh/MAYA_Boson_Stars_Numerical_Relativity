...
{
  #ifdef DECLARE_CCTK_ARGUMENTS_bssnchimatter_project_constraints_psu_boundary_SelectBCs
  DECLARE_CCTK_ARGUMENTS_CHECKED(bssnchimatter_project_constraints_psu_boundary_SelectBCs);
  #else
  ...
  ierr = KrancBdy_SelectGroupForBC(cctkGH, CCTK_ALL_FACES, GetBoundaryWidth(cctkGH), -1 /* no table */, "Kranc2BSSNChiMatter::A_group","flat");
  if (ierr < 0)
...
{
  #ifdef DECLARE_CCTK_ARGUMENTS_bssnchimatter_project_constraints_psu_boundary
  DECLARE_CCTK_ARGUMENTS_CHECKED(bssnchimatter_project_constraints_psu_boundary);
  #else
  DECLARE_CCTK_ARGUMENTS;
  #endif
  DECLARE_CCTK_PARAMETERS;
