...
extern "C" void bssnchimatter_radiativeFLRWboundary_SelectBCs(CCTK_ARGUMENTS)
{
  #ifdef DECLARE_CCTK_ARGUMENTS_bssnchimatter_radiativeFLRWboundary_SelectBCs
  DECLARE_CCTK_ARGUMENTS_CHECKED(bssnchimatter_radiativeFLRWboundary_SelectBCs);
  #else
  DECLARE_CCTK_ARGUMENTS;
  #endif
  DECLARE_CCTK_PARAMETERS;
  
  if (cctk_iteration % bssnchimatter_radiativeFLRWboundary_calc_every != bssnchimatter_radiativeFLRWboundary_calc_offset)
    return;
...
static void bssnchimatter_radiativeFLRWboundary_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  #pragma omp parallel
  CCTK_LOOP3(bssnchimatter_radiativeFLRWboundary,
    ...
    cctk_ash[0],cctk_ash[1],cctk_ash[2])
  {
    ...
    CCTK_REAL eightfac CCTK_ATTRIBUTE_UNUSED = 
      1.632993161855452*pow(pi*(VVbnd + 0.5*pow(inipiabnd,2)),0.5);
    
    CCTK_REAL aexact CCTK_ATTRIBUTE_UNUSED = exp(eightfac*t);
    
    CCTK_REAL K0 CCTK_ATTRIBUTE_UNUSED = 
      -4.898979485566356*ksign*pow(pi*(VVbnd + 0.5*pow(inipiabnd,2)),0.5);
    ...
    CCTK_REAL KrhsL CCTK_ATTRIBUTE_UNUSED = PDonesided2nd1(&K[index])*n1 + 
      PDonesided2nd2(&K[index])*n2 + PDonesided2nd3(&K[index])*n3 + (-KL + 
      K0)*pow(rL,-1);
    ...
    CCTK_REAL chirhsL CCTK_ATTRIBUTE_UNUSED = 
      ...
      PDonesided2nd3(&chi[index])*n3 - pow(rL,-1)*(chiL - pow(aexact,-2)) - 
      2.*eightfac*pow(aexact,-2);
  ...
  CCTK_ENDLOOP3(bssnchimatter_radiativeFLRWboundary);
}
extern "C" void bssnchimatter_radiativeFLRWboundary(CCTK_ARGUMENTS)
{
  #ifdef DECLARE_CCTK_ARGUMENTS_bssnchimatter_radiativeFLRWboundary
  DECLARE_CCTK_ARGUMENTS_CHECKED(bssnchimatter_radiativeFLRWboundary);
  #else
  DECLARE_CCTK_ARGUMENTS;
  #endif
  DECLARE_CCTK_PARAMETERS;
  ...
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Entering bssnchimatter_radiativeFLRWboundary_Body");
  }
  if (cctk_iteration % bssnchimatter_radiativeFLRWboundary_calc_every != bssnchimatter_radiativeFLRWboundary_calc_offset)
  {
    return;
  ...
  AssertGroupStorage(cctkGH, "bssnchimatter_radiativeFLRWboundary", 18, groups);
  
  switch (constraintsfdorder)
  {
    case 2:
    {
      EnsureStencilFits(cctkGH, "bssnchimatter_radiativeFLRWboundary", 2, 2, 2);
      break;
    }
  ...
  LoopOverBoundary(cctkGH, bssnchimatter_radiativeFLRWboundary_Body);
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Leaving bssnchimatter_radiativeFLRWboundary_Body");
  }
}
