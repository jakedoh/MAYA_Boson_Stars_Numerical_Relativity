...
extern "C" void bssnmatter_calc_constraints_vacuum_SelectBCs(CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  
  if (cctk_iteration % bssnmatter_calc_constraints_vacuum_calc_every != bssnmatter_calc_constraints_vacuum_calc_offset)
    return;
  CCTK_INT ierr CCTK_ATTRIBUTE_UNUSED = 0;
  ierr = Boundary_SelectGroupForBC(cctkGH, CCTK_ALL_FACES, GetBoundaryWidth(cctkGH), -1 /* no table */, "Kranc2BSSNMatter::bssnCcons_group","flat");
  if (ierr < 0)
    CCTK_WARN(1, "Failed to register flat BC for Kranc2BSSNMatter::bssnCcons_group.");
  ...
  if (ierr < 0)
    CCTK_WARN(1, "Failed to register flat BC for Kranc2BSSNMatter::bssnmom_group.");
...
static void bssnmatter_calc_constraints_vacuum_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  #pragma omp parallel
  CCTK_LOOP3(bssnmatter_calc_constraints_vacuum,
  ...
  CCTK_ENDLOOP3(bssnmatter_calc_constraints_vacuum);
}
extern "C" void bssnmatter_calc_constraints_vacuum(CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Entering bssnmatter_calc_constraints_vacuum_Body");
  }
  if (cctk_iteration % bssnmatter_calc_constraints_vacuum_calc_every != bssnmatter_calc_constraints_vacuum_calc_offset)
  {
    return;
  ...
  const char* const groups[] = {
    ...
    "Kranc2BSSNMatter::bssnCcons_group",
  ...
  AssertGroupStorage(cctkGH, "bssnmatter_calc_constraints_vacuum", 9, groups);
  
  switch (constraintsfdorder)
  {
    case 2:
    {
      EnsureStencilFits(cctkGH, "bssnmatter_calc_constraints_vacuum", 1, 1, 1);
      break;
    }
    
    case 4:
    {
      EnsureStencilFits(cctkGH, "bssnmatter_calc_constraints_vacuum", 2, 2, 2);
      break;
    }
    
    case 6:
    {
      EnsureStencilFits(cctkGH, "bssnmatter_calc_constraints_vacuum", 3, 3, 3);
      break;
    }
    default:
      CCTK_BUILTIN_UNREACHABLE();
  ...
  LoopOverInterior(cctkGH, bssnmatter_calc_constraints_vacuum_Body);
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Leaving bssnmatter_calc_constraints_vacuum_Body");
  }
}
