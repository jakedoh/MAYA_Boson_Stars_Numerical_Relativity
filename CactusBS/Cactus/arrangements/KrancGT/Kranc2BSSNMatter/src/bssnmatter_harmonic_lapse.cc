...
extern "C" void bssnmatter_harmonic_lapse_SelectBCs(CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  
  if (cctk_iteration % bssnmatter_harmonic_lapse_calc_every != bssnmatter_harmonic_lapse_calc_offset)
    return;
...
static void bssnmatter_harmonic_lapse_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  #pragma omp parallel
  CCTK_LOOP3(bssnmatter_harmonic_lapse,
  ...
  CCTK_ENDLOOP3(bssnmatter_harmonic_lapse);
}
extern "C" void bssnmatter_harmonic_lapse(CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Entering bssnmatter_harmonic_lapse_Body");
  }
  if (cctk_iteration % bssnmatter_harmonic_lapse_calc_every != bssnmatter_harmonic_lapse_calc_offset)
  {
    return;
  ...
  AssertGroupStorage(cctkGH, "bssnmatter_harmonic_lapse", 4, groups);
  
  switch (constraintsfdorder)
  {
    case 2:
    {
      EnsureStencilFits(cctkGH, "bssnmatter_harmonic_lapse", 2, 2, 2);
      break;
    }
  ...
  LoopOverInterior(cctkGH, bssnmatter_harmonic_lapse_Body);
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Leaving bssnmatter_harmonic_lapse_Body");
  }
}
