...
static void bssnmatter_project_constraints_psu_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
  ...
  #pragma omp parallel
  CCTK_LOOP3(bssnmatter_project_constraints_psu,
    ...
    cctk_ash[0],cctk_ash[1],cctk_ash[2])
  {
    ...
    invdeth = pow(2*h21L*h31L*h32L - h33L*pow(h21L,2) + h22L*(h11L*h33L - 
      pow(h31L,2)) - h11L*pow(h32L,2),-1);
  ...
  CCTK_ENDLOOP3(bssnmatter_project_constraints_psu);
}
extern "C" void bssnmatter_project_constraints_psu(CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Entering bssnmatter_project_constraints_psu_Body");
  }
  if (cctk_iteration % bssnmatter_project_constraints_psu_calc_every != bssnmatter_project_constraints_psu_calc_offset)
  {
    return;
  }
  ...
  AssertGroupStorage(cctkGH, "bssnmatter_project_constraints_psu", 2, groups);
  ...
  LoopOverEverything(cctkGH, bssnmatter_project_constraints_psu_Body);
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Leaving bssnmatter_project_constraints_psu_Body");
  }
}
