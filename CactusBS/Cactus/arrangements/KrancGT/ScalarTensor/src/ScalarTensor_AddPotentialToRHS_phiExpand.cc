...
extern "C" void ScalarTensor_AddPotentialToRHS_phiExpand_SelectBCs(CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  
  if (cctk_iteration % ScalarTensor_AddPotentialToRHS_phiExpand_calc_every != ScalarTensor_AddPotentialToRHS_phiExpand_calc_offset)
    return;
...
static void ScalarTensor_AddPotentialToRHS_phiExpand_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  #pragma omp parallel
  CCTK_LOOP3(ScalarTensor_AddPotentialToRHS_phiExpand,
    ...
    cctk_ash[0],cctk_ash[1],cctk_ash[2])
  {
    ...
    CCTK_REAL derivV CCTK_ATTRIBUTE_UNUSED = STphiL*Vlambda*(pow(STphiL,2) 
      - pow(Veta,2))*(pow(STphiL,2) - pow(Veps,2)*pow(Veta,2));
    
    STpirhsL = STpirhsL - derivV;
    /* Copy local copies back to grid functions */
    STpirhs[index] = STpirhsL;
  }
  CCTK_ENDLOOP3(ScalarTensor_AddPotentialToRHS_phiExpand);
}
extern "C" void ScalarTensor_AddPotentialToRHS_phiExpand(CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Entering ScalarTensor_AddPotentialToRHS_phiExpand_Body");
  }
  if (cctk_iteration % ScalarTensor_AddPotentialToRHS_phiExpand_calc_every != ScalarTensor_AddPotentialToRHS_phiExpand_calc_offset)
  {
    return;
  ...
  AssertGroupStorage(cctkGH, "ScalarTensor_AddPotentialToRHS_phiExpand", 2, groups);
  
  
  LoopOverInterior(cctkGH, ScalarTensor_AddPotentialToRHS_phiExpand_Body);
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Leaving ScalarTensor_AddPotentialToRHS_phiExpand_Body");
  }
}
