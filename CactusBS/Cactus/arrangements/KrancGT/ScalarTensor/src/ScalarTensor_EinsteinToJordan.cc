...
static void ScalarTensor_EinsteinToJordan_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
  ...
  #pragma omp parallel
  CCTK_LOOP3(ScalarTensor_EinsteinToJordan,
    ...
    cctk_ash[0],cctk_ash[1],cctk_ash[2])
  {
    ...
    CCTK_REAL aofphi CCTK_ATTRIBUTE_UNUSED = STalpha0 + 1.*STphiL*STBeta;
    
    alpL = alpL*ConfFactorA;
    
    kxxL = (kxxL - gxxL*STpiL*aofphi)*ConfFactorA;
    
    kxyL = (kxyL - gxyL*STpiL*aofphi)*ConfFactorA;
    
    kxzL = (kxzL - gxzL*STpiL*aofphi)*ConfFactorA;
    
    kyyL = (kyyL - gyyL*STpiL*aofphi)*ConfFactorA;
    
    kyzL = (kyzL - gyzL*STpiL*aofphi)*ConfFactorA;
    
    kzzL = (kzzL - gzzL*STpiL*aofphi)*ConfFactorA;
    
    gxxL = gxxL*pow(ConfFactorA,2);
    
    gxyL = gxyL*pow(ConfFactorA,2);
    
    gxzL = gxzL*pow(ConfFactorA,2);
    
    gyyL = gyyL*pow(ConfFactorA,2);
    
    gyzL = gyzL*pow(ConfFactorA,2);
    
    gzzL = gzzL*pow(ConfFactorA,2);
    /* Copy local copies back to grid functions */
    alp[index] = alpL;
    gxx[index] = gxxL;
    gxy[index] = gxyL;
  ...
  CCTK_ENDLOOP3(ScalarTensor_EinsteinToJordan);
}
extern "C" void ScalarTensor_EinsteinToJordan(CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Entering ScalarTensor_EinsteinToJordan_Body");
  }
  if (cctk_iteration % ScalarTensor_EinsteinToJordan_calc_every != ScalarTensor_EinsteinToJordan_calc_offset)
  {
    return;
  }
  ...
  AssertGroupStorage(cctkGH, "ScalarTensor_EinsteinToJordan", 5, groups);
  
  
  LoopOverEverything(cctkGH, ScalarTensor_EinsteinToJordan_Body);
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Leaving ScalarTensor_EinsteinToJordan_Body");
  }
}
