...
extern "C" void ScalarTensor_ScalarFieldTmunu_phiBubble_6th_SelectBCs(CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  
  if (cctk_iteration % ScalarTensor_ScalarFieldTmunu_phiBubble_6th_calc_every != ScalarTensor_ScalarFieldTmunu_phiBubble_6th_calc_offset)
    return;
...
static void ScalarTensor_ScalarFieldTmunu_phiBubble_6th_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  #pragma omp parallel
  CCTK_LOOP3(ScalarTensor_ScalarFieldTmunu_phiBubble_6th,
    ...
    cctk_ash[0],cctk_ash[1],cctk_ash[2])
  {
    ...
    CCTK_REAL STTtt CCTK_ATTRIBUTE_UNUSED = pow(dtSTphi,2) - 0.5*(betaSqr 
      ...
      0.25*Vlambda*(4*(-STphiL + phiBG)*Veps*pow(phiBG,3) + 
      pow(-pow(STphiL,2) + pow(phiBG,2),2)));
    
    CCTK_REAL STTtj1 CCTK_ATTRIBUTE_UNUSED = dtSTphi*PDstandard6th1STphi - 
      0.125*(gLocal11L*betaLocal1 + gLocal21L*betaLocal2 + 
      gLocal31L*betaLocal3)*(4*(dSTphiSqr + pow(STphiL,2)*pow(fieldMass,2)) + 
      Vlambda*(pow(STphiL,4) - 2*pow(STphiL,2)*pow(phiBG,2) + pow(phiBG,4) + 
      Veps*(-4*STphiL*pow(phiBG,3) + 4*pow(phiBG,4))));
    
    CCTK_REAL STTtj2 CCTK_ATTRIBUTE_UNUSED = dtSTphi*PDstandard6th2STphi - 
      0.125*(gLocal21L*betaLocal1 + gLocal22L*betaLocal2 + 
      gLocal32L*betaLocal3)*(4*(dSTphiSqr + pow(STphiL,2)*pow(fieldMass,2)) + 
      Vlambda*(pow(STphiL,4) - 2*pow(STphiL,2)*pow(phiBG,2) + pow(phiBG,4) + 
      Veps*(-4*STphiL*pow(phiBG,3) + 4*pow(phiBG,4))));
    
    CCTK_REAL STTtj3 CCTK_ATTRIBUTE_UNUSED = dtSTphi*PDstandard6th3STphi - 
      0.125*(gLocal31L*betaLocal1 + gLocal32L*betaLocal2 + 
      gLocal33L*betaLocal3)*(4*(dSTphiSqr + pow(STphiL,2)*pow(fieldMass,2)) + 
      Vlambda*(pow(STphiL,4) - 2*pow(STphiL,2)*pow(phiBG,2) + pow(phiBG,4) + 
      Veps*(-4*STphiL*pow(phiBG,3) + 4*pow(phiBG,4))));
    ...
    CCTK_REAL STTij21 CCTK_ATTRIBUTE_UNUSED = 
      ...
      pow(STphiL,2)*pow(fieldMass,2) + 0.25*Vlambda*(4*(-STphiL + 
  ...
  CCTK_ENDLOOP3(ScalarTensor_ScalarFieldTmunu_phiBubble_6th);
}
extern "C" void ScalarTensor_ScalarFieldTmunu_phiBubble_6th(CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Entering ScalarTensor_ScalarFieldTmunu_phiBubble_6th_Body");
  }
  if (cctk_iteration % ScalarTensor_ScalarFieldTmunu_phiBubble_6th_calc_every != ScalarTensor_ScalarFieldTmunu_phiBubble_6th_calc_offset)
  {
    return;
  ...
  AssertGroupStorage(cctkGH, "ScalarTensor_ScalarFieldTmunu_phiBubble_6th", 6, groups);
  
  EnsureStencilFits(cctkGH, "ScalarTensor_ScalarFieldTmunu_phiBubble_6th", 4, 4, 4);
  
  LoopOverInterior(cctkGH, ScalarTensor_ScalarFieldTmunu_phiBubble_6th_Body);
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Leaving ScalarTensor_ScalarFieldTmunu_phiBubble_6th_Body");
  }
}
