...
extern "C" void ScalarTensor_ScalarFieldTmunu_phiFourth_6th_SelectBCs(CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  
  if (cctk_iteration % ScalarTensor_ScalarFieldTmunu_phiFourth_6th_calc_every != ScalarTensor_ScalarFieldTmunu_phiFourth_6th_calc_offset)
    return;
...
static void ScalarTensor_ScalarFieldTmunu_phiFourth_6th_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  #pragma omp parallel
  CCTK_LOOP3(ScalarTensor_ScalarFieldTmunu_phiFourth_6th,
    ...
    cctk_ash[0],cctk_ash[1],cctk_ash[2])
  {
    ...
    CCTK_REAL STTtt CCTK_ATTRIBUTE_UNUSED = pow(dtSTphi,2) + 
      ...
      pow(STphiL,2)*pow(fieldMass,2)) + Vlambda*pow(-pow(STphiL,2) + 
      pow(phiBG,2),2));
    ...
    CCTK_REAL STTij11 CCTK_ATTRIBUTE_UNUSED = pow(PDstandard6th1STphi,2) - 
      ...
      0.25*Vlambda*pow(-pow(STphiL,2) + pow(phiBG,2),2));
  ...
  CCTK_ENDLOOP3(ScalarTensor_ScalarFieldTmunu_phiFourth_6th);
}
extern "C" void ScalarTensor_ScalarFieldTmunu_phiFourth_6th(CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Entering ScalarTensor_ScalarFieldTmunu_phiFourth_6th_Body");
  }
  if (cctk_iteration % ScalarTensor_ScalarFieldTmunu_phiFourth_6th_calc_every != ScalarTensor_ScalarFieldTmunu_phiFourth_6th_calc_offset)
  {
    return;
  ...
  AssertGroupStorage(cctkGH, "ScalarTensor_ScalarFieldTmunu_phiFourth_6th", 6, groups);
  
  EnsureStencilFits(cctkGH, "ScalarTensor_ScalarFieldTmunu_phiFourth_6th", 4, 4, 4);
  
  LoopOverInterior(cctkGH, ScalarTensor_ScalarFieldTmunu_phiFourth_6th_Body);
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Leaving ScalarTensor_ScalarFieldTmunu_phiFourth_6th_Body");
  }
}
