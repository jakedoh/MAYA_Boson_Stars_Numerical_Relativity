...
static void ScalarTensor_ZeroTmunuTrace_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
  ...
  #pragma omp parallel
  CCTK_LOOP3(ScalarTensor_ZeroTmunuTrace,
    ...
    cctk_ash[0],cctk_ash[1],cctk_ash[2])
  {
    ...
    CCTK_REAL TmunuTraceMatterLocalL CCTK_ATTRIBUTE_UNUSED = 0;
    /* Copy local copies back to grid functions */
    TmunuTraceMatterLocal[index] = TmunuTraceMatterLocalL;
  }
  CCTK_ENDLOOP3(ScalarTensor_ZeroTmunuTrace);
}
extern "C" void ScalarTensor_ZeroTmunuTrace(CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Entering ScalarTensor_ZeroTmunuTrace_Body");
  }
  if (cctk_iteration % ScalarTensor_ZeroTmunuTrace_calc_every != ScalarTensor_ZeroTmunuTrace_calc_offset)
  {
    return;
  }
  ...
  AssertGroupStorage(cctkGH, "ScalarTensor_ZeroTmunuTrace", 1, groups);
  
  
  LoopOverEverything(cctkGH, ScalarTensor_ZeroTmunuTrace_Body);
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Leaving ScalarTensor_ZeroTmunuTrace_Body");
  }
}
