...
extern "C" void ScalarTensor_calc_rho_phiSixth_full4th_SelectBCs(CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  
  if (cctk_iteration % ScalarTensor_calc_rho_phiSixth_full4th_calc_every != ScalarTensor_calc_rho_phiSixth_full4th_calc_offset)
    return;
...
static void ScalarTensor_calc_rho_phiSixth_full4th_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  #pragma omp parallel
  CCTK_LOOP3(ScalarTensor_calc_rho_phiSixth_full4th,
    ...
    cctk_ash[0],cctk_ash[1],cctk_ash[2])
  {
    ...
    rhoEL = 1.*(gLocalInv23*PDstandard4th2STphi*PDstandard4th3STphi + 
      ...
      0.125*Vlambda*pow(pow(STphiL,3) - STphiL*pow(phiBG,2),2);
  ...
  CCTK_ENDLOOP3(ScalarTensor_calc_rho_phiSixth_full4th);
}
extern "C" void ScalarTensor_calc_rho_phiSixth_full4th(CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Entering ScalarTensor_calc_rho_phiSixth_full4th_Body");
  }
  if (cctk_iteration % ScalarTensor_calc_rho_phiSixth_full4th_calc_every != ScalarTensor_calc_rho_phiSixth_full4th_calc_offset)
  {
    return;
  ...
  AssertGroupStorage(cctkGH, "ScalarTensor_calc_rho_phiSixth_full4th", 6, groups);
  
  EnsureStencilFits(cctkGH, "ScalarTensor_calc_rho_phiSixth_full4th", 2, 2, 2);
  
  LoopOverInterior(cctkGH, ScalarTensor_calc_rho_phiSixth_full4th_Body);
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Leaving ScalarTensor_calc_rho_phiSixth_full4th_Body");
  }
}
