...
void Meudon_Bin_NS_initialise (CCTK_ARGUMENTS)
{
  ...
  if (!(*init_eos_key = EOS_Omni_GetHandle(init_eos_table)))
    CCTK_WARN(0,"Cannot get initial eos handle, aborting...");
  
  CCTK_VInfo(CCTK_THORNSTRING, "Meudon_Bin_NS will use the %s equation of state.", init_eos_table);
