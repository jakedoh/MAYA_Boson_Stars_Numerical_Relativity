 /*@@
   ...
   @enddesc 
   @version  $Id: IO.c 58 2009-09-21 17:09:44Z schnetter $
 @@*/
...
CCTK_FILEVERSION(HerrmannCVS_WaveExtract_IO_c)

void WavExtr_WriteData(CCTK_ARGUMENTS);

void WavExtr_WriteLotsOfData(CCTK_ARGUMENTS);

int WavExtr_TimeForOutput (const cGH *GH, int vindex);

static void WavExtr_WriteTimer(cTimerData *info);

static void *WavExtr_SetupGH (tFleshConfig *config, int conv_level, cGH *GH);

static int WavExtr_WriteScalar(const cGH *GH,
                        CCTK_INT vindex,
                        CCTK_REAL  *value,
                        CCTK_INT   max,
                        const char *alias,
...
static FILE *WavExtr_OpenFile (const cGH *GH,
                               int vindex,
                               const char *filename,
...
@@*/
int WavExtr_Startup (void)
{
  if (CCTK_GHExtensionHandle ("IO") < 0)
  {
    CCTK_WARN (1, "Thorn IOUtil was not activated. "
                  "No WaveExtract I/O methods will be enabled.");
    return 0;
  }

  CCTK_RegisterGHExtensionSetupGH(CCTK_RegisterGHExtension("WaveExtract"),
                                   WavExtr_SetupGH);
...
void WavExtr_WriteData(CCTK_ARGUMENTS)
{
  ...
  vindex_rsch  = CCTK_VarIndex("WaveExtract::Schwarzschild_Radius");
  vindex_mass  = CCTK_VarIndex("WaveExtract::Schwarzschild_Mass");
  vindex_Qeven_Re = CCTK_VarIndex("WaveExtract::Qeven_Re");
  vindex_Qodd_Re  = CCTK_VarIndex("WaveExtract::Qodd_Re");
  vindex_Qeven_Im = CCTK_VarIndex("WaveExtract::Qeven_Im");
  vindex_Qodd_Im  = CCTK_VarIndex("WaveExtract::Qodd_Im");
  ...
  if(CCTK_MyProc (cctkGH) == 0)
  {
    ...
    WavExtr_WriteScalar(cctkGH,vindex_mass,Schwarzschild_Mass,
                        -1,string_var,NULL);

    /* Schw Radius */
    snprintf(string_var, sizeof string_var,
             "Schwarzschild_Radius_%s",radiusname);
    WavExtr_WriteScalar(cctkGH,vindex_rsch,Schwarzschild_Radius,
                        -1,string_var,NULL);
...
void WavExtr_WriteLotsOfData(CCTK_ARGUMENTS)
{
  ...
  if (*do_nothing==1)
    return;
  vindex_rsch  = CCTK_VarIndex("WaveExtract::Schw_Radii");
  vindex_mass  = CCTK_VarIndex("WaveExtract::Schw_Masses");
  vindex_Qeven_Re = CCTK_VarIndex("WaveExtract::Qeven_Re_Array");
  vindex_Qodd_Re  = CCTK_VarIndex("WaveExtract::Qodd_Re_Array");
  vindex_Qeven_Im = CCTK_VarIndex("WaveExtract::Qeven_Im_Array");
  vindex_Qodd_Im  = CCTK_VarIndex("WaveExtract::Qodd_Im_Array");
  ...
  if(CCTK_MyProc (cctkGH) == 0)
  {
    ...
    WavExtr_WriteScalar(cctkGH,vindex_mass,Schw_Masses,
                        maximum_detector_number,
                        string_var,detector_radius);
    /* Schw Radius */
    snprintf(string_var, sizeof string_var,
             "Schwarzschild_Radii");
    WavExtr_WriteScalar(cctkGH,vindex_rsch,Schw_Radii,
                        maximum_detector_number,
                        string_var,detector_radius);
    ...
    for (il=*l_min; il<=*l_max; il=il+*l_step) {
      for (im=*m_min; im<=*m_max; im=im+*m_step) {
        ...
        snprintf(string_var, sizeof string_var,
                 "Qeven_Re_l%d_m%d",il,im);
        WavExtr_WriteScalar(cctkGH,vindex_Qeven_Re,
                            val_even_Re,maximum_detector_number,
                            string_var,detector_radius);
        snprintf(string_var, sizeof string_var,
                 "Qodd_Re_l%d_m%d",il,im);
        WavExtr_WriteScalar(cctkGH,vindex_Qodd_Re,
        ...
        snprintf(string_var, sizeof string_var,
                 "Qeven_Im_l%d_m%d",il,im);
        WavExtr_WriteScalar(cctkGH,vindex_Qeven_Im,
                            val_even_Im,maximum_detector_number,
                            string_var,detector_radius);
        snprintf(string_var, sizeof string_var,
                 "Qodd_Im_l%d_m%d",il,im);
        WavExtr_WriteScalar(cctkGH,vindex_Qodd_Im,
...
void WavExtr_TimerInfo(CCTK_ARGUMENTS)
{
  cTimerData *info;
  CCTK_INT ierr;
  ...
  if (write_timer_info == 1)
  {
    ...
    ierr = CCTK_Timer("Interpolation",info);
    if (ierr >= 0)
    {
      CCTK_INFO("  Interpolation Timer");
      WavExtr_WriteTimer(info);
    }

    ierr = CCTK_Timer("Schwarzschild",info);
    if (ierr >= 0)
    {
    ...
    ierr = CCTK_Timer("MoncriefQ",info);
    if (ierr >= 0)
    {
                        ...
                        const CCTK_REAL  *detector_radii)
{
  ...
  myGH = (extractGH *) CCTK_GHExtension (GH, "WaveExtract");
  ...
  if(max==-1)
  {
    ...
    file = WavExtr_OpenFile(GH, vindex, filename, "tl", "Scalar value", 
                            -1, alias, NULL);
    if (file)
    {
  ...
  else /* multiple detectors in one file */
  {
    ...
    {
      CCTK_ParameterSet(out_style,"WaveExtract","gnuplot");
    }
    /* create/reopen the file */
    file = WavExtr_OpenFile(GH, vindex, filename, "tl", "Scalar value",
                               ...
                               const CCTK_REAL  *detector_radii)
{
  ...
  {
    CCTK_VWarn (1, __LINE__, __FILE__, CCTK_THORNSTRING,
                "WavExtr_OpenFile: Cannot open output file '%s'", filename);
  }
...
static void *WavExtr_SetupGH (tFleshConfig *config, int conv_level, cGH *GH)
{
  ...
  if (myGH)
  {
    i = CCTK_RegisterIOMethod ("WaveExtract");
    CCTK_RegisterIOMethodTimeToOutput (i, WavExtr_TimeForOutput);
 ...
 ********************************************************************/
 /*@@
   @routine    WavExtr_CheckSteerableParameters
