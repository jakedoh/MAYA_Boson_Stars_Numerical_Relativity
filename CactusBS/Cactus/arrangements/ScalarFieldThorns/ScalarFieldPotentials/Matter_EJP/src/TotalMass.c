...
static int sumLocalMass(CCTK_ARGUMENTS, CCTK_REAL * localVolMass, CCTK_REAL * localVolAngularJ);
...
void SumRestMass(CCTK_ARGUMENTS)
{
  ...
  CCTK_REAL sumVolMass[intvolumes], sumVolAngularJ[intvolumes];
  double localVolMass[intvolumes];
  double localVolAngularJ[intvolumes];
  int vol, ierr;
  ...
  if (do_calc_mass) 
  {
	...
	localVolAngularJ[vol] = -1e44;
     }

     if (verbose) CCTK_INFO("Ready for local loops");
     const int ierri = sumLocalMass(CCTK_PASS_CTOC, localVolMass, localVolAngularJ);
     assert(!ierri);
     ...
     ierr = CCTK_ReduceLocalArray1D (cctkGH, 0, sum_op, localVolAngularJ, sumVolAngularJ, intvolumes, CCTK_VARIABLE_REAL);
     assert(!ierr);
     if (ierr) CCTK_WARN (CCTK_WARN_ABORT, "error");
     for (vol=0; vol<intvolumes; vol++) {
     	     VolumeMass[vol] += *SymmetryFactor*sumVolMass[vol];
     	     Volume_AngulatJ[vol] += *SymmetryFactor*sumVolAngularJ[vol];
     }
...
static int sumLocalMass(CCTK_ARGUMENTS, CCTK_REAL * localVolMass, CCTK_REAL * localVolAngularJ )
{
  ...
  CCTK_REAL rhoEl,rhoJl;
  CCTK_REAL gxxL,gyyL,gzzL,gxyL,gxzL,gyzL,detg;
  CCTK_REAL densLe,densLJ,weightL,MassL,AngularJL;
  ...
  for(int k = imin[2] ; k < imax[2] ; k++)
  {
	...
	energy_momentum[0][0] = eTtt[idx];
	energy_momentum[0][1] = eTtx[idx];
	energy_momentum[0][2] = eTty[idx];
	energy_momentum[0][3] = eTtz[idx];
	energy_momentum[1][1] = eTxx[idx];
	energy_momentum[1][2] = eTxy[idx];
	energy_momentum[1][3] = eTxz[idx];
	energy_momentum[2][2] = eTyy[idx];
	energy_momentum[2][3] = eTyz[idx];
	energy_momentum[3][3] = eTzz[idx];
	...
	rhoEl = (1./lapse)*(energy_momentum[0][0]-shiftx*energy_momentum[0][1]-shifty*energy_momentum[0][2]-shiftz*energy_momentum[0][3]);
	...
	double xdc=0, ydc=0, zdc=0, rdc=0, rxy=0;
	double sint=0, cost=0, sinp=0,cosp=0,cott=0,csct=0;

	xdc= xL- x0L[vol];
	ydc= yL- y0L[vol];
	zdc= zL- z0L[vol];

	rxy=sqrt(xdc*xdc+ydc*ydc);
	...
	double Ttphi, Txphi, Typhi, Tzphi;
	Ttphi=-energy_momentum[0][1]*rdc*sint*sinp+energy_momentum[0][2]*rdc*sint*cosp;
	Txphi=-energy_momentum[1][1]*rdc*sint*sinp+energy_momentum[1][2]*rdc*sint*cosp;
	Typhi=-energy_momentum[1][2]*rdc*sint*sinp+energy_momentum[2][2]*rdc*sint*cosp;
	Tzphi=-energy_momentum[1][3]*rdc*sint*sinp+energy_momentum[2][3]*rdc*sint*cosp;

	rhoJl = (1./lapse)*(-Ttphi+shiftx*Txphi+shifty*Typhi+shiftz*Tzphi);
	...
	for (vol=0; vol<intvolumes; vol++)
	{
		...
		localVolAngularJ[vol] += weightL*AngularJL;
	    }
        }
 ...
 if (verbose) {
	CCTK_VInfo(CCTK_THORNSTRING,"After local integration ....");
        if (intvolumes>0)
	    CCTK_VInfo(CCTK_THORNSTRING,"      Integrated volume masses are (%g,%g,%g,%g,%g,%g,%g,%g,%g,%g)",localVolMass[0],
		localVolMass[1], localVolMass[2], localVolMass[3], localVolMass[4],localVolAngularJ[0],localVolAngularJ[1],localVolAngularJ[2],localVolAngularJ[3],localVolAngularJ[4]);
 }
...
void TotalRestMass_output(CCTK_ARGUMENTS)
{
  ...
  if ( (myproc == 0) && do_calc_mass )
  {
    ...
    if (file_created<=0) {
      fprintf(file,"# Total enerngy and angular momentum totals\n");
      fprintf(file,"# gnuplot column index:\n");
      fprintf(file,"# 1:it 2:t \n");
      for (vol=0;vol<intvolumes;vol++) 
      {
	  fprintf(file,"#\t( r=%g ) %d:Energy %d:AngularJ\n",int_wi_rad[vol],2*vol+3,2*vol+4);
      }
      fprintf(file,"\n");
    }
    ...
    for (vol=0;vol<intvolumes;vol++)
    {
	fprintf(file,addformat,VolumeMass[vol],Volume_AngulatJ[vol]);
    }
    fprintf(file,"\n");
