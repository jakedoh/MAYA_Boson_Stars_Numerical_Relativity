...
//      we rescale the momentum
    ...
    double rhoSc = 0;
    double KK = 0;
    double eta = (-3*C3 + sqrt(9*C3*C3 - 32*C2))/8.0;
    double C0 = -(C2*eta^2 + C3*eta^3 + eta^4);
    double lambdap = V0 / C0	;
		

    if (CCTK_Equals(potential_type,"Quadratic")) {
       VV = 0.5 * m0*m0 * ini_phia*ini_phia;
//       CCTK_VInfo (CCTK_THORNSTRING, "Value of quadratic potential  is %g",VV);
    }
    else if (CCTK_Equals(potential_type,"Fourth")) {
       VV = 0.5 * m0*m0 * ini_phia*ini_phia +  0.25*lamda0*pow(ini_phia,4);
//       CCTK_VInfo (CCTK_THORNSTRING, "Value of Fourth potential  is %g",VV);
    }
    else if (CCTK_Equals(potential_type,"Fourthshifted")){
       VV = V0 + 0.25 * lamda0 * pow(ini_phia*ini_phia - phi00*phi00, 2 );
//       CCTK_VInfo (CCTK_THORNSTRING, "Value of Pablo's potential  is %g",VV);
    }
    else if (CCTK_Equals(potential_type,"Pablo")){
       VV = lambdap * (C0 + C2*ini_phia*ini_phia + C3*ini_phia*ini_phia*ini_phia + ini_phia*ini_phia*ini_phia*ini_phia);
//       CCTK_VInfo (CCTK_THORNSTRING, "Jake's first scalar potential",VV);
    }

    rhoSc = 0.5 * pow(ini_pia,2) + VV;
    KK =-1. * Ksign * pow(24.*pi*rhoSc,0.5);
    kxx[ind] = kxx[ind] + gxx[ind]*KK/3.;
    kxy[ind] = kxy[ind] + gxy[ind]*KK/3.;
    kxz[ind] = kxz[ind] + gxz[ind]*KK/3.;
    kyy[ind] = kyy[ind] + gyy[ind]*KK/3.;
    kyz[ind] = kyz[ind] + gyz[ind]*KK/3.;                                             	   kzz[ind] = kzz[ind] + gzz[ind]*KK/3.;
