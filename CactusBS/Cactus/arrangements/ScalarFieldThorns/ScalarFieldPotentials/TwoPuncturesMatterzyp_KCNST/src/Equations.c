...
BY_KKofxyz (double x, double y, double z, double time)
{
  ...
  {
    for (j = 0; j < 3; j++)
    {				// eq. (55) of arxiv:1308.3502, Bowen-York-Curvature eqs. (12.38) and (12.39) book: Tomas W. Baumgarte & Stuart L Shapiro:
      Aij =
		 ...
		 double y, double z, derivs U, double *values, double time)
{
  ...
  // multi scalar fields //old: Damour&Farese PRL_70_2220, Sotani&Kokkotas PRD_70_084026
  if(scalar_field) {
double scalarphia, grad_scalarphiax, grad_scalarphiay, grad_scalarphiaz, scalarpia, grad_scalarphia_sq;
double scalarphib, grad_scalarphibx, grad_scalarphiby, grad_scalarphibz, scalarpib, grad_scalarphib_sq;
double VV, vpotential;
    // caclInitialScalarFields expects the coordinates of the simulation domain
    calcInitialScalarFields(x - move_origin_x, y, z, &scalarphia, &grad_scalarphiax, &grad_scalarphiay, &grad_scalarphiaz, &scalarpia, &scalarphib, &grad_scalarphibx, &grad_scalarphiby, &grad_scalarphibz, &scalarpib, &VV);
    //calcInitialScalarFields(x - par_b, y, z, &scalarphia, &grad_scalarphia[0], &scalarpia, &scalarphib, &grad_scalarphib[0], &scalarpib, &VV);
    grad_scalarphia_sq = grad_scalarphiax*grad_scalarphiax+grad_scalarphiay*grad_scalarphiay+grad_scalarphiaz*grad_scalarphiaz;
    grad_scalarphib_sq = grad_scalarphibx*grad_scalarphibx+grad_scalarphiby*grad_scalarphiby+grad_scalarphibz*grad_scalarphibz;
       
  if(scalar_backreaction_to_geometry) {
    /*values[0] += Pi* ( (scalarpia*scalarpia + scalarpib*scalarpib) *psi5 + (grad_scalarphia_sq + grad_scalarphib_sq) * psi + 2.0 * vpotential * psi5);}*/
    values[0] += Pi* ( (scalarpia*scalarpia + scalarpib*scalarpib) / psi7 + (grad_scalarphia_sq + grad_scalarphib_sq) * psi + 2.0 * VV * psi5);}
    //values[0] += Pi *((gvphiaa*scalarpia*scalarpia + gvphibb*scalarpib*scalarpib + 2*gvphiab*scalarpia*scalarpib) * psi5 + ( gvphiaa*grad_scalarphia_sq + gvphibb*grad_scalarphib_sq + gvphiab*grad_scalarphiab_sq + gvphiab*grad_scalarphiba_sq)*psi +2.0* psi5 * VV);
  else
  {values[0] +=0;}
  }
}
	      ...
	      double y, double z, derivs dU, derivs U, double *values, double time)
{
  ...
  // multi scalar fields in PRD_97_023526 
  if(scalar_field) {
double scalarphia, grad_scalarphiax, grad_scalarphiay, grad_scalarphiaz, scalarpia, grad_scalarphia_sq;
double scalarphib, grad_scalarphibx, grad_scalarphiby, grad_scalarphibz, scalarpib, grad_scalarphib_sq;
double VV, vpotential;
...
//values[0] += Pi* ( (gvphiaa*scalarpia*scalarpia + gvphibb*scalarpib*scalarpib + 2*gvphiab*scalarpia*scalarpib) / psi7 + (gvphiaa*grad_scalarphia_sq + gvphibb*grad_scalarphib_sq + gvphiab*grad_scalarphiab_sq + gvphiab*grad_scalarphiba_sq) * psi + 2.0 * VV * psi5);
    
    //values[0] += -7*Pi * (gvphiaa*scalarpia*scalarpia + gvphibb*scalarpib*scalarpib + 2*gvphiab*scalarpia*scalarpib) / psi8 * dU.d0[0] + Pi * (gvphiaa*grad_scalarphia_sq + gvphibb*grad_scalarphib_sq + gvphiab*grad_scalarphiab_sq + gvphiab*grad_scalarphiba_sq) * dU.d0[0] + 10.0 * Pi * VV * psi4 * dU.d0[0] - 8.0 * Pi * VV * psi4 * psi;

  if(scalar_backreaction_to_geometry) {
    /*values[0] += -7*Pi * (scalarpia*scalarpia + scalarpib*scalarpib) / psi8 * dU.d0[0] + Pi * (grad_scalarphia_sq + grad_scalarphib_sq) * dU.d0[0] + 2 * Pi * dvpotential * dU.d0[0];}*/
    /*values[0] += 5*Pi * (scalarpia*scalarpia + scalarpib*scalarpib) * psi4 * dU.d0[0] + Pi * (grad_scalarphia_sq + grad_scalarphib_sq) * dU.d0[0] + 10.0 * Pi * VV * psi4 * dU.d0[0];}*/
    values[0] += -7*Pi * (scalarpia*scalarpia + scalarpib*scalarpib) / psi8 * dU.d0[0] + Pi * (grad_scalarphia_sq + grad_scalarphib_sq) * dU.d0[0] + 10.0 * Pi * VV * psi4 * dU.d0[0];}
   else
{    values[0] += 0;}
