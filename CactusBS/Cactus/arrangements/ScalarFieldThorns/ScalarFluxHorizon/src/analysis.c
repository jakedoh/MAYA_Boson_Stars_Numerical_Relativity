...
static int ScalarFluxHorizon_write_data(CCTK_ARGUMENTS,
                                   CCTK_INT det,
                                   CCTK_REAL rad,
                                   CCTK_REAL dEdt,
...
void ScalarFluxHorizon_Compute_EPRad (CCTK_ARGUMENTS)
{
  ...
  if (myproc==0) {
    CCTK_INT ierr=ScalarFluxHorizon_write_data(CCTK_PASS_CTOC,
                                          det,
                                          rad,
                                          dEdt,
				  ...
				  )
{
  ...
  if (*ScalarFluxHorizon_files_exist>0) {
    sprintf(fmode,"a");
  }
  else {
    sprintf(fmode,"w");
  ...
  if (name_detector_by_radius) {
      sprintf(varname, "ScalarFluxHorizon_r%.2f",rad);
  }
  else {
      sprintf(varname,"ScalarFluxHorizon_%d",det);
  }
  ...
  if (*ScalarFluxHorizon_files_exist<=0) {
    fprintf(file,"# Scalar Horizon Flux  Quantities\n");
    fprintf(file,"# det no.=%d",det);
    fprintf(file,"# gnuplot column index:\n");
    fprintf(file,"# 1:t 2:E 3:Px 4:Py 5:Pz 6:Jz 7:dE/dt 8:dPx/dt 9:dPy/dt 10:dPz/dt 11:dJz/dt 12:r\n");
  }
  ...
  fprintf(file, format_str_real, cctk_time, energy[det],
          ...
          dEdt, dPxdt, dPydt, dPzdt, dJzdt, rad);

  fclose(file); 
  free(filename); 
  if (det==number_of_detectors-1) { // we have written all detectors
    *ScalarFluxHorizon_files_exist=1;
  }
  return 1;
}
