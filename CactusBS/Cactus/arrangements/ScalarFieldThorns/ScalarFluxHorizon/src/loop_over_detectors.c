...
int ScalarFluxHorizon_setup_out_every_det(CCTK_ARGUMENTS)
{
  ...
  for (int i=0;i<number_of_detectors;i++) {
    if (out_every_det[i]==-1) {
      ...
      ierr=CCTK_ParameterSet(paramstr,"ScalarFluxHorizon",default_out_every);
      if (ierr<0) {
...
void ScalarFluxHorizon_loop_over_detectors (CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  CCTK_INT ierr=ScalarFluxHorizon_setup_out_every_det(CCTK_PASS_CTOC);
  if (ierr<0) {
  ...
  for (int i=0;i<number_of_detectors;i++) {
    ...
    *nphi=maxnphi;
    ...
    ScalarFluxHorizon_ProjectSphere(CCTK_PASS_CTOC);

    if (verbose>2) {
      CCTK_INFO("calling Compute_EPRad");
    }

    ScalarFluxHorizon_Compute_EPRad(CCTK_PASS_CTOC);
