...
//BK: Compute the Hamiltonian constraint violation - Equation 62 from the Michael's paper
{
  ...
  double Aij[6], psiguess, junk, junk3[3], Phi[3], varphi, confPi[3], confPisqr, V, gradPhisqr;

  getSourceInfo(x-move_origin_x, y, z, junk3, &junk, &psiguess, Aij);
  getInitialPhi(x-move_origin_x, y, z, Phi);
  getInitialConformalPi(x-move_origin_x, y, z, confPi);

  varphi = sqrt(Phi[0]*Phi[0] + Phi[1]*Phi[1]);

  confPisqr = confPi[0]*confPi[0] + confPi[1]*confPi[1];
  gradPhisqr = getGradPhisqr(x-move_origin_x, y, z);;
  V = SQR(varphi);

  double psi = psiguess + U.d0[0];
  double AA = traceSquare(Aij);
  double lhs = U.d11[0] + U.d22[0] + U.d33[0] + 0.125 * AA /( QUAD(psi)*CUBE(psi)); 
  double rhs = -Pi* ( confPisqr / (QUAD(psi)*CUBE(psi)) + gradPhisqr * psi + V * QUAD(psi)*psi);
  values[0]=lhs-rhs;
	      ...
	      double y, double z, derivs dU, derivs U, double *values, double time)
{
  ...
  double rhs = -Pi* (-7*confPisqr/QUAD(psi)/QUAD(psi)*dU.d0[0] + gradPhisqr*dU.d0[0] + 5*V*QUAD(psi)*dU.d0[0]);
  values[0] = lhs - rhs;
}
...
void makeInitialGuess_Psi (double x, double y, double z, double *u, double time)
{
  ...
  getSourceInfo(x-move_origin_x, y, z, junk3, &psi, &psiguess, junk6);
  u[0] = psi-psiguess;
}
