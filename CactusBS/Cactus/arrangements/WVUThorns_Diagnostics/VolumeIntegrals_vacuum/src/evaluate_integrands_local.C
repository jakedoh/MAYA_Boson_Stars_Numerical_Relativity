...
void ComputeIntegrand(CCTK_ARGUMENTS)
{
...
#pragma omp parallel for
    for (int k=0;k<cctk_lsh[2];k++) for (int j=0;j<cctk_lsh[1];j++) for (int i=0;i<cctk_lsh[0];i++) { int index = CCTK_GFINDEX3D(cctkGH,i,j,k);
	  L2_integrand(VolIntegrand1, index,H,x,y,z); 
	  L2_integrand(VolIntegrand2, index,M1,x,y,z); 
	  L2_integrand(VolIntegrand3, index,M2,x,y,z); 
	  L2_integrand(VolIntegrand4, index,M3,x,y,z); } 
  } else if(CCTK_EQUALS(Integration_quantity_keyword[which_integral],"centeroflapse")) { 
#pragma omp parallel for
    for (int k=0;k<cctk_lsh[2];k++) for (int j=0;j<cctk_lsh[1];j++) for (int i=0;i<cctk_lsh[0];i++) { int index = CCTK_GFINDEX3D(cctkGH,i,j,k);
	  CoL_integrand(VolIntegrand1,VolIntegrand2,VolIntegrand3,VolIntegrand4, index,alp,x,y,z); }
