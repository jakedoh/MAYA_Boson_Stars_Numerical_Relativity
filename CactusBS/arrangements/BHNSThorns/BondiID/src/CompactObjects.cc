...
double BondiSource::getNonFlatPsi(double xx, double yy, double zz)
{
...
BondiSource::BondiSource(int idx) : CompactSource(idx)
{
	DECLARE_CCTK_PARAMETERS;
...
VectorAlgebra::Vector BondiSource::getJhat(double xx, double yy, double zz)
{
...
VectorAlgebra::LinearOperator BondiSource::getAij(double xx, double yy, double zz)
{
	DECLARE_CCTK_PARAMETERS;
	VectorAlgebra::Vector rr = getRelativeVector(xx, yy, zz);
	double rmag = smoothR(rr.getMagnitude());
...
BlackHole::BlackHole(int idx) : BondiSource(idx)
{
	DECLARE_CCTK_PARAMETERS;
...
double BondiSource::radialIntegrate(double (BondiSource::*dFdr)(double), double rmin, double rmax)
{
...
double BondiSource::simpsonIntegrate(double (BondiSource::*dFdr)(double), double rmin, double rmid, double rmax, double fmin, double fmid, double fmax, int depth)
{
	DECLARE_CCTK_PARAMETERS;
...
double TOVStar::getMomentumWeight(double rr)
{
	...
	return radialIntegrate(&BondiSource::getMomentumWeightDeriv, 0., realrad);
	*/
...
double TOVStar::getPointSourceDeviation(double rr)
{
	/*
	double realrad = rr > radius ? radius : rr;
	return radialIntegrate(&BondiSource::getPSDDeriv, 0., realrad);
	*/
...
double TOVStar::getSpinWeight(double rr)
{
	/*
	double realrad = rr > radius ? radius : rr;
	return radialIntegrate(&BondiSource::getSpinWeightDeriv, 0., realrad);
	*/
