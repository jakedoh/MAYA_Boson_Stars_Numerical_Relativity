...
CCTK_REAL bowenid_getLorentzFactor(CCTK_REAL x, CCTK_REAL y, CCTK_REAL z)
{
	...
	for(auto it = sources.begin(); it != sources.end(); ++it)
	{
		res_src = (*it)->getLorentzFac(x, y, z);	
		if (res_src != 1.)
		{    res = res_src;
		}
	}
...
CCTK_REAL bowenid_getConfDensity(CCTK_REAL x, CCTK_REAL y, CCTK_REAL z)
{
	...
	for(auto it = sources.begin(); it != sources.end(); ++it)
	{
		res += (*it)->getConfDensity(x, y, z);	
	}
	return res;
}

extern "C" 
CCTK_REAL bowenid_getConfPressure(CCTK_REAL x, CCTK_REAL y, CCTK_REAL z)
{
	CCTK_REAL res = 0.;
	for(auto it = sources.begin(); it != sources.end(); ++it)
	{
		res += (*it)->getConfPressure(x, y, z);	
	}
	return res;
}
