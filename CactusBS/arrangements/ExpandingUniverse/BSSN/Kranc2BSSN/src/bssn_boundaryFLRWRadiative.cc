...
extern "C" void bssn_boundaryFLRWRadiative_SelectBCs(CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  
  if (cctk_iteration % bssn_boundaryFLRWRadiative_calc_every != bssn_boundaryFLRWRadiative_calc_offset)
    return;
  ...
  ierr = Boundary_SelectGroupForBC(cctkGH, CCTK_ALL_FACES, GetBoundaryWidth(cctkGH), -1 /* no table */, "Kranc2BSSN::alpha_grouprhs","flat");
  if (ierr < 0)
    CCTK_WARN(CCTK_WARN_ALERT, "Failed to register flat BC for Kranc2BSSN::alpha_grouprhs.");
...
static void bssn_boundaryFLRWRadiative_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  #pragma omp parallel
  CCTK_LOOP3(bssn_boundaryFLRWRadiative,
  ...
  CCTK_ENDLOOP3(bssn_boundaryFLRWRadiative);
}
extern "C" void bssn_boundaryFLRWRadiative(CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Entering bssn_boundaryFLRWRadiative_Body");
  }
  if (cctk_iteration % bssn_boundaryFLRWRadiative_calc_every != bssn_boundaryFLRWRadiative_calc_offset)
  {
    return;
  ...
  AssertGroupStorage(cctkGH, "bssn_boundaryFLRWRadiative", 18, groups);
  
  switch (constraintsfdorder)
  {
    case 2:
    {
      EnsureStencilFits(cctkGH, "bssn_boundaryFLRWRadiative", 2, 2, 2);
      break;
    }
  ...
  LoopOverBoundary(cctkGH, bssn_boundaryFLRWRadiative_Body);
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Leaving bssn_boundaryFLRWRadiative_Body");
  }
}
