...
extern "C" void bssn_evolve_metric_4thcentred_SelectBCs(CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  
  if (cctk_iteration % bssn_evolve_metric_4thcentred_calc_every != bssn_evolve_metric_4thcentred_calc_offset)
    return;
  ...
  if (ierr < 0)
    CCTK_WARN(CCTK_WARN_ALERT, "Failed to register flat BC for Kranc2BSSN::h_grouprhs.");
  ierr = Boundary_SelectGroupForBC(cctkGH, CCTK_ALL_FACES, GetBoundaryWidth(cctkGH), -1 /* no table */, "Kranc2BSSN::phi_grouprhs","flat");
  if (ierr < 0)
    CCTK_WARN(CCTK_WARN_ALERT, "Failed to register flat BC for Kranc2BSSN::phi_grouprhs.");
  return;
}

static void bssn_evolve_metric_4thcentred_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  #pragma omp parallel
  CCTK_LOOP3(bssn_evolve_metric_4thcentred,
  ...
  CCTK_ENDLOOP3(bssn_evolve_metric_4thcentred);
}
extern "C" void bssn_evolve_metric_4thcentred(CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Entering bssn_evolve_metric_4thcentred_Body");
  }
  if (cctk_iteration % bssn_evolve_metric_4thcentred_calc_every != bssn_evolve_metric_4thcentred_calc_offset)
  {
    return;
  ...
  AssertGroupStorage(cctkGH, "bssn_evolve_metric_4thcentred", 8, groups);
  
  switch (constraintsfdorder)
  {
    case 2:
    {
      EnsureStencilFits(cctkGH, "bssn_evolve_metric_4thcentred", 2, 2, 2);
      break;
    }
  ...
  LoopOverInterior(cctkGH, bssn_evolve_metric_4thcentred_Body);
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Leaving bssn_evolve_metric_4thcentred_Body");
  }
}
