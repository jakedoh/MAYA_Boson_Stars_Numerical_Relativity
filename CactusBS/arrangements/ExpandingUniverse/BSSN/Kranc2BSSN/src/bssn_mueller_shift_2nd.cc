...
extern "C" void bssn_mueller_shift_2nd_SelectBCs(CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  
  if (cctk_iteration % bssn_mueller_shift_2nd_calc_every != bssn_mueller_shift_2nd_calc_offset)
    return;
...
static void bssn_mueller_shift_2nd_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  #pragma omp parallel
  CCTK_LOOP3(bssn_mueller_shift_2nd,
    ...
    cctk_ash[0],cctk_ash[1],cctk_ash[2])
  {
    ...
    CCTK_REAL muellerTop CCTK_ATTRIBUTE_UNUSED = 
      2*(PDonesided2nd1(&phi[index])*(PDonesided2nd2(&phi[index])*hInv12 + 
      PDonesided2nd3(&phi[index])*hInv13) + 
      PDonesided2nd2(&phi[index])*PDonesided2nd3(&phi[index])*hInv23) + 
      hInv11*pow(PDonesided2nd1(&phi[index]),2) + 
      hInv22*pow(PDonesided2nd2(&phi[index]),2) + 
      hInv33*pow(PDonesided2nd3(&phi[index]),2);
  ...
  CCTK_ENDLOOP3(bssn_mueller_shift_2nd);
}
extern "C" void bssn_mueller_shift_2nd(CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Entering bssn_mueller_shift_2nd_Body");
  }
  if (cctk_iteration % bssn_mueller_shift_2nd_calc_every != bssn_mueller_shift_2nd_calc_offset)
  {
    return;
  ...
  AssertGroupStorage(cctkGH, "bssn_mueller_shift_2nd", 9, groups);
  
  switch (constraintsfdorder)
  {
    case 2:
    {
      EnsureStencilFits(cctkGH, "bssn_mueller_shift_2nd", 2, 2, 2);
      break;
    }
  ...
  LoopOverInterior(cctkGH, bssn_mueller_shift_2nd_Body);
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Leaving bssn_mueller_shift_2nd_Body");
  }
}
