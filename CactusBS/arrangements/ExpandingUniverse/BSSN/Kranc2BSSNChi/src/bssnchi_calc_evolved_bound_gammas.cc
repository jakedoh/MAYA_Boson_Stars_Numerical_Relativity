...
extern "C" void bssnchi_calc_evolved_bound_gammas_SelectBCs(CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  
  if (cctk_iteration % bssnchi_calc_evolved_bound_gammas_calc_every != bssnchi_calc_evolved_bound_gammas_calc_offset)
    return;
  CCTK_INT ierr CCTK_ATTRIBUTE_UNUSED = 0;
  ierr = Boundary_SelectGroupForBC(cctkGH, CCTK_ALL_FACES, GetBoundaryWidth(cctkGH), -1 /* no table */, "Kranc2BSSNChi::Gam_group","flat");
  if (ierr < 0)
    CCTK_WARN(CCTK_WARN_ALERT, "Failed to register flat BC for Kranc2BSSNChi::Gam_group.");
  return;
}

static void bssnchi_calc_evolved_bound_gammas_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  #pragma omp parallel
  CCTK_LOOP3(bssnchi_calc_evolved_bound_gammas,
  ...
  CCTK_ENDLOOP3(bssnchi_calc_evolved_bound_gammas);
}
extern "C" void bssnchi_calc_evolved_bound_gammas(CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Entering bssnchi_calc_evolved_bound_gammas_Body");
  }
  if (cctk_iteration % bssnchi_calc_evolved_bound_gammas_calc_every != bssnchi_calc_evolved_bound_gammas_calc_offset)
  {
    return;
  ...
  AssertGroupStorage(cctkGH, "bssnchi_calc_evolved_bound_gammas", 3, groups);
  
  switch (constraintsfdorder)
  {
    case 2:
    {
      EnsureStencilFits(cctkGH, "bssnchi_calc_evolved_bound_gammas", 2, 2, 2);
      break;
    }
  ...
  LoopOverBoundary(cctkGH, bssnchi_calc_evolved_bound_gammas_Body);
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Leaving bssnchi_calc_evolved_bound_gammas_Body");
  }
}
