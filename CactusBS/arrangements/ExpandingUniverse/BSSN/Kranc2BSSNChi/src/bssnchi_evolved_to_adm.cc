...
static void bssnchi_evolved_to_adm_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
  ...
  #pragma omp parallel
  CCTK_LOOP3(bssnchi_evolved_to_adm,
  ...
  CCTK_ENDLOOP3(bssnchi_evolved_to_adm);
}
extern "C" void bssnchi_evolved_to_adm(CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Entering bssnchi_evolved_to_adm_Body");
  }
  if (cctk_iteration % bssnchi_evolved_to_adm_calc_every != bssnchi_evolved_to_adm_calc_offset)
  {
    return;
  }
  ...
  AssertGroupStorage(cctkGH, "bssnchi_evolved_to_adm", 10, groups);
  ...
  LoopOverEverything(cctkGH, bssnchi_evolved_to_adm_Body);
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Leaving bssnchi_evolved_to_adm_Body");
  }
}
