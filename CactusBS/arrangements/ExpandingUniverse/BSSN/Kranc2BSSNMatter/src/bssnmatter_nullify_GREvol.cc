...
static void bssnmatter_nullify_GREvol_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
  ...
  #pragma omp parallel
  CCTK_LOOP3(bssnmatter_nullify_GREvol,
  ...
  CCTK_ENDLOOP3(bssnmatter_nullify_GREvol);
}
extern "C" void bssnmatter_nullify_GREvol(CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Entering bssnmatter_nullify_GREvol_Body");
  }
  if (cctk_iteration % bssnmatter_nullify_GREvol_calc_every != bssnmatter_nullify_GREvol_calc_offset)
  {
    return;
  }
  ...
  AssertGroupStorage(cctkGH, "bssnmatter_nullify_GREvol", 8, groups);
  ...
  LoopOverEverything(cctkGH, bssnmatter_nullify_GREvol_Body);
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Leaving bssnmatter_nullify_GREvol_Body");
  }
}
