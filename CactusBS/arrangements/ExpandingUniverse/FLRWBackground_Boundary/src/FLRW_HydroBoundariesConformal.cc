...
extern "C" void FLRWBackground_HydroBoundaryConformal(CCTK_ARGUMENTS) {
  ...
  for(int which_bdry_pt=0;which_bdry_pt<cctk_nghostzones[0];which_bdry_pt++) {
    ...
    if(cctk_bbox[1]) { XMAX_OB(eps,imax,epst); XMAX_OB(rho,imax,rhot); XMAX_OB(press,imax,presst); XMAX_OB(velx,imax,velt); XMAX_OB(vely,imax,velt);XMAX_OB(velz,imax,velt);}
    if(cctk_bbox[3]) { YMAX_OB(eps,jmax,epst); YMAX_OB(rho,jmax,rhot); YMAX_OB(press,jmax,presst); YMAX_OB(velx,jmax,velt);YMAX_OB(vely,jmax,velt);YMAX_OB(velz,jmax,velt);}
    if(cctk_bbox[5]) { ZMAX_OB(eps,kmax,epst); ZMAX_OB(rho,kmax,rhot); ZMAX_OB(press,kmax,presst); ZMAX_OB(velx,kmax,velt);ZMAX_OB(vely,kmax,velt);ZMAX_OB(velz,kmax,velt);}

    if((cctk_bbox[0]) && NotSymx) { XMIN_OB(eps,imin,epst); XMIN_OB(rho,imin,rhot); XMIN_OB(press,imin,presst); XMIN_OB(velx,imin,velt);XMIN_OB(vely,imin,velt);XMIN_OB(velz,imin,velt);}
    if((cctk_bbox[2]) && NotSymy) { YMIN_OB(eps,jmin,epst); YMIN_OB(rho,jmin,rhot); YMIN_OB(press,jmin,presst); YMIN_OB(velx,jmin,velt);YMIN_OB(vely,jmin,velt);YMIN_OB(velz,jmin,velt);}
    if((cctk_bbox[4]) && NotSymz) { ZMIN_OB(eps,kmin,epst); ZMIN_OB(rho,kmin,rhot); ZMIN_OB(press,kmin,presst); ZMIN_OB(velx,kmin,velt);ZMIN_OB(vely,kmin,velt);ZMIN_OB(velz,kmin,velt);}
  }
}
