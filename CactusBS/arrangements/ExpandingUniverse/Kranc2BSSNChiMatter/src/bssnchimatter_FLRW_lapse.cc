...
extern "C" void bssnchimatter_FLRW_lapse_SelectBCs(CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  
  if (cctk_iteration % bssnchimatter_FLRW_lapse_calc_every != bssnchimatter_FLRW_lapse_calc_offset)
    return;
...
static void bssnchimatter_FLRW_lapse_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  #pragma omp parallel
  CCTK_LOOP3(bssnchimatter_FLRW_lapse,
  ...
  CCTK_ENDLOOP3(bssnchimatter_FLRW_lapse);
}
extern "C" void bssnchimatter_FLRW_lapse(CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Entering bssnchimatter_FLRW_lapse_Body");
  }
  if (cctk_iteration % bssnchimatter_FLRW_lapse_calc_every != bssnchimatter_FLRW_lapse_calc_offset)
  {
    return;
  ...
  AssertGroupStorage(cctkGH, "bssnchimatter_FLRW_lapse", 5, groups);
  
  switch (constraintsfdorder)
  {
    case 2:
    {
      EnsureStencilFits(cctkGH, "bssnchimatter_FLRW_lapse", 3, 3, 3);
      break;
    }
  ...
  LoopOverInterior(cctkGH, bssnchimatter_FLRW_lapse_Body);
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Leaving bssnchimatter_FLRW_lapse_Body");
  }
}
