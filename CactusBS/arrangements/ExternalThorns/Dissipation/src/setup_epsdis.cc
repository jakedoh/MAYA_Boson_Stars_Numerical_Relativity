/* $Header: /numrelcvs/AEIThorns/Dissipation/src/setup_epsdis.c,v 1.5 2006/04/11 09:58:46 knarf Exp $ */
...
extern "C" void setup_epsdis (CCTK_ARGUMENTS)
{
  ...
  if (dissipation_per_level>0) {
    for (i=0;i<ni*nj*nk;i++) {
      epsdisA[i]=epsdis_lev[reflevel];
    }
  }
  else {
    for (i=0;i<ni*nj*nk;i++) {
