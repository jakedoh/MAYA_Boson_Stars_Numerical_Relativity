...
extern "C" void Meudon_Bin_NS_Scotch_Hybrid_initialise(CCTK_ARGUMENTS) {
  DECLARE_CCTK_ARGUMENTS;
...
#pragma omp parallel for
    for (int i = 0; i < npoints; ++i) {
      ...
      if (CCTK_EQUALS(initial_lapse, "Meudon_Bin_NS_Scotch_Hybrid")) {
        alp[i] = bin_ns.nnn[i];
      }

      if (CCTK_EQUALS(initial_shift, "Meudon_Bin_NS_Scotch_Hybrid")) {
        betax[i] = -bin_ns.beta_x[i];
        betay[i] = -bin_ns.beta_y[i];
        betaz[i] = -bin_ns.beta_z[i];
      }

      if (CCTK_EQUALS(initial_data, "Meudon_Bin_NS_Scotch_Hybrid")) {
        gxx[i] = bin_ns.g_xx[i];
        gxy[i] = bin_ns.g_xy[i];
        gxz[i] = bin_ns.g_xz[i];
        gyy[i] = bin_ns.g_yy[i];
...

    {
      ...
      if (CCTK_EQUALS(initial_lapse, "Meudon_Bin_NS_Scotch_Hybrid")) {
        if (CCTK_EQUALS(initial_dtlapse, "Meudon_Bin_NS_Scotch_Hybrid")) {
      ...
      if (CCTK_EQUALS(initial_shift, "Meudon_Bin_NS_Scotch_Hybrid")) {
        if (CCTK_EQUALS(initial_dtshift, "Meudon_Bin_NS_Scotch_Hybrid")) {
