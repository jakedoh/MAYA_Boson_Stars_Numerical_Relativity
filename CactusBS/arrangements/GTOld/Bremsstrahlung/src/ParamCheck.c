...
//****** External Routines ******//
//*******************************//
void Brems_Startup(CCTK_ARGUMENTS);
...
void Brems_Startup(CCTK_ARGUMENTS)
{
  ...
  for (int vol=0; vol<intvolumes; vol++) {
	...
	if (ierr < 0)
	{
		CCTK_PARAMWARN("Bremsstrahlung: Error in checking weight variable.");
	}
        ...
        { 
                ...
                if (sf_error)
		  CCTK_PARAMWARN("Bremsstrahlung: Error in checking spherical surfaces.");
        }
  }
