...
namespace ExternalEtaBeta {


static void eta_beta_calc_etaBeta_exp_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
  ...
  #pragma omp parallel
  CCTK_LOOP3(eta_beta_calc_etaBeta_exp,
    ...
    cctk_ash[0],cctk_ash[1],cctk_ash[2])
  {
    ...
    CCTK_REAL Mtot CCTK_ATTRIBUTE_UNUSED = Mminus + Mplus;
    
    CCTK_REAL A CCTK_ATTRIBUTE_UNUSED = 2.*pow(Mtot,-1);
    
    CCTK_REAL C1 CCTK_ATTRIBUTE_UNUSED = -A + 1.*pow(Mplus,-1);
    
    CCTK_REAL C2 CCTK_ATTRIBUTE_UNUSED = -A + 1.*pow(Mminus,-1);
    
    CCTK_REAL xx1 CCTK_ATTRIBUTE_UNUSED = 0;
    
    CCTK_REAL yy1 CCTK_ATTRIBUTE_UNUSED = 0;
    
    CCTK_REAL zz1 CCTK_ATTRIBUTE_UNUSED = 0;
    
    CCTK_REAL xx2 CCTK_ATTRIBUTE_UNUSED = 0;
    
    CCTK_REAL yy2 CCTK_ATTRIBUTE_UNUSED = 0;
    
    CCTK_REAL zz2 CCTK_ATTRIBUTE_UNUSED = 0;
    
    CCTK_REAL r1hat CCTK_ATTRIBUTE_UNUSED = pow(pow(xL - xx1,2) + pow(yL - 
      yy1,2) + pow(zL - zz1,2),0.5)*pow(pow(xx1 - xx2,2) + pow(yy1 - yy2,2) + 
      pow(zz1 - zz2,2),-0.5);
    
    CCTK_REAL r2hat CCTK_ATTRIBUTE_UNUSED = pow(pow(xL - xx2,2) + pow(yL - 
      yy2,2) + pow(zL - zz2,2),0.5)*pow(pow(xx1 - xx2,2) + pow(yy1 - yy2,2) + 
      pow(zz1 - zz2,2),-0.5);
    
    CCTK_REAL exetaBetaL CCTK_ATTRIBUTE_UNUSED = A + 
      C1*exp(-(w1*pow(pow(r1hat,2),n))) + C2*exp(-(w2*pow(pow(r2hat,2),n)));
    /* Copy local copies back to grid functions */
    exetaBeta[index] = exetaBetaL;
  }
  CCTK_ENDLOOP3(eta_beta_calc_etaBeta_exp);
}
extern "C" void eta_beta_calc_etaBeta_exp(CCTK_ARGUMENTS)
{
  #ifdef DECLARE_CCTK_ARGUMENTS_eta_beta_calc_etaBeta_exp
  DECLARE_CCTK_ARGUMENTS_CHECKED(eta_beta_calc_etaBeta_exp);
  #else
  DECLARE_CCTK_ARGUMENTS;
  #endif
  DECLARE_CCTK_PARAMETERS;
  ...
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Entering eta_beta_calc_etaBeta_exp_Body");
  }
  if (cctk_iteration % eta_beta_calc_etaBeta_exp_calc_every != eta_beta_calc_etaBeta_exp_calc_offset)
  {
    return;
  }
  ...
  const char* const groups[] = {
    ...
    "grid::coordinates"};
  AssertGroupStorage(cctkGH, "eta_beta_calc_etaBeta_exp", 2, groups);
  
  
  LoopOverEverything(cctkGH, eta_beta_calc_etaBeta_exp_Body);
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Leaving eta_beta_calc_etaBeta_exp_Body");
  }
}
