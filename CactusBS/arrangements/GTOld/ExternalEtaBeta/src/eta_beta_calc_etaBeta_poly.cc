...
static void eta_beta_calc_etaBeta_poly_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
  ...
  #pragma omp parallel
  CCTK_LOOP3(eta_beta_calc_etaBeta_poly,
    ...
    cctk_ash[0],cctk_ash[1],cctk_ash[2])
  {
    ...
    CCTK_REAL exetaBetaL CCTK_ATTRIBUTE_UNUSED = A + C1*pow(1 + 
      w1*pow(pow(r1hat,2),n),-1) + C2*pow(1 + w2*pow(pow(r2hat,2),n),-1);
    /* Copy local copies back to grid functions */
    exetaBeta[index] = exetaBetaL;
  }
  CCTK_ENDLOOP3(eta_beta_calc_etaBeta_poly);
}
extern "C" void eta_beta_calc_etaBeta_poly(CCTK_ARGUMENTS)
{
  ...
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Entering eta_beta_calc_etaBeta_poly_Body");
  }
  if (cctk_iteration % eta_beta_calc_etaBeta_poly_calc_every != eta_beta_calc_etaBeta_poly_calc_offset)
  {
    return;
  }
  ...
  const char* const groups[] = {
    ...
    "grid::coordinates"};
  AssertGroupStorage(cctkGH, "eta_beta_calc_etaBeta_poly", 2, groups);
  
  
  LoopOverEverything(cctkGH, eta_beta_calc_etaBeta_poly_Body);
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Leaving eta_beta_calc_etaBeta_poly_Body");
  }
}
