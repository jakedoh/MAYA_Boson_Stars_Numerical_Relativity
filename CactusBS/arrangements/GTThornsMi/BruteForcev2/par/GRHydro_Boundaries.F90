...
! +++++++++++++=ESTO ES UN HACK A GRHydro QUITANDO LO DE DE DENTRO RECUPERAMOS
!++++++++++++++LA VERSION ORIGINAL++++++++++++++++++++++++++++++++++++++++++++
!!!custom boundaries if required
  if (CCTK_EQUALS(bound,"custom")) then
    call GRHydro_RegisterBoundariesWithMethod(CCTK_PASS_FTOF, "custom")
  end if
...
!++++++++++ESTE ES UN HACK A GRHydro hay que quitarlo si queremos la version Original
!!$None boundaries if required
  if (CCTK_EQUALS(bound,"custom")) then
    call GRHydro_SelectPrimitiveInitialGuessesBoundariesForMethod(CCTK_PASS_FTOF, "custom")
  end if
...
!!!++++++++++++++++++ESTES ES UN HACK A GRHydro quitalo si quieres tener la version orginal
!!$custom boundaries if required
  if (CCTK_EQUALS(bound,"custom")) then
    call GRHydro_SelectPrimitiveBoundariesForMethod(CCTK_ARGUMENTS, "custom")
  end if
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
!!!!ESTE ES UN HACK borralo si quieres la version original
  if (CCTK_EQUALS(bound,"custom")) then      
      ierr = ierr + Boundary_SelectGroupForBC(cctkGH, faces, GRHydro_stencil, -ione, &
         "GRHydro::GRHydro_atmosphere_mask_real", "custom")
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  end if
