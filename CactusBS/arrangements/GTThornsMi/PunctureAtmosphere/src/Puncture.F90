...
#define velz(i,j,k) vel(i,j,k,3)
  ...
  subroutine Puncture_Atmosphere(CCTK_ARGUMENTS)
...
!==============>>>>>>> ===================================================
    if (CCTK_EQUALS(parallel, "no")) then
         vel(i,j,k,1)= v_inf_x/(sqrt(gxx(i,j,k)+gzz(i,j,k)+2.0d0*gxz(i,j,k)))
         vel(i,j,k,2)= v_inf_y/(sqrt(gyy(i,j,k)))
         vel(i,j,k,3)= v_inf_z/(sqrt(gxx(i,j,k)+gzz(i,j,k)+2.0d0*gxz(i,j,k)))
      
    else if (CCTK_EQUALS(parallel, "yes")) then
         vel(i,j,k,1)= v_inf_x/(sqrt(gxx(i,j,k)))
         vel(i,j,k,2)= v_inf_y/(sqrt(gyy(i,j,k)))
         vel(i,j,k,3)= v_inf_z/(sqrt(gzz(i,j,k)))
    end if
...
!    end if 
  ...
  end subroutine Puncture_Atmosphere
