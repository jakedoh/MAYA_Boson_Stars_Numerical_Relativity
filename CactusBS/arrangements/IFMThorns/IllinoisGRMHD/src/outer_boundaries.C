...
#define XMAX_OB_SIMPLE_COPY(FUNC,imax) for(int k=0;k<cctk_lsh[2];k++) for(int j=0;j<cctk_lsh[1];j++) FUNC[IDX(imax,j,k)] = FUNC[IDX(imax-1,j,k)];
   // j=0;j<cctk_lsh[1];j++) FUNC[IDX(imax,j,k)] = FUNC[IDX(imax-1,j,k)];
...
extern "C" void IllinoisGRMHD_outer_boundaries_on_P_rho_b_vx_vy_vz(CCTK_ARGUMENTS) {
  ...
  if(CCTK_EQUALS(Matter_BC,"frozen")) {
      printf("Appltying, FROZEN boundaries!");
    
    
    
      return;}
    
  else (CCTK_EQUALS(Matter_BC,"outflow")); {
      printf("Applyting OUTFLOW boundaries!");
...
//      return;}
  ...
  if(cctk_iteration==0 || levelnumber!=0)
      printf("NO INITIAL DATA BOUNDARY CONDITIONS !!!!!!!!!!");
