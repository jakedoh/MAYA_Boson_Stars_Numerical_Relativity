...
subroutine Whisky_FSAlpha(CCTK_ARGUMENTS)
  ...
  if (flux_direction == 1) then
    ...
    !$OMP uxx, uxy, uxz, uyy, uyz, uzz, det, psi4pt, beta, &
    !$OMP lambda) &
    !$OMP REDUCTION (MAX : alpha1_local, alpha2_local, alpha3_local, alpha4_local, &
    !$OMP alpha5_local)
    do k = 1, nz
      do j = 1, ny
        do i = 1, nx
...
subroutine Whisky_SplitFlux(CCTK_ARGUMENTS)
  ...
  if (flux_direction == 1) then
    !$OMP PARALLEL PRIVATE (i,j,k,upper,det,dummy,uxx, &
    !$OMP uxy, uxz, uyy, uyz, uzz, psi4pt)
    ...
    do k = whisky_stencil, cctk_lsh(3) - whisky_stencil + 2
      do j = whisky_stencil, cctk_lsh(2) - whisky_stencil + 2
        ...
        dummy = betax(:,j,k)
        do i = 1, cctk_lsh(1)
          det(i) = SPATIAL_DET(gxx(i,j,k),gxy(i,j,k),gxz(i,j,k),gyy(i,j,k),gyz(i,j,k),gzz(i,j,k))
        ...
        call Whisky_SplitFlux_1D(whisky_eos_handle, nx,&
             ...
             upper, det,&
             alp(:,j,k),dummy,&
             rho(:,j,k),velx(:,j,k),vely(:,j,k),velz(:,j,k),press(:,j,k),&
             w_lorentz(:,j,k),eps(:,j,k),&
             dens(:,j,k),sx(:,j,k),sy(:,j,k),sz(:,j,k),tau(:,j,k),&
    ...
    do k = whisky_stencil, cctk_lsh(3) - whisky_stencil + 2
      do i = whisky_stencil, cctk_lsh(1) - whisky_stencil + 2
        ...
        dummy = betay(i,:,k)
        do j = 1, cctk_lsh(2)
          det(j) = SPATIAL_DET(gxx(i,j,k),gxy(i,j,k),gxz(i,j,k),gyy(i,j,k),gyz(i,j,k),gzz(i,j,k))
        ...
        call Whisky_SplitFlux_1D(whisky_eos_handle, ny,&
             ...
             upper, det,&
             alp(i,:,k),dummy,&
             rho(i,:,k),vely(i,:,k),velz(i,:,k),velx(i,:,k),press(i,:,k),&
             w_lorentz(i,:,k),eps(i,:,k),&
             dens(i,:,k),sy(i,:,k),sz(i,:,k),sx(i,:,k),tau(i,:,k),&
    ...
    do j = whisky_stencil, cctk_lsh(2) - whisky_stencil + 2
      do i = whisky_stencil, cctk_lsh(1) - whisky_stencil + 2
        ...
        do k = 1, cctk_lsh(3)
          det(k) = SPATIAL_DET(gxx(i,j,k),gxy(i,j,k),gxz(i,j,k),gyy(i,j,k),gyz(i,j,k),gzz(i,j,k))
        ...
        call Whisky_SplitFlux_1D(whisky_eos_handle, nz,&
             ...
             upper, det,&
             alp(i,j,:),dummy,&
             rho(i,j,:),velz(i,j,:),velx(i,j,:),vely(i,j,:),press(i,j,:),&
             w_lorentz(i,j,:),eps(i,j,:),&
             dens(i,j,:),sz(i,j,:),sx(i,j,:),sy(i,j,:),tau(i,j,:),&
...
subroutine Whisky_SplitFlux_1D(handle, nx, &
     ...
     rho, velx, vely, velz, press, w_lorentz, eps, &
     dens, sx, sy, sz, tau, &
...
!!$    Calculate the maximum eigenvalue and put it here
    ...
    call eigenproblem_leftright(handle, &
         0.5d0 * (rho      (i) + rho      (i+1)), &
         0.5d0 * (velx     (i) + velx     (i+1)), &
         0.5d0 * (vely     (i) + vely     (i+1)), &
         0.5d0 * (velz     (i) + velz     (i+1)), &
         0.5d0 * (eps      (i) + eps      (i+1)), &
...
!!$    Initialize the pointwise fluxes temporarily into the minus
      ...
      call num_x_flux(dens(ll), sx(ll), sy(ll), sz(ll), tau(ll), &
           ...
           taufminus(ll), &
           velx(ll), press(ll), det(ll), alp(ll), beta(ll))
