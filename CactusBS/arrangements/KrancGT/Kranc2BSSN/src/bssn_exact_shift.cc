...
static void bssn_exact_shift_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
  ...
  CCTK_ENDLOOP3(bssn_exact_shift);
}
extern "C" void bssn_exact_shift(CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Entering bssn_exact_shift_Body");
  }
  if (cctk_iteration % bssn_exact_shift_calc_every != bssn_exact_shift_calc_offset)
  {
    return;
  }
  ...
  AssertGroupStorage(cctkGH, "bssn_exact_shift", 2, groups);
  ...
  LoopOverEverything(cctkGH, bssn_exact_shift_Body);
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Leaving bssn_exact_shift_Body");
  }
}
