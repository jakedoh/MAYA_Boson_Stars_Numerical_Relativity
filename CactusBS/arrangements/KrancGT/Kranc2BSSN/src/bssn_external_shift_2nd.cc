...
extern "C" void bssn_external_shift_2nd_SelectBCs(CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  
  if (cctk_iteration % bssn_external_shift_2nd_calc_every != bssn_external_shift_2nd_calc_offset)
    return;
...
static void bssn_external_shift_2nd_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  #pragma omp parallel
  CCTK_LOOP3(bssn_external_shift_2nd,
  ...
  CCTK_ENDLOOP3(bssn_external_shift_2nd);
}
extern "C" void bssn_external_shift_2nd(CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Entering bssn_external_shift_2nd_Body");
  }
  if (cctk_iteration % bssn_external_shift_2nd_calc_every != bssn_external_shift_2nd_calc_offset)
  {
    return;
  ...
  AssertGroupStorage(cctkGH, "bssn_external_shift_2nd", 8, groups);
  
  switch (constraintsfdorder)
  {
    case 2:
    {
      EnsureStencilFits(cctkGH, "bssn_external_shift_2nd", 2, 2, 2);
      break;
    }
  ...
  LoopOverInterior(cctkGH, bssn_external_shift_2nd_Body);
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Leaving bssn_external_shift_2nd_Body");
  }
}
