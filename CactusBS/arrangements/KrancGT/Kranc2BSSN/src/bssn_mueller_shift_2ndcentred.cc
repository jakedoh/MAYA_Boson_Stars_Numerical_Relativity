...
extern "C" void bssn_mueller_shift_2ndcentred_SelectBCs(CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  
  if (cctk_iteration % bssn_mueller_shift_2ndcentred_calc_every != bssn_mueller_shift_2ndcentred_calc_offset)
    return;
...
static void bssn_mueller_shift_2ndcentred_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  #pragma omp parallel
  CCTK_LOOP3(bssn_mueller_shift_2ndcentred,
    ...
    cctk_ash[0],cctk_ash[1],cctk_ash[2])
  {
    ...
    CCTK_REAL muellerTop CCTK_ATTRIBUTE_UNUSED = 
      2*(hInv23*PDstandard2nd2phi*PDstandard2nd3phi + 
      PDstandard2nd1phi*(hInv12*PDstandard2nd2phi + 
      hInv13*PDstandard2nd3phi)) + hInv11*pow(PDstandard2nd1phi,2) + 
      hInv22*pow(PDstandard2nd2phi,2) + hInv33*pow(PDstandard2nd3phi,2);
  ...
  CCTK_ENDLOOP3(bssn_mueller_shift_2ndcentred);
}
extern "C" void bssn_mueller_shift_2ndcentred(CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Entering bssn_mueller_shift_2ndcentred_Body");
  }
  if (cctk_iteration % bssn_mueller_shift_2ndcentred_calc_every != bssn_mueller_shift_2ndcentred_calc_offset)
  {
    return;
  ...
  AssertGroupStorage(cctkGH, "bssn_mueller_shift_2ndcentred", 9, groups);
  
  switch (constraintsfdorder)
  {
    case 2:
    {
      EnsureStencilFits(cctkGH, "bssn_mueller_shift_2ndcentred", 1, 1, 1);
      break;
    }
  ...
  LoopOverInterior(cctkGH, bssn_mueller_shift_2ndcentred_Body);
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Leaving bssn_mueller_shift_2ndcentred_Body");
  }
}
