...
extern "C" void bssnchi_boundary_SelectBCs(CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  
  if (cctk_iteration % bssnchi_boundary_calc_every != bssnchi_boundary_calc_offset)
    return;
...
static void bssnchi_boundary_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  CCTK_ENDLOOP3(bssnchi_boundary);
}
extern "C" void bssnchi_boundary(CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Entering bssnchi_boundary_Body");
  }
  if (cctk_iteration % bssnchi_boundary_calc_every != bssnchi_boundary_calc_offset)
  {
    return;
  ...
  AssertGroupStorage(cctkGH, "bssnchi_boundary", 18, groups);
  
  switch (constraintsfdorder)
  {
    case 2:
    {
      EnsureStencilFits(cctkGH, "bssnchi_boundary", 2, 2, 2);
      break;
    }
  ...
  LoopOverBoundary(cctkGH, bssnchi_boundary_Body);
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Leaving bssnchi_boundary_Body");
  }
}
