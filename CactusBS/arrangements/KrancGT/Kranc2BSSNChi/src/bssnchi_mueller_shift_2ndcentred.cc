...
extern "C" void bssnchi_mueller_shift_2ndcentred_SelectBCs(CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  
  if (cctk_iteration % bssnchi_mueller_shift_2ndcentred_calc_every != bssnchi_mueller_shift_2ndcentred_calc_offset)
    return;
...
static void bssnchi_mueller_shift_2ndcentred_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  #pragma omp parallel
  CCTK_LOOP3(bssnchi_mueller_shift_2ndcentred,
    ...
    cctk_ash[0],cctk_ash[1],cctk_ash[2])
  {
    ...
    CCTK_REAL muellerTop CCTK_ATTRIBUTE_UNUSED = 
      2*(hInv23*PDstandard2nd2chi*PDstandard2nd3chi + 
      PDstandard2nd1chi*(hInv12*PDstandard2nd2chi + 
      hInv13*PDstandard2nd3chi)) + hInv11*pow(PDstandard2nd1chi,2) + 
      hInv22*pow(PDstandard2nd2chi,2) + hInv33*pow(PDstandard2nd3chi,2);
  ...
  CCTK_ENDLOOP3(bssnchi_mueller_shift_2ndcentred);
}
extern "C" void bssnchi_mueller_shift_2ndcentred(CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Entering bssnchi_mueller_shift_2ndcentred_Body");
  }
  if (cctk_iteration % bssnchi_mueller_shift_2ndcentred_calc_every != bssnchi_mueller_shift_2ndcentred_calc_offset)
  {
    return;
  ...
  AssertGroupStorage(cctkGH, "bssnchi_mueller_shift_2ndcentred", 9, groups);
  
  switch (constraintsfdorder)
  {
    case 2:
    {
      EnsureStencilFits(cctkGH, "bssnchi_mueller_shift_2ndcentred", 1, 1, 1);
      break;
    }
  ...
  LoopOverInterior(cctkGH, bssnchi_mueller_shift_2ndcentred_Body);
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Leaving bssnchi_mueller_shift_2ndcentred_Body");
  }
}
