...
externalEtaBetaGroups =
   {{"ExternalEtaBeta::exetaBeta_group", {exetaBeta} } };
...
realParameters = 
{
  ...
  {Name -> newNASAAdvection, 
   ...
   Default -> 0, Steerable -> Always},
  {Name -> etaBeta, 
   Description -> "Gauge Parameter, NASA/Mueller Shifts: dt_betat^i ~ exetaBeta*betat^i + ...",
   Default -> 0, Steerable -> Always},
  {Name -> flrwcf,
   Description -> "Parameter to activete 24pirho term , 1+log lapse: d_t alp = - harmonicf*alp*K + ... ",
   Default -> 0, Steerable -> Always},

  {Name -> inipiabnd,
   Description -> "initial pi=phidot fort inflation , must match with ini_pia in Twopuncturesmatter_flrw thorn",
   Default -> 0, Steerable -> Always},

  {Name -> VVbnd,
   Description -> "Potential at the boundary for inflation potential, Must match with VV in Twopunctures_matter_flrw thorn ",
   Default -> 0.000625, Steerable -> Always},

  {Name -> ksign,
   Description -> "sign of K for inflation",
   Default -> 1.0, Steerable -> Always},
...
lapseParam = 
{
  ...
  AllowedValues -> {"1 + log", "1 + log 2ndcentred", "1 + log full4th", "1 + log 4thcentred", "constant",
    "harmonic", "exact", "1 + log 6thcentred", "1 + log 6th", "1 + log full4thFLRW","1 + log full6thFLRW"},
  Steerable -> Recover
};
...
onePlusLogLapseFull6thFLRWCalc =
{
  Name -> fnPrefix <> "_one_plus_log_lapse_full6thFLRW",
  Schedule -> {"in MoL_CalcRHS as bssn_lapse"},
  Where -> Interior,
  ConditionalOnKeyword -> {"lapse_condition", "1 + log full6thFLRW"},
  Shorthands -> {dir[ui],deth,invdeth,hInv[ua,ub],em4phi, e4phi,pi,rho,Sij[la,lb],Tj[la],Si[la],S},
  Equations ->
  {
    deth -> hDet,
    invdeth -> 1 / deth,
    ...
    dot[alpha] -> - harmonicf alpha (K + flrwcf (24 pi rho)^0.5) + lapseAdvection beta[ua] PDlopsided6th[alpha,la]
  }
};


onePlusLogLapseFull4thFLRWCalc =
{
  Name -> fnPrefix <> "_one_plus_log_lapse_full4thFLRW",
  Schedule -> {"in MoL_CalcRHS as bssn_lapse"},
  Where -> Interior,
  ConditionalOnKeyword -> {"lapse_condition", "1 + log full4thFLRW"},
  Shorthands -> {dir[ui],deth,invdeth,hInv[ua,ub],em4phi, e4phi,pi,rho,Sij[la,lb],Tj[la],Si[la],S},
  Equations ->
  {
    deth -> hDet,
    ...
    dot[alpha] -> - harmonicf alpha (K + flrwcf (24 pi rho)^0.5) + lapseAdvection beta[ua] PDlopsided4th[alpha,la]
  }
};
...
shiftParam = 
{
  ...
  Default -> "constant",
  AllowedValues -> {"constant", "NASA", "NASA4th", "NASAfull4th", "NASA6th", "NASA2ndcentred", "NASA4thcentred", "exact", "Mueller2nd", "Muellerfull4th", "Mueller6th", "Mueller4thcentred", "Mueller2ndcentred", "Mueller6thcentred", "NASAExternalEtaBeta"},
  Steerable -> Recover
};
...
nasaShiftCalc[fdOrder_, PD_, PDadvect_] := 
{
  ...
  Equations -> 
  {
    ...
    dot[betat[ui]] -> -exetaBeta betat[ui] + chiBeta alpha^gammaDriverLapsePower dot[Gam[ui]] - NASAAdvection beta[ua] PDadvect[Gam[ui],la]  + betatAdvection beta[ua] PDadvect[betat[ui],la]
...
muellerShiftCalc[fdOrder_, PD_, PDadvect_] :=
{
  ...
  Equations ->
  {
    ...
    mueller -> exetaBeta Sqrt[Abs[muellerTop]] / muellerBottom,
...
externalShiftCalc[fdOrder_, PD_, PDadvect_] :=
{
   Name -> fnPrefix <> "_external_shift_" <> fdOrder,
   Schedule -> {"in MoL_CalcRHS after bssn_evolve after bssn_evolve_matter as bssn_shift"},
   Where -> Interior,
   Shorthands -> {dir[ui]},
   ConditionalOnKeywords -> {{"shift_condition", "NASAExternalEtaBeta"},{"fd_order",fdOrder}},
(*   DeclarationIncludes -> {"externaletabeta.h"},*)
   Equations ->
   {
     (* Shift advection *)
     dir[ui] -> Sign[beta[ui]],
...
boundaryParam = 
{
  ...
  Default -> "radiative",
  AllowedValues -> {"none", "radiative", "radiativeflrw"},
  Steerable -> Recover
};
...
boundaryCalcRadiativeFLRW =
{
  ...
  Where -> Boundary,
  Shorthands -> {n1,n2,n3,dir[ui],pi,aexact,aexactdot,K0,eightfac},
  Equations ->
  {
    pi -> 3.14159265358979323846,
    n1 -> -x/r,
    n2 -> -y/r,
    n3 -> -z/r,
    eightfac -> (8.0 pi (0.5 inipiabnd^2 + VVbnd)/3)^0.5,
    aexact -> Exp[ eightfac t],
    aexactdot-> aexact eightfac,
    K0 -> -ksign (24 pi (0.5 inipiabnd^2 + VVbnd) )^0.5,
    dir[ui] -> Sign[n[ui]],
    ...
    dot[h[li,lj]] -> -(h[li,lj] - Euc[li,lj]) / r + n[uk] PDonesided2nd[h[li,lj], lk],
    dot[A[li,lj]] -> -(A[li,lj]) / r + n[uk] PDonesided2nd[A[li,lj], lk],
    dot[K] -> -(K - K0) / r + n[uk] PDonesided2nd[K, lk],
    dot[Gam[ui]] -> 0, (* This is conventional *)

    If[phiMethod,
      dot[phi] -> 0.5 eightfac -(phi - 0.5 Log[aexact]) / r + n[uk] PDonesided2nd[phi, lk],
      dot[chi] -> -2.0 eightfac/aexact^2 - (chi - 1/aexact^2) / r + n[uk] PDonesided2nd[chi, lk]],
...
(*  nasaShiftCalc["4th", PDstandard4th, PDonesided2nd],*)
  ...
  muellerShiftCalc["2ndcentred", PDstandard2nd, PDstandard2nd],
  muellerShiftCalc["full4th",PDstandard4th, PDlopsided4th],
  muellerShiftCalc["4thcentred", PDstandard4th, PDstandard4th],
  muellerShiftCalc["6th",PDstandard6th, PDlopsided6th],
  muellerShiftCalc["6thcentred", PDstandard6th, PDstandard6th],
  externalShiftCalc["2nd", PDstandard2nd, PDonesided2nd],
  externalShiftCalc["2ndcentred", PDstandard2nd, PDstandard2nd],
  externalShiftCalc["full4th",PDstandard4th, PDlopsided4th],
  externalShiftCalc["4thcentred", PDstandard4th, PDstandard4th],
  externalShiftCalc["6th",PDstandard6th, PDlopsided6th],
  externalShiftCalc["6thcentred", PDstandard6th, PDstandard6th]
...
CreateKrancThornTT[groups, ".", thornName,
  ...
  (*UseJacobian -> True,*)
  (*UseVectors -> True,*)
  If[addMatter, InheritedImplementations -> {"admbase","TmunuBase", "ExternalEtaBeta"}, InheritedImplementations -> {"admbase"} ]];
