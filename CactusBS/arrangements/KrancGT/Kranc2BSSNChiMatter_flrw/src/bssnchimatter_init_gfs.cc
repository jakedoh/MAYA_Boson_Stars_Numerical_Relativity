...
static void bssnchimatter_init_gfs_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
  ...
  #pragma omp parallel
  CCTK_LOOP3(bssnchimatter_init_gfs,
  ...
  CCTK_ENDLOOP3(bssnchimatter_init_gfs);
}
extern "C" void bssnchimatter_init_gfs(CCTK_ARGUMENTS)
{
  #ifdef DECLARE_CCTK_ARGUMENTS_bssnchimatter_init_gfs
  DECLARE_CCTK_ARGUMENTS_CHECKED(bssnchimatter_init_gfs);
  #else
  DECLARE_CCTK_ARGUMENTS;
  #endif
  DECLARE_CCTK_PARAMETERS;
  ...
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Entering bssnchimatter_init_gfs_Body");
  }
  if (cctk_iteration % bssnchimatter_init_gfs_calc_every != bssnchimatter_init_gfs_calc_offset)
  {
    return;
  }
  ...
  AssertGroupStorage(cctkGH, "bssnchimatter_init_gfs", 8, groups);
  ...
  LoopOverEverything(cctkGH, bssnchimatter_init_gfs_Body);
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Leaving bssnchimatter_init_gfs_Body");
  }
}
