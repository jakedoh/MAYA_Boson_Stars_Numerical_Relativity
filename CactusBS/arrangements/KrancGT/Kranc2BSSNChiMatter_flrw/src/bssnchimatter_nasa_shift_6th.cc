...
extern "C" void bssnchimatter_nasa_shift_6th_SelectBCs(CCTK_ARGUMENTS)
{
  #ifdef DECLARE_CCTK_ARGUMENTS_bssnchimatter_nasa_shift_6th_SelectBCs
  DECLARE_CCTK_ARGUMENTS_CHECKED(bssnchimatter_nasa_shift_6th_SelectBCs);
  #else
  DECLARE_CCTK_ARGUMENTS;
  #endif
  DECLARE_CCTK_PARAMETERS;
  
  if (cctk_iteration % bssnchimatter_nasa_shift_6th_calc_every != bssnchimatter_nasa_shift_6th_calc_offset)
    return;
...
static void bssnchimatter_nasa_shift_6th_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  #pragma omp parallel
  CCTK_LOOP3(bssnchimatter_nasa_shift_6th,
  ...
  CCTK_ENDLOOP3(bssnchimatter_nasa_shift_6th);
}
extern "C" void bssnchimatter_nasa_shift_6th(CCTK_ARGUMENTS)
{
  #ifdef DECLARE_CCTK_ARGUMENTS_bssnchimatter_nasa_shift_6th
  DECLARE_CCTK_ARGUMENTS_CHECKED(bssnchimatter_nasa_shift_6th);
  #else
  DECLARE_CCTK_ARGUMENTS;
  #endif
  DECLARE_CCTK_PARAMETERS;
  ...
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Entering bssnchimatter_nasa_shift_6th_Body");
  }
  if (cctk_iteration % bssnchimatter_nasa_shift_6th_calc_every != bssnchimatter_nasa_shift_6th_calc_offset)
  {
    return;
  ...
  AssertGroupStorage(cctkGH, "bssnchimatter_nasa_shift_6th", 8, groups);
  
  switch (constraintsfdorder)
  {
    case 2:
    {
      EnsureStencilFits(cctkGH, "bssnchimatter_nasa_shift_6th", 4, 4, 4);
      break;
    }
  ...
  LoopOverInterior(cctkGH, bssnchimatter_nasa_shift_6th_Body);
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Leaving bssnchimatter_nasa_shift_6th_Body");
  }
}
