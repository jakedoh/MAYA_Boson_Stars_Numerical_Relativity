...
extern "C" void bssnchimatter_one_plus_log_lapse_SelectBCs(CCTK_ARGUMENTS)
{
  #ifdef DECLARE_CCTK_ARGUMENTS_bssnchimatter_one_plus_log_lapse_SelectBCs
  DECLARE_CCTK_ARGUMENTS_CHECKED(bssnchimatter_one_plus_log_lapse_SelectBCs);
  #else
  DECLARE_CCTK_ARGUMENTS;
  #endif
  DECLARE_CCTK_PARAMETERS;
  
  if (cctk_iteration % bssnchimatter_one_plus_log_lapse_calc_every != bssnchimatter_one_plus_log_lapse_calc_offset)
    return;
...
static void bssnchimatter_one_plus_log_lapse_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  #pragma omp parallel
  CCTK_LOOP3(bssnchimatter_one_plus_log_lapse,
    ...
    cctk_ash[0],cctk_ash[1],cctk_ash[2])
  {
    ...
    CCTK_REAL alpharhsL CCTK_ATTRIBUTE_UNUSED = -(alphaL*KL*harmonicf) + 
      (beta1L*PDonesided2nd1(&alpha[index]) + 
      beta2L*PDonesided2nd2(&alpha[index]) + 
      beta3L*PDonesided2nd3(&alpha[index]))*lapseAdvection;
    /* Copy local copies back to grid functions */
    alpharhs[index] = alpharhsL;
  }
  CCTK_ENDLOOP3(bssnchimatter_one_plus_log_lapse);
}
extern "C" void bssnchimatter_one_plus_log_lapse(CCTK_ARGUMENTS)
{
  #ifdef DECLARE_CCTK_ARGUMENTS_bssnchimatter_one_plus_log_lapse
  DECLARE_CCTK_ARGUMENTS_CHECKED(bssnchimatter_one_plus_log_lapse);
  #else
  DECLARE_CCTK_ARGUMENTS;
  #endif
  DECLARE_CCTK_PARAMETERS;
  ...
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Entering bssnchimatter_one_plus_log_lapse_Body");
  }
  if (cctk_iteration % bssnchimatter_one_plus_log_lapse_calc_every != bssnchimatter_one_plus_log_lapse_calc_offset)
  {
    return;
  ...
  AssertGroupStorage(cctkGH, "bssnchimatter_one_plus_log_lapse", 4, groups);
  
  switch (constraintsfdorder)
  {
    case 2:
    {
      EnsureStencilFits(cctkGH, "bssnchimatter_one_plus_log_lapse", 2, 2, 2);
      break;
    }
  ...
  LoopOverInterior(cctkGH, bssnchimatter_one_plus_log_lapse_Body);
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Leaving bssnchimatter_one_plus_log_lapse_Body");
  }
}
