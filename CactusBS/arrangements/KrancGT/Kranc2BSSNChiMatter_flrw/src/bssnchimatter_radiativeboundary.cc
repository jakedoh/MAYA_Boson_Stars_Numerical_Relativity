...
extern "C" void bssnchimatter_radiativeboundary_SelectBCs(CCTK_ARGUMENTS)
{
  #ifdef DECLARE_CCTK_ARGUMENTS_bssnchimatter_radiativeboundary_SelectBCs
  DECLARE_CCTK_ARGUMENTS_CHECKED(bssnchimatter_radiativeboundary_SelectBCs);
  #else
  DECLARE_CCTK_ARGUMENTS;
  #endif
  DECLARE_CCTK_PARAMETERS;
  
  if (cctk_iteration % bssnchimatter_radiativeboundary_calc_every != bssnchimatter_radiativeboundary_calc_offset)
    return;
...
static void bssnchimatter_radiativeboundary_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  #pragma omp parallel
  CCTK_LOOP3(bssnchimatter_radiativeboundary,
    ...
    cctk_ash[0],cctk_ash[1],cctk_ash[2])
  {
    ...
    CCTK_REAL b1 CCTK_ATTRIBUTE_UNUSED = betaXasymp;
    
    CCTK_REAL b2 CCTK_ATTRIBUTE_UNUSED = betaYasymp;
    
    CCTK_REAL b3 CCTK_ATTRIBUTE_UNUSED = betaZasymp;
  ...
  CCTK_ENDLOOP3(bssnchimatter_radiativeboundary);
}
extern "C" void bssnchimatter_radiativeboundary(CCTK_ARGUMENTS)
{
  #ifdef DECLARE_CCTK_ARGUMENTS_bssnchimatter_radiativeboundary
  DECLARE_CCTK_ARGUMENTS_CHECKED(bssnchimatter_radiativeboundary);
  #else
  DECLARE_CCTK_ARGUMENTS;
  #endif
  DECLARE_CCTK_PARAMETERS;
  ...
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Entering bssnchimatter_radiativeboundary_Body");
  }
  if (cctk_iteration % bssnchimatter_radiativeboundary_calc_every != bssnchimatter_radiativeboundary_calc_offset)
  {
    return;
  ...
  AssertGroupStorage(cctkGH, "bssnchimatter_radiativeboundary", 18, groups);
  
  switch (constraintsfdorder)
  {
    case 2:
    {
      EnsureStencilFits(cctkGH, "bssnchimatter_radiativeboundary", 2, 2, 2);
      break;
    }
  ...
  LoopOverBoundary(cctkGH, bssnchimatter_radiativeboundary_Body);
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Leaving bssnchimatter_radiativeboundary_Body");
  }
}
