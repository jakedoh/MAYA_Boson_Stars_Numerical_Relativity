...
extern "C" void bssnmatter_calc_constraints_matter_SelectBCs(CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  
  if (cctk_iteration % bssnmatter_calc_constraints_matter_calc_every != bssnmatter_calc_constraints_matter_calc_offset)
    return;
...
static void bssnmatter_calc_constraints_matter_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  #pragma omp parallel
  CCTK_LOOP3(bssnmatter_calc_constraints_matter,
  ...
  CCTK_ENDLOOP3(bssnmatter_calc_constraints_matter);
}
extern "C" void bssnmatter_calc_constraints_matter(CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Entering bssnmatter_calc_constraints_matter_Body");
  }
  if (cctk_iteration % bssnmatter_calc_constraints_matter_calc_every != bssnmatter_calc_constraints_matter_calc_offset)
  {
    return;
  ...
  AssertGroupStorage(cctkGH, "bssnmatter_calc_constraints_matter", 10, groups);
  
  switch (constraintsfdorder)
  {
    case 2:
    {
      EnsureStencilFits(cctkGH, "bssnmatter_calc_constraints_matter", 1, 1, 1);
      break;
    }
    
    case 4:
    {
      EnsureStencilFits(cctkGH, "bssnmatter_calc_constraints_matter", 2, 2, 2);
      break;
    }
    
    case 6:
    {
      EnsureStencilFits(cctkGH, "bssnmatter_calc_constraints_matter", 3, 3, 3);
      break;
    }
    default:
      CCTK_BUILTIN_UNREACHABLE();
  ...
  LoopOverInterior(cctkGH, bssnmatter_calc_constraints_matter_Body);
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Leaving bssnmatter_calc_constraints_matter_Body");
  }
}
