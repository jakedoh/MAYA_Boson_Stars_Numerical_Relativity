...
static void ScalarTensor_ADMToLocal_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
  ...
  #pragma omp parallel
  CCTK_LOOP3(ScalarTensor_ADMToLocal,
    ...
    cctk_ash[0],cctk_ash[1],cctk_ash[2])
  {
    ...
    CCTK_REAL alpLocalL CCTK_ATTRIBUTE_UNUSED = alpL;
    ...
    CCTK_REAL kLocal11 CCTK_ATTRIBUTE_UNUSED = kxxL;
    
    CCTK_REAL kLocal21 CCTK_ATTRIBUTE_UNUSED = kxyL;
    
    CCTK_REAL kLocal31 CCTK_ATTRIBUTE_UNUSED = kxzL;
    
    CCTK_REAL kLocal22 CCTK_ATTRIBUTE_UNUSED = kyyL;
    
    CCTK_REAL kLocal32 CCTK_ATTRIBUTE_UNUSED = kyzL;
    
    CCTK_REAL kLocal33 CCTK_ATTRIBUTE_UNUSED = kzzL;
    ...
    CCTK_REAL KtraceLocalL CCTK_ATTRIBUTE_UNUSED = gLocalInv11*kLocal11 + 
      gLocalInv22*kLocal22 + 2*(gLocalInv12*kLocal21 + gLocalInv13*kLocal31 + 
      gLocalInv23*kLocal32) + gLocalInv33*kLocal33;
    /* Copy local copies back to grid functions */
    alpLocal[index] = alpLocalL;
    gLocal11[index] = gLocal11L;
    gLocal21[index] = gLocal21L;
    gLocal22[index] = gLocal22L;
    gLocal31[index] = gLocal31L;
    gLocal32[index] = gLocal32L;
    gLocal33[index] = gLocal33L;
    KtraceLocal[index] = KtraceLocalL;
  }
  CCTK_ENDLOOP3(ScalarTensor_ADMToLocal);
}
extern "C" void ScalarTensor_ADMToLocal(CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Entering ScalarTensor_ADMToLocal_Body");
  }
  if (cctk_iteration % ScalarTensor_ADMToLocal_calc_every != ScalarTensor_ADMToLocal_calc_offset)
  {
    return;
  }
  ...
  const char* const groups[] = {
    ...
    "ScalarTensor::KtraceLocal_group"};
  AssertGroupStorage(cctkGH, "ScalarTensor_ADMToLocal", 6, groups);
  
  
  LoopOverEverything(cctkGH, ScalarTensor_ADMToLocal_Body);
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Leaving ScalarTensor_ADMToLocal_Body");
  }
}
