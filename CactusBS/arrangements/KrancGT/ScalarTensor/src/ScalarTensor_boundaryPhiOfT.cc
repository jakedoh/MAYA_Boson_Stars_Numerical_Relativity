...
extern "C" void ScalarTensor_boundaryPhiOfT_SelectBCs(CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  
  if (cctk_iteration % ScalarTensor_boundaryPhiOfT_calc_every != ScalarTensor_boundaryPhiOfT_calc_offset)
    return;
...
static void ScalarTensor_boundaryPhiOfT_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  #pragma omp parallel
  CCTK_LOOP3(ScalarTensor_boundaryPhiOfT,
    ...
    cctk_ash[0],cctk_ash[1],cctk_ash[2])
  {
    ...
    CCTK_REAL STphirhsL CCTK_ATTRIBUTE_UNUSED = 
      PDonesided2nd1(&STphi[index])*n1 + PDonesided2nd2(&STphi[index])*n2 + 
      PDonesided2nd3(&STphi[index])*n3 + (-STphiL + piBG*t)*pow(rL,-1);
  ...
  CCTK_ENDLOOP3(ScalarTensor_boundaryPhiOfT);
}
extern "C" void ScalarTensor_boundaryPhiOfT(CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Entering ScalarTensor_boundaryPhiOfT_Body");
  }
  if (cctk_iteration % ScalarTensor_boundaryPhiOfT_calc_every != ScalarTensor_boundaryPhiOfT_calc_offset)
  {
    return;
  ...
  AssertGroupStorage(cctkGH, "ScalarTensor_boundaryPhiOfT", 7, groups);
  
  EnsureStencilFits(cctkGH, "ScalarTensor_boundaryPhiOfT", 2, 2, 2);
  
  LoopOverBoundary(cctkGH, ScalarTensor_boundaryPhiOfT_Body);
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Leaving ScalarTensor_boundaryPhiOfT_Body");
  }
}
