...
extern "C" void ScalarTensor_boundaryRadiative_SelectBCs(CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  
  if (cctk_iteration % ScalarTensor_boundaryRadiative_calc_every != ScalarTensor_boundaryRadiative_calc_offset)
    return;
...
static void ScalarTensor_boundaryRadiative_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  #pragma omp parallel
  CCTK_LOOP3(ScalarTensor_boundaryRadiative,
    ...
    cctk_ash[0],cctk_ash[1],cctk_ash[2])
  {
    ...
    CCTK_REAL STpirhsL CCTK_ATTRIBUTE_UNUSED = (-STpiL + 
      rL*(PDonesided2nd1(&STpi[index])*n1 + PDonesided2nd2(&STpi[index])*n2 + 
      PDonesided2nd3(&STpi[index])*n3) + piBG)*pow(rL,-1);
    
    CCTK_REAL STphirhsL CCTK_ATTRIBUTE_UNUSED = (-STphiL + 
      rL*(PDonesided2nd1(&STphi[index])*n1 + PDonesided2nd2(&STphi[index])*n2 
      + PDonesided2nd3(&STphi[index])*n3) + phiBG)*pow(rL,-1);
    
    CCTK_REAL rhoEL CCTK_ATTRIBUTE_UNUSED = 0;
    
    CCTK_REAL rhoJL CCTK_ATTRIBUTE_UNUSED = 0;
    /* Copy local copies back to grid functions */
    rhoE[index] = rhoEL;
    rhoJ[index] = rhoJL;
  ...
  CCTK_ENDLOOP3(ScalarTensor_boundaryRadiative);
}
extern "C" void ScalarTensor_boundaryRadiative(CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Entering ScalarTensor_boundaryRadiative_Body");
  }
  if (cctk_iteration % ScalarTensor_boundaryRadiative_calc_every != ScalarTensor_boundaryRadiative_calc_offset)
  {
    return;
  ...
  AssertGroupStorage(cctkGH, "ScalarTensor_boundaryRadiative", 7, groups);
  
  EnsureStencilFits(cctkGH, "ScalarTensor_boundaryRadiative", 2, 2, 2);
  
  LoopOverBoundary(cctkGH, ScalarTensor_boundaryRadiative_Body);
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Leaving ScalarTensor_boundaryRadiative_Body");
  }
}
