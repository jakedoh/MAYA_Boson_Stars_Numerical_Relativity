...
subroutine LeanBSSN_calc_bssn_rhs( CCTK_ARGUMENTS )
  ...
  CCTK_REAL                rhs_ww, rhs_hh(3,3), rhs_trk, rhs_aa(3,3),      &
                           rhs_gammat(3), rhs_alph, rhs_beta(3)
  ...
  !$OMP rhs_ww, rhs_hh, rhs_trk, rhs_aa, rhs_gammat, rhs_alph, rhs_beta, &
 ...
 if (use_jacobian) then
  ...
  if (use_jacobian) then  
    ...
    ! Note: we implement W^2 R_{ij}
    c_ri_ww = ww * ( cd2_ww + hh * tr_cd2_ww ) - 2.0 * hh * tr_dww_dww
    ...
    do a = 1, 3
      do m = 1, 3
        rhs_gammat(a) = rhs_gammat(a) - 4 * alph * hu(a,m) * d1_trk(m) / 3   &
                       - 2.0 * au(a,m) * ( d1_alph(m) + 3.0 * alph * d1_ww(m) / ww )
        do n = 1, 3
    ...
    srcSijTF = srcSij - srcS_ww2 * hh / 3.0
...
subroutine LeanBSSN_calc_bssn_rhs_bdry_sph( CCTK_ARGUMENTS )
  ...
  odr2 = 1.0 / (2.0*CCTK_DELTA_SPACE(3))
  ...
  ! k = cctk_lsh(3)-cctk_nghostzones(3)+1
  do k = cctk_lsh(3)-cctk_nghostzones(3)+1, cctk_lsh(3)
     do j = 1, cctk_lsh(2)
        do i = 1, cctk_lsh(1)
           ...
           rhs_conf_fac(i,j,k)  = -dr_ww - (ww - 1.0) / rr

           rhs_hxx(i,j,k)  = -dr_hh(1,1) - (hh(1,1) - 1.0) / rr
           rhs_hxy(i,j,k)  = -dr_hh(1,2) -  hh(1,2)        / rr
           rhs_hxz(i,j,k)  = -dr_hh(1,3) -  hh(1,3)        / rr
           rhs_hyy(i,j,k)  = -dr_hh(2,2) - (hh(2,2) - 1.0) / rr
           rhs_hyz(i,j,k)  = -dr_hh(2,3) -  hh(2,3)        / rr
           rhs_hzz(i,j,k)  = -dr_hh(3,3) - (hh(3,3) - 1.0) / rr
           ...
           rhs_alp(i,j,k)  = -dr_alph - (alph - 1.0) / rr

           rhs_betax(i,j,k) = -dr_beta(1) - beta(1) / rr
           rhs_betay(i,j,k) = -dr_beta(2) - beta(2) / rr
           rhs_betaz(i,j,k) = -dr_beta(3) - beta(3) / rr
