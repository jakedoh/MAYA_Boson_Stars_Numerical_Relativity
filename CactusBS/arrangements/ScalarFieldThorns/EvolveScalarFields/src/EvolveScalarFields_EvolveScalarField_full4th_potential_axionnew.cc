...
extern "C" void EvolveScalarFields_EvolveScalarField_full4th_potential_axionnew_SelectBCs(CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  
  if (cctk_iteration % EvolveScalarFields_EvolveScalarField_full4th_potential_axionnew_calc_every != EvolveScalarFields_EvolveScalarField_full4th_potential_axionnew_calc_offset)
    return;
...
static void EvolveScalarFields_EvolveScalarField_full4th_potential_axionnew_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  #pragma omp parallel
  CCTK_LOOP3(EvolveScalarFields_EvolveScalarField_full4th_potential_axionnew,
    ...
    cctk_ash[0],cctk_ash[1],cctk_ash[2])
  {
    ...
    CCTK_REAL derivVa CCTK_ATTRIBUTE_UNUSED = 
      0.25*phiaL*lambdaphian*pow(vv1 - phivsign*tanh(pow(sigma,-1)*(delta + 
      r0apotential - rL*pow(fabs(gzzL*(pow(xL,2) + pow(yL,2)) + (2*gxyL*xL*yL 
      + gxxL*pow(xL,2) + 
      ...
      pow(yL,2),0.5)),0.5)*pow(fmax(1.e-20,rL),-1))),2);
    
    CCTK_REAL derivVb CCTK_ATTRIBUTE_UNUSED = 
      0.5*phibL*lambdaphibn*(pow(vv1,2) + 
      pow(phivsign,2)*pow(tanh(pow(sigma,-1)*(delta + r0apotential - 
      rL*pow(fabs(gzzL*(pow(xL,2) + pow(yL,2)) + (2*gxyL*xL*yL + 
      gxxL*pow(xL,2) + 
      gyyL*pow(yL,2))*pow(zL,2)*pow(fmax(1.e-20,pow(pow(xL,2) + 
      pow(yL,2),0.5)),-2) - 2*(gxzL*xL + 
      gyzL*yL)*zL*pow(fmax(1.e-20,pow(pow(xL,2) + 
  ...
  CCTK_ENDLOOP3(EvolveScalarFields_EvolveScalarField_full4th_potential_axionnew);
}
extern "C" void EvolveScalarFields_EvolveScalarField_full4th_potential_axionnew(CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Entering EvolveScalarFields_EvolveScalarField_full4th_potential_axionnew_Body");
  }
  if (cctk_iteration % EvolveScalarFields_EvolveScalarField_full4th_potential_axionnew_calc_every != EvolveScalarFields_EvolveScalarField_full4th_potential_axionnew_calc_offset)
  {
    return;
  ...
  AssertGroupStorage(cctkGH, "EvolveScalarFields_EvolveScalarField_full4th_potential_axionnew", 13, groups);
  
  EnsureStencilFits(cctkGH, "EvolveScalarFields_EvolveScalarField_full4th_potential_axionnew", 3, 3, 3);
  
  LoopOverInterior(cctkGH, EvolveScalarFields_EvolveScalarField_full4th_potential_axionnew_Body);
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Leaving EvolveScalarFields_EvolveScalarField_full4th_potential_axionnew_Body");
  }
}
