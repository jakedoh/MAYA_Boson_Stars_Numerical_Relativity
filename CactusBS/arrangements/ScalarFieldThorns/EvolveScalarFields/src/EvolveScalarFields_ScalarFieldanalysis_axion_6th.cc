...
extern "C" void EvolveScalarFields_ScalarFieldanalysis_axion_6th_SelectBCs(CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  
  if (cctk_iteration % EvolveScalarFields_ScalarFieldanalysis_axion_6th_calc_every != EvolveScalarFields_ScalarFieldanalysis_axion_6th_calc_offset)
    return;
...
static void EvolveScalarFields_ScalarFieldanalysis_axion_6th_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
  ...
  #pragma omp parallel
  CCTK_LOOP3(EvolveScalarFields_ScalarFieldanalysis_axion_6th,
    ...
    cctk_ash[0],cctk_ash[1],cctk_ash[2])
  {
    ...
    SFTttL = pow(dtphia,2) + pow(dtphib,2) - 0.125*(betaSqr - 
      pow(alpL,2))*(4*(dphiaSqrL + dphibSqrL) + lambdaphian*pow(pow(phiaL,2) 
      - pow(vv0,2)*pow(vv1 + phivsign*tanh(rL - r0apotential),2),2) + 
      lambdaphibn*pow(pow(phibL,2) - pow(vv0,2)*pow(vv1 + phivsign*tanh(rL - 
      r0apotential),2),2));
    
    CCTK_REAL SFTtj1 CCTK_ATTRIBUTE_UNUSED = dtphia*PDstandard6th1phia + 
      dtphib*PDstandard6th1phib + 0.125*(betaxL*gxxL + betayL*gxyL + 
      betazL*gxzL)*(-4*(dphiaSqrL + dphibSqrL) - lambdaphian*pow(pow(phiaL,2) 
      - pow(vv0,2)*pow(vv1 + phivsign*tanh(rL - r0apotential),2),2) - 
      lambdaphibn*pow(pow(phibL,2) - pow(vv0,2)*pow(vv1 + phivsign*tanh(rL - 
      r0apotential),2),2));
    
    CCTK_REAL SFTtj2 CCTK_ATTRIBUTE_UNUSED = dtphia*PDstandard6th2phia + 
      dtphib*PDstandard6th2phib + 0.125*(betaxL*gxyL + betayL*gyyL + 
      betazL*gyzL)*(-4*(dphiaSqrL + dphibSqrL) - lambdaphian*pow(pow(phiaL,2) 
      - pow(vv0,2)*pow(vv1 + phivsign*tanh(rL - r0apotential),2),2) - 
      lambdaphibn*pow(pow(phibL,2) - pow(vv0,2)*pow(vv1 + phivsign*tanh(rL - 
      r0apotential),2),2));
    
    CCTK_REAL SFTtj3 CCTK_ATTRIBUTE_UNUSED = dtphia*PDstandard6th3phia + 
      dtphib*PDstandard6th3phib + 0.125*(betaxL*gxzL + betayL*gyzL + 
      betazL*gzzL)*(-4*(dphiaSqrL + dphibSqrL) - lambdaphian*pow(pow(phiaL,2) 
      - pow(vv0,2)*pow(vv1 + phivsign*tanh(rL - r0apotential),2),2) - 
      lambdaphibn*pow(pow(phibL,2) - pow(vv0,2)*pow(vv1 + phivsign*tanh(rL - 
      r0apotential),2),2));
    
    CCTK_REAL SFTij11 CCTK_ATTRIBUTE_UNUSED = pow(PDstandard6th1phia,2) + 
      pow(PDstandard6th1phib,2) - 0.125*gxxL*(4*(dphiaSqrL + dphibSqrL) + 
      lambdaphian*pow(pow(phiaL,2) - pow(vv0,2)*pow(vv1 + phivsign*tanh(rL - 
      r0apotential),2),2) + lambdaphibn*pow(pow(phibL,2) - pow(vv0,2)*pow(vv1 
      + phivsign*tanh(rL - r0apotential),2),2));
    
    CCTK_REAL SFTij21 CCTK_ATTRIBUTE_UNUSED = 
      PDstandard6th1phia*PDstandard6th2phia + 
      PDstandard6th1phib*PDstandard6th2phib - 0.125*gxyL*(4*(dphiaSqrL + 
      dphibSqrL) + lambdaphian*pow(pow(phiaL,2) - pow(vv0,2)*pow(vv1 + 
      phivsign*tanh(rL - r0apotential),2),2) + lambdaphibn*pow(pow(phibL,2) - 
      pow(vv0,2)*pow(vv1 + phivsign*tanh(rL - r0apotential),2),2));
    
    CCTK_REAL SFTij31 CCTK_ATTRIBUTE_UNUSED = 
      PDstandard6th1phia*PDstandard6th3phia + 
      PDstandard6th1phib*PDstandard6th3phib - 0.125*gxzL*(4*(dphiaSqrL + 
      dphibSqrL) + lambdaphian*pow(pow(phiaL,2) - pow(vv0,2)*pow(vv1 + 
      phivsign*tanh(rL - r0apotential),2),2) + lambdaphibn*pow(pow(phibL,2) - 
      pow(vv0,2)*pow(vv1 + phivsign*tanh(rL - r0apotential),2),2));
    
    CCTK_REAL SFTij22 CCTK_ATTRIBUTE_UNUSED = pow(PDstandard6th2phia,2) + 
      pow(PDstandard6th2phib,2) - 0.125*gyyL*(4*(dphiaSqrL + dphibSqrL) + 
      lambdaphian*pow(pow(phiaL,2) - pow(vv0,2)*pow(vv1 + phivsign*tanh(rL - 
      r0apotential),2),2) + lambdaphibn*pow(pow(phibL,2) - pow(vv0,2)*pow(vv1 
      + phivsign*tanh(rL - r0apotential),2),2));
    ...
    CCTK_REAL SFTij32 CCTK_ATTRIBUTE_UNUSED = 
      ...
      PDstandard6th2phib*PDstandard6th3phib - 0.125*gyzL*(4*(dphiaSqrL + 
      dphibSqrL) + lambdaphian*pow(pow(phiaL,2) - pow(vv0,2)*pow(vv1 + 
      phivsign*tanh(rL - r0apotential),2),2) + lambdaphibn*pow(pow(phibL,2) - 
      pow(vv0,2)*pow(vv1 + phivsign*tanh(rL - r0apotential),2),2));
    
    CCTK_REAL SFTij33 CCTK_ATTRIBUTE_UNUSED = pow(PDstandard6th3phia,2) + 
      pow(PDstandard6th3phib,2) - 0.125*gzzL*(4*(dphiaSqrL + dphibSqrL) + 
      lambdaphian*pow(pow(phiaL,2) - pow(vv0,2)*pow(vv1 + phivsign*tanh(rL - 
      r0apotential),2),2) + lambdaphibn*pow(pow(phibL,2) - pow(vv0,2)*pow(vv1 
      + phivsign*tanh(rL - r0apotential),2),2));
    ...
    CCTK_REAL SFpotentialL CCTK_ATTRIBUTE_UNUSED = 
      0.125*(lambdaphian*pow(pow(phiaL,2) - pow(vv0,2)*pow(vv1 + 
      phivsign*tanh(rL - r0apotential),2),2) + lambdaphibn*pow(pow(phibL,2) - 
      pow(vv0,2)*pow(vv1 + phivsign*tanh(rL - r0apotential),2),2));
  ...
  CCTK_ENDLOOP3(EvolveScalarFields_ScalarFieldanalysis_axion_6th);
}
extern "C" void EvolveScalarFields_ScalarFieldanalysis_axion_6th(CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Entering EvolveScalarFields_ScalarFieldanalysis_axion_6th_Body");
  }
  if (cctk_iteration % EvolveScalarFields_ScalarFieldanalysis_axion_6th_calc_every != EvolveScalarFields_ScalarFieldanalysis_axion_6th_calc_offset)
  {
    return;
  ...
  AssertGroupStorage(cctkGH, "EvolveScalarFields_ScalarFieldanalysis_axion_6th", 19, groups);
  
  EnsureStencilFits(cctkGH, "EvolveScalarFields_ScalarFieldanalysis_axion_6th", 4, 4, 4);
  
  LoopOverInterior(cctkGH, EvolveScalarFields_ScalarFieldanalysis_axion_6th_Body);
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Leaving EvolveScalarFields_ScalarFieldanalysis_axion_6th_Body");
  }
}
