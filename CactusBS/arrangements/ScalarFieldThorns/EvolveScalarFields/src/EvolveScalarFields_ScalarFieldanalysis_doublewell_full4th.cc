...
extern "C" void EvolveScalarFields_ScalarFieldanalysis_doublewell_full4th_SelectBCs(CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  
  if (cctk_iteration % EvolveScalarFields_ScalarFieldanalysis_doublewell_full4th_calc_every != EvolveScalarFields_ScalarFieldanalysis_doublewell_full4th_calc_offset)
    return;
...
static void EvolveScalarFields_ScalarFieldanalysis_doublewell_full4th_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
  ...
  #pragma omp parallel
  CCTK_LOOP3(EvolveScalarFields_ScalarFieldanalysis_doublewell_full4th,
    ...
    cctk_ash[0],cctk_ash[1],cctk_ash[2])
  {
    ...
    CCTK_REAL SFTij11 CCTK_ATTRIBUTE_UNUSED = pow(PDstandard4th1phia,2) + 
      pow(PDstandard4th1phib,2) - 0.25*gxxL*(2*(dphiaSqrL + dphibSqrL + 
      pow(phiaL,2)*(gcouple*pow(phibL,2) + pow(phiamass,2)) + 
    ...
    CCTK_REAL SFTij21 CCTK_ATTRIBUTE_UNUSED = 
      ...
      PDstandard4th1phib*PDstandard4th2phib - 0.25*gxyL*(2*(dphiaSqrL + 
    ...
    CCTK_REAL SFTij31 CCTK_ATTRIBUTE_UNUSED = 
      ...
      PDstandard4th1phib*PDstandard4th3phib - 0.25*gxzL*(2*(dphiaSqrL + 
      dphibSqrL + pow(phiaL,2)*(gcouple*pow(phibL,2) + pow(phiamass,2)) + 
      pow(phibL,2)*pow(phibmass,2)) + lambdaphian*pow(pow(phiaL,2) - 
      pow(phiav,2),2) + lambdaphibn*pow(pow(phibL,2) - pow(phibv,2),2));
    
    CCTK_REAL SFTij22 CCTK_ATTRIBUTE_UNUSED = pow(PDstandard4th2phia,2) + 
      pow(PDstandard4th2phib,2) - 0.25*gyyL*(2*(dphiaSqrL + dphibSqrL + 
      pow(phiaL,2)*(gcouple*pow(phibL,2) + pow(phiamass,2)) + 
      pow(phibL,2)*pow(phibmass,2)) + lambdaphian*pow(pow(phiaL,2) - 
      pow(phiav,2),2) + lambdaphibn*pow(pow(phibL,2) - pow(phibv,2),2));
    ...
    CCTK_REAL SFTij32 CCTK_ATTRIBUTE_UNUSED = 
      ...
      PDstandard4th2phib*PDstandard4th3phib - 0.25*gyzL*(2*(dphiaSqrL + 
      dphibSqrL + pow(phiaL,2)*(gcouple*pow(phibL,2) + pow(phiamass,2)) + 
      pow(phibL,2)*pow(phibmass,2)) + lambdaphian*pow(pow(phiaL,2) - 
      pow(phiav,2),2) + lambdaphibn*pow(pow(phibL,2) - pow(phibv,2),2));
    
    CCTK_REAL SFTij33 CCTK_ATTRIBUTE_UNUSED = pow(PDstandard4th3phia,2) + 
      pow(PDstandard4th3phib,2) - 0.25*gzzL*(2*(dphiaSqrL + dphibSqrL + 
      pow(phiaL,2)*(gcouple*pow(phibL,2) + pow(phiamass,2)) + 
      pow(phibL,2)*pow(phibmass,2)) + lambdaphian*pow(pow(phiaL,2) - 
      pow(phiav,2),2) + lambdaphibn*pow(pow(phibL,2) - pow(phibv,2),2));
  ...
  CCTK_ENDLOOP3(EvolveScalarFields_ScalarFieldanalysis_doublewell_full4th);
}
extern "C" void EvolveScalarFields_ScalarFieldanalysis_doublewell_full4th(CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Entering EvolveScalarFields_ScalarFieldanalysis_doublewell_full4th_Body");
  }
  if (cctk_iteration % EvolveScalarFields_ScalarFieldanalysis_doublewell_full4th_calc_every != EvolveScalarFields_ScalarFieldanalysis_doublewell_full4th_calc_offset)
  {
    return;
  ...
  AssertGroupStorage(cctkGH, "EvolveScalarFields_ScalarFieldanalysis_doublewell_full4th", 19, groups);
  
  EnsureStencilFits(cctkGH, "EvolveScalarFields_ScalarFieldanalysis_doublewell_full4th", 3, 3, 3);
  
  LoopOverInterior(cctkGH, EvolveScalarFields_ScalarFieldanalysis_doublewell_full4th_Body);
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Leaving EvolveScalarFields_ScalarFieldanalysis_doublewell_full4th_Body");
  }
}
