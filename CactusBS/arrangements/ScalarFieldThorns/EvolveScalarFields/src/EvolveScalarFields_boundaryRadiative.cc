...
static void EvolveScalarFields_boundaryRadiative_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
    ...
    cctk_ash[0],cctk_ash[1],cctk_ash[2])
  {
    ...
    CCTK_REAL piarhsL CCTK_ATTRIBUTE_UNUSED = (-piaL + 
      rL*(PDonesided2nd1(&pia[index])*n1 + PDonesided2nd2(&pia[index])*n2 + 
      PDonesided2nd3(&pia[index])*n3) + piaBG)*pow(rL,-1);
    
    CCTK_REAL phiarhsL CCTK_ATTRIBUTE_UNUSED = (-phiaL + 
      rL*(PDonesided2nd1(&phia[index])*n1 + PDonesided2nd2(&phia[index])*n2 + 
      PDonesided2nd3(&phia[index])*n3) + phiaBG)*pow(rL,-1);
    
    CCTK_REAL pibrhsL CCTK_ATTRIBUTE_UNUSED = (-pibL + 
      rL*(PDonesided2nd1(&pib[index])*n1 + PDonesided2nd2(&pib[index])*n2 + 
      PDonesided2nd3(&pib[index])*n3) + pibBG)*pow(rL,-1);
    
    CCTK_REAL phibrhsL CCTK_ATTRIBUTE_UNUSED = (-phibL + 
      rL*(PDonesided2nd1(&phib[index])*n1 + PDonesided2nd2(&phib[index])*n2 + 
      PDonesided2nd3(&phib[index])*n3) + phibBG)*pow(rL,-1);
...
extern "C" void EvolveScalarFields_boundaryRadiative(CCTK_ARGUMENTS)
{
  ...
  AssertGroupStorage(cctkGH, "EvolveScalarFields_boundaryRadiative", 10, groups);
