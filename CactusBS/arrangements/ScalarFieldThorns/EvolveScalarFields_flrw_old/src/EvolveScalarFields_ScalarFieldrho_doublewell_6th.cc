...
extern "C" void EvolveScalarFields_ScalarFieldrho_doublewell_6th_SelectBCs(CCTK_ARGUMENTS)
{
  #ifdef DECLARE_CCTK_ARGUMENTS_EvolveScalarFields_ScalarFieldrho_doublewell_6th_SelectBCs
  DECLARE_CCTK_ARGUMENTS_CHECKED(EvolveScalarFields_ScalarFieldrho_doublewell_6th_SelectBCs);
  #else
  DECLARE_CCTK_ARGUMENTS;
  #endif
  DECLARE_CCTK_PARAMETERS;
  
  if (cctk_iteration % EvolveScalarFields_ScalarFieldrho_doublewell_6th_calc_every != EvolveScalarFields_ScalarFieldrho_doublewell_6th_calc_offset)
    return;
...
static void EvolveScalarFields_ScalarFieldrho_doublewell_6th_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  #pragma omp parallel
  CCTK_LOOP3(EvolveScalarFields_ScalarFieldrho_doublewell_6th,
    ...
    cctk_ash[0],cctk_ash[1],cctk_ash[2])
  {
    ...
    SFTttL = pow(dtphia,2) + pow(dtphib,2) + 0.25*(-betaSqr + 
      ...
      pow(phiav,4)) + 2*(dphiaSqr + dphibSqr + 
    ...
    CCTK_REAL SFTij11 CCTK_ATTRIBUTE_UNUSED = pow(PDstandard6th1phia,2) + 
      pow(PDstandard6th1phib,2) - 0.25*gxxL*(2*(dphiaSqr + dphibSqr + 
      pow(phiaL,2)*(gcouple*pow(phibL,2) + pow(phiamass,2)) + 
    ...
    CCTK_REAL SFTij21 CCTK_ATTRIBUTE_UNUSED = 
      ...
      PDstandard6th1phib*PDstandard6th2phib - 0.25*gxyL*(2*(dphiaSqr + 
      dphibSqr + pow(phiaL,2)*(gcouple*pow(phibL,2) + pow(phiamass,2)) + 
      pow(phibL,2)*pow(phibmass,2)) + lambdaphian*pow(pow(phiaL,2) - 
      pow(phiav,2),2) + lambdaphibn*pow(pow(phibL,2) - pow(phibv,2),2));
    
    CCTK_REAL SFTij31 CCTK_ATTRIBUTE_UNUSED = 
      PDstandard6th1phia*PDstandard6th3phia + 
      PDstandard6th1phib*PDstandard6th3phib - 0.25*gxzL*(2*(dphiaSqr + 
      dphibSqr + pow(phiaL,2)*(gcouple*pow(phibL,2) + pow(phiamass,2)) + 
      pow(phibL,2)*pow(phibmass,2)) + lambdaphian*pow(pow(phiaL,2) - 
      pow(phiav,2),2) + lambdaphibn*pow(pow(phibL,2) - pow(phibv,2),2));
    
    CCTK_REAL SFTij22 CCTK_ATTRIBUTE_UNUSED = pow(PDstandard6th2phia,2) + 
      pow(PDstandard6th2phib,2) - 0.25*gyyL*(2*(dphiaSqr + dphibSqr + 
      pow(phiaL,2)*(gcouple*pow(phibL,2) + pow(phiamass,2)) + 
      pow(phibL,2)*pow(phibmass,2)) + lambdaphian*pow(pow(phiaL,2) - 
      pow(phiav,2),2) + lambdaphibn*pow(pow(phibL,2) - pow(phibv,2),2));
    ...
    CCTK_REAL SFTij32 CCTK_ATTRIBUTE_UNUSED = 
      ...
      PDstandard6th2phib*PDstandard6th3phib - 0.25*gyzL*(2*(dphiaSqr + 
      dphibSqr + pow(phiaL,2)*(gcouple*pow(phibL,2) + pow(phiamass,2)) + 
      pow(phibL,2)*pow(phibmass,2)) + lambdaphian*pow(pow(phiaL,2) - 
      pow(phiav,2),2) + lambdaphibn*pow(pow(phibL,2) - pow(phibv,2),2));
    
    CCTK_REAL SFTij33 CCTK_ATTRIBUTE_UNUSED = pow(PDstandard6th3phia,2) + 
      pow(PDstandard6th3phib,2) - 0.25*gzzL*(2*(dphiaSqr + dphibSqr + 
      pow(phiaL,2)*(gcouple*pow(phibL,2) + pow(phiamass,2)) + 
      pow(phibL,2)*pow(phibmass,2)) + lambdaphian*pow(pow(phiaL,2) - 
      pow(phiav,2),2) + lambdaphibn*pow(pow(phibL,2) - pow(phibv,2),2));
  ...
  CCTK_ENDLOOP3(EvolveScalarFields_ScalarFieldrho_doublewell_6th);
}
extern "C" void EvolveScalarFields_ScalarFieldrho_doublewell_6th(CCTK_ARGUMENTS)
{
  #ifdef DECLARE_CCTK_ARGUMENTS_EvolveScalarFields_ScalarFieldrho_doublewell_6th
  DECLARE_CCTK_ARGUMENTS_CHECKED(EvolveScalarFields_ScalarFieldrho_doublewell_6th);
  #else
  DECLARE_CCTK_ARGUMENTS;
  #endif
  DECLARE_CCTK_PARAMETERS;
  ...
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Entering EvolveScalarFields_ScalarFieldrho_doublewell_6th_Body");
  }
  if (cctk_iteration % EvolveScalarFields_ScalarFieldrho_doublewell_6th_calc_every != EvolveScalarFields_ScalarFieldrho_doublewell_6th_calc_offset)
  {
    return;
  ...
  AssertGroupStorage(cctkGH, "EvolveScalarFields_ScalarFieldrho_doublewell_6th", 9, groups);
  
  EnsureStencilFits(cctkGH, "EvolveScalarFields_ScalarFieldrho_doublewell_6th", 4, 4, 4);
  
  LoopOverInterior(cctkGH, EvolveScalarFields_ScalarFieldrho_doublewell_6th_Body);
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Leaving EvolveScalarFields_ScalarFieldrho_doublewell_6th_Body");
  }
}
