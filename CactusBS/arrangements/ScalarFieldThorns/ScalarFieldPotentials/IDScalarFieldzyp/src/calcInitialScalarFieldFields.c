   ...
   */
   ...
   if ( CCTK_EQUALS(field_type, "Bubbles") )
   {
      for (int i=0; i < num_clouds; i++)
      {
         ...
         else if ( CCTK_EQUALS(scalar_profile, "InverseShallowGaussian" ) )
         {
            curr_phia = phi0a[i]*(1 - exp(-chia2));
            curr_phib = phi0b[i]*(1 - exp(-chib2));
            ...
            {
               ret_phia         += curr_phia;
               ret_grad_phia[0] += 2*curr_phia/SQR(sigma0a[i])*xxa;
               ret_grad_phia[1] += 2*curr_phia/SQR(sigma0a[i])*yya;
               ret_grad_phia[2] += 2*curr_phia/SQR(sigma0a[i])*zza;
               ret_pia += 0.0;

               ret_phib         += curr_phib;
               ret_grad_phib[0] += 2*curr_phib*rrb/SQR(sigma0b[i])*xxb/sqrt(rb2);
               ret_grad_phib[1] += 2*curr_phib*rrb/SQR(sigma0b[i])*yyb/sqrt(rb2);
               ret_grad_phib[2] += 2*curr_phib*rrb/SQR(sigma0b[i])*zzb/sqrt(rb2);
               ret_pib += 0.0;
            }
         }
         ...
         else if ( CCTK_EQUALS(scalar_profile, "Y21Tanh" ) )
         {
            ...
            if ( ra2 != 0 || rb2 != 0)
            {
               ...
               ret_grad_phia[0] += -eps * phi0a[i]/sigma0a[i]*(1-SQR(tanh(rra/sigma0a[i])))*xxa/sqrt(ra2) * ReY21a + eps * ReY21CONST * Tanka * (zza/ra2 * ( 1 - 2*SQR(xxa)/ra2 ) );
               ret_grad_phia[1] += -eps * phi0a[i]/sigma0a[i]*(1-SQR(tanh(rra/sigma0a[i])))*yya/sqrt(ra2) * ReY21a + eps * ReY21CONST * Tanka * (-2 * xxa * yya * zza/SQR(ra2) );
               ret_grad_phia[2] += -eps * phi0a[i]/sigma0a[i]*(1-SQR(tanh(rra/sigma0a[i])))*zza/sqrt(ra2) * ReY21a + eps * ReY21CONST * Tanka * (xxa/ra2 * ( 1 - 2*SQR(zza)/ra2 ) );
               ret_pia += 0.0;

               ReY21b = ReY21CONST * xxb*zzb/rb2;
               Tankb  = phi0b[i] * (1 - tanh( rrb/sigma0b[i] ));
               ret_phib += eps * Tankb * ReY21b;
               ret_grad_phib[0] += -eps * phi0b[i]/sigma0b[i]*(1-SQR(tanh(rrb/sigma0b[i])))*xxb/sqrt(rb2) * ReY21b + eps * ReY21CONST * Tankb * (zzb/rb2 * ( 1 - 2*SQR(xxb)/rb2 ) );
               ret_grad_phib[1] += -eps * phi0b[i]/sigma0b[i]*(1-SQR(tanh(rrb/sigma0b[i])))*yyb/sqrt(rb2) * ReY21b + eps * ReY21CONST * Tankb * (-2 * xxb * yyb * zzb/SQR(rb2) );
               ret_grad_phib[2] += -eps * phi0b[i]/sigma0b[i]*(1-SQR(tanh(rrb/sigma0b[i])))*zzb/sqrt(rb2) * ReY21b + eps * ReY21CONST * Tankb * (xxb/rb2 * ( 1 - 2*SQR(zzb)/rb2 ) );
               ret_pib += 0.0;
            }
         }
         ...
         else if (CCTK_EQUALS(scalar_profile, "fermi_sin"))
         {
            ...
            ret_grad_phia[0] += phiBG * epsilon * (k * cos(k * xxa) * sin(k * yya) * sin(k * zza) * (pow(exp((sqrt(ra2) - rcutoff)/alpha) + 1.0 , -1)) + sin(k*xxa) * sin(k*yya) * sin(k*zza) * (-1 * pow((exp((sqrt(ra2) - rcutoff)/alpha) + 1 ),-2)) * (xxa/alpha)*sqrt(ra2));

            ret_grad_phia[1] += phiBG * epsilon * (k * sin(k * xxa) * cos(k * yya) * sin(k * zza) * (pow(exp((sqrt(ra2) - rcutoff)/alpha)+1.0,-1)) + sin(k*xxa) * sin(k*yya) * sin(k*zza) * (-1 * pow(exp((sqrt(ra2) - rcutoff)/alpha) + 1 , -2)) * (yya/alpha)*sqrt(ra2));

            ret_grad_phia[2] += phiBG * epsilon * (k * sin(k * xxa) * sin(k * yya) * cos(k * zza) * (pow(exp((sqrt(ra2) - rcutoff)/alpha)+1.0,-1)) + sin(k*xxa) * sin(k*yya) * sin(k*zza) * (-1 * pow(exp((sqrt(ra2) - rcutoff)/alpha) + 1, -2)) * (zza/alpha)*sqrt(ra2));

            ret_phib +=  phiBG * (1  + epsilon*(sin(k*xxb) * sin(k*yyb) * sin(k*zzb)) * (pow(exp((sqrt(rb2) - rcutoff)/alpha) + 1.0 , -1 ) ) );

            ret_grad_phib[0] += phiBG * epsilon * (k * cos(k * xxb) * sin(k * yyb) * sin(k * zzb) * (pow(exp((sqrt(rb2) - rcutoff)/alpha) + 1.0 , -1)) + sin(k*xxb) * sin(k*yyb) * sin(k*zzb) * (-1 * pow((exp((sqrt(rb2) - rcutoff)/alpha) + 1 ),-2)) * (xxb/alpha)*sqrt(rb2));

            ret_grad_phib[1] += phiBG * epsilon * (k * sin(k * xxb) * cos(k * yyb) * sin(k * zzb) * (pow(exp((sqrt(rb2) - rcutoff)/alpha)+1.0,-1)) + sin(k*xxb) * sin(k*yyb) * sin(k*zzb) * (-1 * pow(exp((sqrt(rb2) - rcutoff)/alpha) + 1 , -2)) * (yyb/alpha)*sqrt(rb2));

            ret_grad_phib[2] += phiBG * epsilon * (k * sin(k * xxb) * sin(k * yyb) * cos(k * zzb) * (pow(exp((sqrt(rb2) - rcutoff)/alpha)+1.0,-1)) + sin(k*xxb) * sin(k*yyb) * sin(k*zzb) * (-1 * pow(exp((sqrt(rb2) - rcutoff)/alpha) + 1, -2)) * (zzb/alpha)*sqrt(rb2));
         }
         ...
         else if (CCTK_EQUALS(scalar_profile, "fermi_cos"))
         {
            ...
            ret_grad_phia[0] += phiBG * epsilon * (k * -sin(k * xxa) * cos(k * yya) * cos(k * zza) * (pow(exp((sqrt(ra2) - rcutoff)/alpha) + 1.0 , -1) + cos(k*xxa) * cos(k*yya) * cos(k*zza) * (-1 * pow((exp((sqrt(ra2) - rcutoff)/alpha) + 1 ),-2)) * (xxa/alpha)*sqrt(ra2));

            ret_grad_phia[1] += phiBG * epsilon * (k * cos(k * xxa) * -sin(k * yya) * cos(k * zza) * (pow(exp((sqrt(ra2) - rcutoff)/alpha)+1.0,-1)) + cos(k*xxa) * cos(k*yya) * cos(k*zza) * (-1 * pow(exp((sqrt(ra2) - rcutoff)/alpha) + 1 , -2)) * (yya/alpha)*sqrt(ra2));

            ret_grad_phia[2] += phiBG * epsilon * (k * cos(k * xxa) * cos(k * yya) * -sin(k * zza) * (pow(exp((sqrt(ra2) - rcutoff)/alpha)+1.0,-1)) + cos(k*xxa) * cos(k*yya) * cos(k*zza) * (-1 * pow(exp((sqrt(ra2) - rcutoff)/alpha) + 1, -2)) * (zza/alpha)*sqrt(ra2));

            ret_phib +=  phiBG * (1  + epsilon*(cos(k*xxb) * cos(k*yyb) * cos(k*zzb)) * (pow(exp((sqrt(rb2) - rcutoff)/alpha) + 1.0 , -1 ) ) );

            ret_grad_phib[0] += phiBG * epsilon * (k * -sin(k * xxb) * cos(k * yyb) * cos(k * zzb) * (pow(exp((sqrt(rb2) - rcutoff)/alpha) + 1.0 , -1)) + cos(k*xxb) * cos(k*yyb) * cos(k*zzb) * (-1 * pow((exp((sqrt(rb2) - rcutoff)/alpha) + 1 ),-2)) * (xxb/alpha)*sqrt(rb2));

            ret_grad_phib[1] += phiBG * epsilon * (k * cos(k * xxb) * -sin(k * yyb) * cos(k * zzb) * (pow(exp((sqrt(rb2) - rcutoff)/alpha)+1.0,-1)) + cos(k*xxb) * cos(k*yyb) * cos(k*zzb) * (-1 * pow(exp((sqrt(rb2) - rcutoff)/alpha) + 1 , -2)) * (yyb/alpha)*sqrt(rb2));

            ret_grad_phib[2] += phiBG * epsilon * (k * cos(k * xxb) * cos(k * yyb) * -sin(k * zzb) * (pow(exp((sqrt(rb2) - rcutoff)/alpha)+1.0,-1)) + cos(k*xxb) * cos(k*yyb) * cos(k*zzb) * (-1 * pow(exp((sqrt(rb2) - rcutoff)/alpha) + 1, -2)) * (zzb/alpha)*sqrt(rb2));
   ...
   if (potential)
   {
      ...
      else if ( CCTK_EQUALS(potential_type,"Vpablo") )
      {
         double eta, C0, lambdap;
         eta = (-3.0*C3+sqrt(9.0*C3*C3-32.0*C2))/8.0;
         C0 = -(C2*eta*eta + C3*eta*eta*eta + eta*eta*eta*eta);
         lambdap = V0/C0;

         ret_VV = lambdap*(C0 + C2*ret_phia*ret_phia + C3*ret_phia*ret_phia*ret_phia + ret_phia*ret_phia*ret_phia*ret_phia);
      }
