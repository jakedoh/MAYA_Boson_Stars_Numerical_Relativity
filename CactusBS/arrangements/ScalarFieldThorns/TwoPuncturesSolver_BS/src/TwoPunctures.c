...
CCTK_REAL TwoPuncturesSolver_GetConformalFactor(CCTK_REAL x, CCTK_REAL y, CCTK_REAL z)
{
	...
	getSourceInfo(x, y, z, junk3, &junk, &psiguess, Aij_flattened);
...
/*        fprintf(stderr,"x[ind] %f move_origin_x %f x_grid_move %f\n",x[ind],move_origin_x,x_grid_moved);*/
	...
	double psiguess, junk, junk3[3], Aij_flattened[6], Phi[3], confPi[3], vel[3], gradphi[3];
	getSourceInfo(x[ind], y[ind], z[ind], junk3, &junk, &psiguess, Aij_flattened);
	getInitialPhi(x[ind], y[ind], z[ind], Phi);
	getInitialConformalPi(x[ind], y[ind], z[ind], confPi);
        ...
        else if (CCTK_Equals(initial_lapse, "psi^6")) {
          if (black_hole_psi6) {
             DECLARE_CCTK_PARAMETERS;
             double x_bh, d, dmax, f;

             x_bh = black_hole_xcoord;
             dmax = maximum_distance;
             d = sqrt(pow2(x[ind] - x_bh) + pow2(y[ind]) + pow2(z[ind]));

             if (d <= dmax) {
                f = 1./(1. + 1./(d+1e-15));
                alp[ind] = getconfAlpha(x[ind], y[ind], z[ind])*pow(psi1, 8.*f - 2.);
             } else {
                alp[ind] = getconfAlpha(x[ind], y[ind], z[ind])*pow6(psi1);
             }
          } else {
	     alp[ind] = getconfAlpha(x[ind], y[ind], z[ind])*pow6(psi1);
          }
        ...
        scalarphia[ind] = Phi[0];                //Re(Phi)
        scalarphib[ind] = Phi[1];                //Im(Phi)
        scalarpia[ind] = confPi[0]/pow6(psi1);   //Re(Pi)
        scalarpib[ind] = confPi[1]/pow6(psi1);   //Im(Pi)
  }
